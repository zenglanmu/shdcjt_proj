  	...
  	<TABLE class="ViewForm">
		...
		<TR>
			<TD><%=SystemEnv.getHtmlLabelName(345,user.getLanguage())%></TD>
		  	<TD class="Field">
		  		<TEXTAREA class="InputStyle" name="description" rows="5" style="width:98%" onchange="checkinput('description','descriptionImage')"></TEXTAREA><SPAN id="descriptionImage"><IMG src="/images/BacoError.gif" align="absMiddle"></SPAN>
	      	</TD>
		</TR>
		<TR style="height:1px"><TD class="Line" colSpan="2"></TD></TR> 
<%if(isgoveproj==0){%>
		...
		<TR>
			<TD><%=SystemEnv.getHtmlLabelName(782,user.getLanguage())%></TD>
		 	<TD class="Field">
				...
				_displayTemplate="<A href='/proj/data/ViewProject.jsp?#b{id}' target='_blank'>#b{name}</A>&nbsp;">
				<SPAN id="projectSpan">
				<%
					if(!projectID.equals("")) 
					{
...
function clear(dates,e){
	...
	$("#"+$(target).attr("id")+"Span").html("<IMG src='/images/BacoError.gif' align=absMiddle>");
}
