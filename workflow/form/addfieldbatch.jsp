...
<tr>
  ...
  <TABLE class=viewform>
    ...
    <TR class=spacing style="height:1px;"> 
      <TD class=line1 colSpan=2  style='height:1px;padding:0;margin:0px;'></TD>
    </TR>
    </TBODY> 
  </TABLE> 
  	  <table class=ListStyle id="oTable" cols=5  border=0 cellspacing=1>
            ...
            <td NOWRAP width='21%'><%=SystemEnv.getHtmlLabelName(15456,user.getLanguage())%></td>
            <td NOWRAP width='50%' ><%=SystemEnv.getHtmlLabelName(686,user.getLanguage())%></td>
	    	...
	    	<td>
	    		<table id="detailTable_<%=tableNumber%>" class=ListStyle cols=5  border=0 cellspacing=1>
	    				<input type="hidden" value="" name="detaildelids_<%=tableNumber%>">
							...
							<tr style="height:1px;"><td colspan=5 class=line1 colSpan=2 style='height:1px;padding:0;margin:0px;'></td></tr>
								<tr class=header>
								<td  NOWRAP width='3%'><%=SystemEnv.getHtmlLabelName(1426,user.getLanguage())%></td>
								<td  NOWRAP width='20%'><%=SystemEnv.getHtmlLabelName(15024,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(685,user.getLanguage())%></td>
								<td  NOWRAP width='21%'><%=SystemEnv.getHtmlLabelName(15456,user.getLanguage())%></td>
...
<script language="JavaScript">
	...
	function addDetailTable(){
		...
		var sHtml = "<table id='detailTable_"+detailtables+"' class=ListStyle cols=5  border=0 cellspacing=1>"+
								...
								"<tr>"+
									"<td colspan=2><b><%=SystemEnv.getHtmlLabelName(18550,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(87,user.getLanguage())%>:<%=SystemEnv.getHtmlLabelName(19325,user.getLanguage())%>"+$G("detailtables").value+"</b></td>"+
									"<td colspan=3 align='right'>"+
	...
	for (loop=copyedRowArray.length-1; loop >=0 ;loop--){
		...
		if(itemFieldType==3){//浏览按钮
			...
			if(broswerType_value==161||broswerType_value==162){
				...
				$G("sapbrowser_"+detailtables+"_"+nowRowIndex).value=$G("definebroswerType_"+detailtables+"_"+fromRow).value;
		...
		if(itemFieldType==5){//选择框
			...
			//同一个表单，如果多个下拉菜单有同一个子字段，则无法控制子字段的选项受谁控制
			//$G("childfieldid_detail"+detailtables).value = $G("childfieldid_detail"+detailtables).value;
			//$G("childfieldidSpan_detail"+detailtables).innerHTML = $G("childfieldidSpan_detail"+detailtables).innerHTML;
			for(var tempindex=1;tempindex<fromrows;tempindex++){
				...
				//$G("childItem_detail"+detailtables+"_"+nowRowIndex+"_"+tempindex).value = $G("childItem_detail"+detailtables+"_"+fromRow+"_"+tempindex).value;
				//$G("childItemSpan_detail"+detailtables+"_"+nowRowIndex+"_"+tempindex).innerHTML = $G("childItemSpan_detail"+detailtables+"_"+fromRow+"_"+tempindex).innerHTML;
			}
		}
		//为新行赋值
	}
...
function addDetailRow(detailtables){
    ...
    if((detailrowindex%2)==1){rowColor="#e7e7e7";}
    else{rowColor="#f5f5f5";}
				...
				case 3:
					...
					var sHtml = "<select class='InputStyle' name='itemFieldType_"+detailtables+"_"+detailrowindex+"'  onChange='onChangDetailItemFieldType("+detailtables+","+detailrowindex+")'>"+
                      ...
                      "<div id=detail_div2_"+detailtables+"_"+detailrowindex+" style='display:none'>"+
                      	...
                      	"<input class='InputStyle' type='text' size=4 maxlength=2 value=4 id=textheight_"+detailtables+"_"+detailrowindex+" name='textheight_"+detailtables+"_"+detailrowindex+"' onKeyPress='ItemPlusCount_KeyPress()' onblur='checkPlusnumber1(this);checkcount1(textheight_"+detailtables+"_"+detailrowindex+")' style='text-align:right;'>"+
...
function onChangDetailItemFieldType(detailtableid,detailtablerowid){
		...
		if(itemFieldType==1){
			...
			$G("detail_div3_5_"+detailtables+"_"+detailtablerowid).style.display='none';
...
function addRow(){			
	...
	for(j=0; j<ncol; j++) {
		...
		switch(j){
			...
			case 4:
				var oDiv = document.createElement("div");
				var sHtml = " <input class='InputStyle' type='text' style='width:95%' maxlength=7 name='itemDspOrder_"+rowindex+"' value='"+(rowindex*1 +1)+".00'  onKeyPress='ItemNum_KeyPress(\"itemDspOrder_"+rowindex+"\")' onchange='checknumber(\"itemDspOrder_"+rowindex+"\");checkDigit(\"itemDspOrder_"+rowindex+"\",15,2)' style='text-align:right;'>";						   
				oDiv.innerHTML = sHtml;
				oCell.appendChild(oDiv);
				break;				
			case 5:
			...
			if(check_formself(frmMain,check_String)){
				...
				if($G("itemFieldType_"+j).value==5){//选择框可选项文字check
					...
					for(var tempchoiceRows=1;tempchoiceRows<=choiceRows;tempchoiceRows++){
						if($G("field_"+j+"_"+tempchoiceRows+"_name")&&$G("field_"+j+"_"+tempchoiceRows+"_name").value==""){//选择框的可选项文字必填
							alert("<%=SystemEnv.getHtmlNoteName(14,user.getLanguage())%>");
							return;
						}
					}
