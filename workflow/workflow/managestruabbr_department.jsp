...
String titlename = SystemEnv.getHtmlLabelName(22764,user.getLanguage())+"£º"+SystemEnv.getHtmlLabelName(124,user.getLanguage());
String needfav ="1";
String needhelp ="";
	...
	<td valign="top">
     ...
     <table class=viewform cellspacing=1  >
        ...
        <tr class=header>
            ...
            <td class=field colspan=5><input type=text name=departmentNameOfSearch class=Inputstyle value="<%=departmentNameOfSearch%>"></td>
        </tr>
        <TR>
     ...
     <table class=liststyle cellspacing=1  >
        ...
        </tr><TR class=Line><TD colspan="2" style="padding: 0px!important;"></TD></TR>
...
if(operatelevel>-1){
    ...
    abbrSb.append(" select HrmDepartment.id as departmentId,HrmDepartment.departmentName,workflow_deptAbbrDef.id as deptAbbrDefId,workflow_deptAbbrDef.abbr ")
	      .append("   from HrmDepartment left join  workflow_deptAbbrDef ")
	      .append("     on HrmDepartment.id=workflow_deptAbbrDef.departmentId ")
    ...
    if(!departmentNameOfSearch.equals("")){
		abbrSb.append("    and HrmDepartment.departmentName like '%").append(departmentNameOfSearch).append("%' ");
	}
    abbrSb.append("  order by HrmDepartment.showOrder asc,HrmDepartment.id asc ");

    RecordSet.executeSql(abbrSb.toString());
    while(RecordSet.next()){
	    tempDepartmentId     =Util.getIntValue(RecordSet.getString("departmentId"),0);
	    tempDepartmentName   =Util.null2String(RecordSet.getString("departmentName"));
	    tempDeptAbbrDefId  =Util.getIntValue(RecordSet.getString("deptAbbrDefId"),0);
	    tempAbbr=Util.null2String(RecordSet.getString("abbr"));

%>
        <TR class="<%=trClass%>">

            <td  height="23" align="left"><%=tempDepartmentName%>
            <input type="hidden" name="abbr<%=rowNum%>_departmentId" value="<%=tempDepartmentId%>">
            </td>
            <input type="hidden" name="abbr<%=rowNum%>_deptAbbrDefId" value="<%=tempDeptAbbrDefId%>">
