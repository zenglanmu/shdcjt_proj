...
if(detachable==0){
    response.sendRedirect("WorkflowCodeSeq_frmtwo.jsp?sqlwhere="+sqlwhere);
    return;
}
%>
<HTML><HEAD>
...
<IFRAME name=leftframe id=leftframe src="WorkflowCodeSeq_left.jsp?rightStr=StartCode:Maintenance&sqlwhere=<%=sqlwhere%>" width="100%" height="100%" frameborder=no scrolling=no>
...
<IFRAME name=contentframe id=contentframe src="WorkflowCodeSeq_frmtwo.jsp?sqlwhere=<%=sqlwhere%>" width="100%" height="100%" frameborder=no scrolling=yes>
<%=SystemEnv.getHtmlLabelName(15017,user.getLanguage())%></IFRAME>
</td>
</tr>
  </TBODY>
