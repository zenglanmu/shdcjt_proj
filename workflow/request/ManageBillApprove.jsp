...
%>
	  ...
	  <%
	  if (approvetype.equals("9")) 	{%>	
		  <span id="approveSpan" name="approveSpan" ><A href="/docs/docs/DocDsp.jsp?id=<%=approveid%>&isrequest=1&requestid=<%=requestid%>"  target='_new'><%=DocComInfo.getDocname(approveid)%></A></span>
	  <%}
	  	  if (approvetype.equals("10")) 	{%>	
		  <span id="approveSpan" name="approveSpan" ><A href="/CRM/data/ViewCustomer.jsp?CustomerID=<%=approveid%>&isrequest=1&requestid=<%=requestid%>"  target='_new'><%=CustomerInfoComInfo.getCustomerInfoname(approveid)%></A></span>
	  <%}
		  if (approvetype.equals("11")) 	{%>	
		  <span id="approveSpan" name="approveSpan" ><A href="/proj/data/ViewProject.jsp?ProjID=<%=approveid%>&isrequest=1&requestid=<%=requestid%>"  target='_new'><%=ProjectInfoComInfo.getProjectInfoname(approveid)%></A></span>
	  <%}%>
...
sub onShowApprove(approvetype)
	...
	if id(0)<> "" then
		if (approvetype="9") then
			approveSpan.innerHtml = "<A href='/docs/docs/DocDsp.jsp?id="&id(0)&"'>"&id(1)&"</A>"
		end if
		if (approvetype="10") then
			approveSpan.innerHtml = "<A href='/CRM/data/ViewCustomer.jsp?CustomerID="&id(0)&"'>"&id(1)&"</A>"
		end if
		if (approvetype="11") then
			approveSpan.innerHtml = "<A href='/proj/data/ViewProject.jsp?ProjID="&id(0)&"'>"&id(1)&"</A>"
		end if
