...
while(RecordSet.next()){
 ...
 <table class=form>
        ...
        <tr>
            ...
            <BUTTON Class=Btn type=button accessKey=E onclick="if(isdel()){deleteRow1();}"><U>E</U>-删除</BUTTON>
            </td>
        </tr>
        <%  } %>
        <tr>
			...
			<div>
                ...
                if(fieldhtmltype.equals("1")){                          // 单行文本框
                    ...
                    if(isedit.equals("1") && isremark==0 ){
                      ...
                      %>
                      <button class=Browser onclick="onShowBrowser2('<%=fieldid%>_<%=recorderindex%>','<%=url%>','<%=linkurl%>','<%=fieldtype%>','<%=ismand%>')" title="选择"></button>
                       <%       } else {                         // 如果是多文档字段,加入新建文档按钮
                       %>
                       <button class=AddDoc onclick="onShowBrowser2('<%=fieldid%>_<%=recorderindex%>','<%=url%>','<%=linkurl%>','<%=fieldtype%>','<%=ismand%>')" > 添加</button>&nbsp;&nbsp;<button class=AddDoc onclick="onNewDoc('<%=fieldid%>_<%=recorderindex%>')" title="新建">新建</button>
                       <%       }
                    }
                       %>
    ...
    for(int i=0;i<fieldids1.size();i++){         // 循环开始
        ...
        if(fieldhtmltype.equals("1")){                          // 单行文本框
            if(fieldtype.equals("1")){                          // 单行文本框中的文本
                if(isedit.equals("1")){
                    if(ismand.equals("1")) {
                        fieldhtml = "<input datatype='text' type=text name='field"+fieldid+"_\"+rowindex+\"' size=10 onChange='checkinput1(field"+fieldid+"_\"+rowindex+\",field"+fieldid+"_\"+rowindex+\"span)'><span id='field"+fieldid+"_\"+rowindex+\"span'><img src='/images/BacoError.gif' align=absmiddle></span>" ;
                    }else{
                ...
                if(isedit.equals("1")){
                    if(ismand.equals("1")) {
                        fieldhtml = "<input datatype='float' type=text name='field"+fieldid+"_\"+rowindex+\"' size=10 onKeyPress='ItemNum_KeyPress()' onBlur='checknumber1(this);checkinput1(field"+fieldid+"_\"+rowindex+\",field"+fieldid+"_\"+rowindex+\"span);calSum()'><span id='field"+fieldid+"_\"+rowindex+\"span'><img src='/images/BacoError.gif' align=absmiddle></span>" ;
                    }else{
...
%>
        ...
        <%
           for(int j=0;j<fieldids1.size();j++){
            ...
            out.println(calExp+"; if("+calExp.substring(0,calExp.indexOf("value")-1)+".datatype=='int') "+calExp.substring(0,calExp.indexOf("="))+"=toPrecision("+calExp.substring(0,calExp.indexOf("="))+",0);else "+calExp.substring(0,calExp.indexOf("="))+"=toPrecision("+calExp.substring(0,calExp.indexOf("="))+",3);}");
	        %>
