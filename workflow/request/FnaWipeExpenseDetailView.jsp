...
//System.out.println("organizationIdsOfDept="+organizationIdsOfDept);

//int requestid=Util.getIntValue((String)request.getAttribute("requestid"));
//int creater=Util.getIntValue((String)request.getAttribute("creater"));
...
//User user = HrmUserVarify.getUser (request , response) ;
    ...
    //RecordSet.executeSql("select sum(amount) from fnaloaninfo where organizationtype=2 and organizationid="+deptid);
    //RecordSet.next();
    ...
    //System.out.println(" select subject , budgetperiod from Bill_FnaWipeApplyDetail where organizationtype=3 and organizationid="+creater+" and id = " + billid + " order by subject ");
...
//        int relatedcrm=RecordSet.getInt(4);
        //System.out.println("tempbudgetperiod"+tempbudgetperiod);
...
<table  class="viewform" border=1 bordercolor='black'>
  <tbody>
  <TR class="Title">
      <TH><%=SystemEnv.getHtmlLabelName(16961,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(15805,user.getLanguage())%>
...
((TreeSet)crmsubjectmaps.get(""+relatedcrm)).add(s);
    ...
    <div id=odiv_3 style="display:none">
      ...
      for(Iterator i0=prjsubjectmaps.keySet().iterator();i0.hasNext();){
	    ...
	    <TR class="Title">
    	  <TH colSpan=4><%=SystemEnv.getHtmlLabelName(101,user.getLanguage())%>£º<%=Util.toScreen(ProjectInfoComInfo.getProjectInfoname(projectid_str),user.getLanguage())%></TH>
        </TR>
        <tr class=datalight >
    ...
    <div id=odiv_4 style="display:none">
      ...
      for(Iterator i0=crmsubjectmaps.keySet().iterator();i0.hasNext();){
	    ...
	    <TR class="Title">
    	  <TH colSpan=4>CRM£º<%=Util.toScreen(CustomerInfoComInfo.getCustomerInfoname(crmid_str),user.getLanguage())%></TH>
        </TR>
        <tr class=datalight >
