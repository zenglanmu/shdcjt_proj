...
function openSignPrint() {
  var redirectUrl = "PrintRequest.jsp?requestid=<%=requestid%>&isprint=1&fromFlowDoc=1&urger=<%=logs_urger%>&ismonitor=<%=logs_monitor%>" ;
  <%//解决相关流程打印权限问题
  String wflinkno = Util.null2String((String) session.getAttribute(requestid+"wflinkno"));
  if(!wflinkno.equals("")){
  %>
  redirectUrl = "PrintRequest.jsp?requestid=<%=requestid%>&isprint=1&fromFlowDoc=1&isrequest=1&wflinkno=<%=wflinkno%>&urger=<%=logs_urger%>&ismonitor=<%=logs_monitor%>";
  <%}%>
