...
function setmantable(){
    ...
    try{
    	...
    	if(temprow2>0){
	      ...
	      document.getElementById("requestlevel").value="<%=requestlevel%>";
    	...
    	if(temprow3>0){
	      ...
	      document.getElementById("messageType").value="<%=rqMessageType%>";
...
<%}%>
            ...
            <%
	    		...
	    		<%}else{%>
	    			wcell.SetCellVal(temprow4,tempcol4,""); 
	    	<%}}%>
		   	document.getElementById("qianzi").value="<%=annexdocids%>";
...
%>
                document.getElementById("<%=tmpfid%>_linkno").value="<%=linknums%>";
<%
            }
...
%>
                    var temporgtype=3;
                    if(document.getElementById("<%=organizationtype%>_<%=tmprow%>")) temporgtype=document.getElementById("<%=organizationtype%>_<%=tmprow%>").value;
                    wcell.SetCellProtect(nrow,ncol,nrow,ncol,false);
                    ...
                    if (document.getElementById("<%=organizationid%>_<%=tmprow%>_url")) {
                        document.getElementById("<%=organizationid%>_<%=tmprow%>_url").value = turl;
                    }
                    if (document.getElementById("<%=organizationid%>_<%=tmprow%>_urllink")) {
                        document.getElementById("<%=organizationid%>_<%=tmprow%>_urllink").value = turllink;
                    }
<%
                    }
...
%>
	        if(document.getElementById("dtl_id_<%=i%>_<%=k%>")!=null){
			    document.getElementById("dtl_id_<%=i%>_<%=k%>").value="<%=detailids.get(k)%>";
	        }
<%
        }
    }
...
<%							}
            ...
            if(!haspreAdditional&&preAdditionalValue.equals("")){
            	if(ftype==1 ||ftype==17 ||ftype==165 ||ftype==166) fvalue=""+user.getUID();
            	if(ftype==2) {
            		fvalue=""+currentdate;
            	...
            	if(ftype==164){ //浏览按钮为分部,取当前操作者为默认值(由人力资源的分部得到分部默认值)
                fvalue = ResourceComInfo.getSubCompanyID(""+user.getUID());
              }
              if(ftype==4){ //浏览按钮为部门或多部门,取当前操作者为默认值(由人力资源的分部得到分部默认值)
                fvalue = ResourceComInfo.getDepartmentID(""+user.getUID());
              }
              if(ftype==24){ //浏览按钮为职务,从前面的参数中获得人默认值(由人力资源的职务得到职务默认值)
            ...
            if(!fvalue.equals("")){
                ...
                if(ftype==2 || ftype==19){
                    ...
                    //部门，多部门
                    for(int k=0;k<tempshowidlist.size();k++){
                        showname+=Util.StringReplace(DepartmentComInfo1.getDepartmentname((String)tempshowidlist.get(k)),",","，")+",";
                    }
                }else if(ftype==164){
                    //分部
                    for(int k=0;k<tempshowidlist.size();k++){
                        showname+=Util.StringReplace(SubCompanyComInfo1.getSubCompanyname((String)tempshowidlist.get(k)),",","，")+",";
                    }
...
for(int i=0;i<NodeList.size();i++){
    ...
    String nodemark=FieldInfo.GetNodeRemark(workflowid,nodeid,Util.getIntValue(paranodeid));
...
%>
            ...
            var tcalfields=document.getElementById("calfields").value;
            if((","+tcalfields+",").indexOf(",<%=calfid%>,")<0){
                tcalfields+=",<%=calfid%>";
                document.getElementById("calfields").value=tcalfields;
            }
        }
    }
<%
...
%>
               ...
               try{
                        var caldetfields=document.getElementById("caldetfields").value;
                        if((","+caldetfields+",").indexOf(",<%=calfid%>,")<0){
                            caldetfields+=",<%=calfid%>";
                            document.getElementById("caldetfields").value=caldetfields;
                        }
               }catch(e){}
<%
            }
...
function createDocForNewTab(tmpfid,isedit){
   ...
   for(i=0;i<=1;i++){
  	...
  	parent.document.getElementById("oTDtype_1").background="/images/tab.active2.png";
  	parent.document.getElementById("oTDtype_1").className="cycleTDCurrent";
  	*/
...
function doInitDetailChildComboAll(field,pField,fvalue,pFieldValue,rownum){
	try{
		...
		frm.style.display = "none";
	    document.body.appendChild(frm);
		document.getElementById("iframe_"+pField+"_"+field+"_"+rownum).src = "ComboChange.jsp?"+paraStr;
	}catch(e){}
}
