         ...
         <tr>
             ...
             <td class=field>
                 <span id="Intervenorspan">
                     ...
                     <%}%><%=_intervenorusernames%><%if(_intervenorusernames.equals("")){%><img src='/images/BacoError.gif' align=absmiddle><%}%></span>
...
<%
         ...
         %>
              ...
              <textarea  class=Inputstyle id="remark" name=remark rows=4 cols=40 style="width:80%;display:none" temptitle="<%=SystemEnv.getHtmlLabelName(17614,user.getLanguage())%>"   <%if(isSignMustInput.equals("1")){%>onChange="checkinput('remark','remarkSpan')"<%}%>><%=myremark%></textarea>
	  		   	<script defer>
...
<table style="width:100%;height:100%" border=0 cellspacing=0 cellpadding=0  scrolling=no >
  ...
  <TBODY>
	  ...
	  <%if(showDocTab_edit.equals("1")){%><td nowrap name="oTDtype_1"  id="oTDtype_1" background="url(/images/tab2.png) no-repeat" width=79px align=center onmouseover="style.cursor='hand'"  onclick='signtabchange(1)'>
	  <%=SystemEnv.getHtmlLabelName(857,user.getLanguage())%></td><%}%>
      <%if(showWorkflowTab_edit.equals("1")){%><td nowrap name="oTDtype_2"  id="oTDtype_2" background="url(/images/tab2.png) no-repeat" width=79px  align=center onmouseover="style.cursor='hand'"  onclick="signtabchange(2)">
	  <%=SystemEnv.getHtmlLabelName(1044,user.getLanguage())%></td><%}%>
	  <%if(showUploadTab_edit.equals("1")){%><td nowrap name="oTDtype_3"  id="oTDtype_3" background="url(/images/tab2.png) no-repeat" width=79px align=center onmouseover="style.cursor='hand'"  onclick='signtabchange(3)'>
...
function signtabchange(tabindex){
    if(tabindex==0){
        ...
        $GetEle("oTDtype_2").background="url(/images/tab.active2.png) no-repeat";
        <%}%>
        <%if(showUploadTab_edit.equals("1")){%>
        $GetEle("SignTabUpload").style.display='none';
        $GetEle("oTDtype_3").style.fontWeight="normal";
        ...
        $GetEle("oTDtype_3").background="url(/images/tab.active2.png) no-repeat";
        <%}%>
        if(tabupload==1){
...
<%}%>
	...
	function checkNodesNum()
	{
		...
		if(_isbill>0)
		{
			if(_formid==7||_formid==156 || _formid==157 || _formid==158)
			{
				%>
