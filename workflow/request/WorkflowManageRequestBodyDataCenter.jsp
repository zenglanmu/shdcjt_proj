...
%>
						...
						旬：<select name="day">
						<option value="05" <%if(dayTemp.compareTo("10") < 0 ) {%>selected<%}%>>上旬</option>
						<option value="15" <%if(dayTemp.compareTo("10")>=0 && dayTemp.compareTo("20")<0) {%>selected<%}%>>中旬</option>
						<option value="25" <%if(dayTemp.compareTo("20") >=0 ) {%>selected<%}%>>下旬</option>
						</select>
						<%}%>
						<% if(inprepfrequenceTemp.equals("6")) { %>
						半年：<select name="month">
						<option value="01" <%if(monthTemp.compareTo("07") < 0 ) {%>selected<%}%>>上半年</option>
						<option value="07" <%if(monthTemp.compareTo("07") >=0 ) {%>selected<%}%>>下半年</option>
						</select>
						<%}%>
						<% if(inprepfrequenceTemp.equals("7")) { %>
						季：<select name="month">
						<option value="01" <%if(monthTemp.compareTo("04") < 0 ) {%>selected<%}%>>一季度</option>
						<option value="04" <%if(monthTemp.compareTo("04")>=0 && monthTemp.compareTo("07")<0) {%>selected<%}%>>二季度</option>
						<option value="07" <%if(monthTemp.compareTo("07")>=0 && monthTemp.compareTo("10")<0) {%>selected<%}%>>三季度</option>
						<option value="10" <%if(monthTemp.compareTo("10") >=0 ) {%>selected<%}%>>四季度</option>
						</select>
						<%}%>
						<%}  else {%>
	...
	if ($G(fieldidnum).value=="0")
	  {
		...
		$G(fieldidspan).innerHTML="<IMG src='/images/BacoError.gif' align=absMiddle>";
        }
