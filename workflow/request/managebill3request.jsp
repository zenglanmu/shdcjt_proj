...
<table class=form>
  ...
  <tr>
  	...
  	<%if(requestname.equals("")){%><span id=requestnamespan><IMG src="/images/BacoError.gif" align=absMiddle><%}%></span>
  	</td>
  	<%} else {%>
...
<%
	if(fieldhtmltype.equals("1")){
		...
		<%}%>
		<%if(ismand.equals("1")){%><%if(fieldvalue.equals("")){%><img src="/images/BacoError.gif" align=absmiddle><%}%>
		<%	needcheck+=",field"+fieldid;	
			}%>
		</span>
		<%
		...
		switch(j) {
			...
			case 1: 
				...
				var sHtml = "<button class=Browser onClick='onShowAsset(node_"+rowindex+"_assetspan,node_"+rowindex+"_assetid)'></button> " + 
        					"<span class=saveHistory id=node_"+rowindex+"_assetspan><img src='/images/BacoError.gif' align=absMiddle></span> "+
