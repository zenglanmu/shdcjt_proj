...
<%
        ...
        if(htmltype==3){
            ...
            if(!haspreAdditional && preAdditionalValue.equals("")){
                ...
                if((ftype==1 ||ftype==17 ||ftype==165 ||ftype==166) && !hrmid.equals("")){ //浏览按钮为人,从前面的参数中获得人默认值
                    fvalue =hrmid;
                }
                ...
                if((ftype==4 ||ftype==57 ||ftype==167 ||ftype==168) && !hrmid.equals("")){ //浏览按钮为部门,从前面的参数中获得人默认值(由人力资源的部门得到部门默认值)
                    fvalue = ResourceComInfo.getDepartmentID(hrmid);
                }
                if((ftype==164 || ftype==169 || ftype==170) && !hrmid.equals("")){ //浏览按钮为分部,从前面的参数中获得人默认值(由人力资源的分部得到分部默认值)
                    fvalue = ResourceComInfo.getSubCompanyID(hrmid);
                }
                if(ftype==24 && !hrmid.equals("")){ //浏览按钮为职务,从前面的参数中获得人默认值(由人力资源的职务得到职务默认值)
