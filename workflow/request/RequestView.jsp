...
<%@page import="org.json.JSONObject"%> <%@ include file="/systeminfo/init.jsp" %>
    ...
    for(int i=0;i<typerowcounts;i++){
        ...
        for(int j=0;j<workflowList.size();j++){
			...
			if(ovtimenum>0){
				...
				wfText+="<a href = # onClick=javaScript:loadGrid('"+paras+"',true)  >"+ovtimenum+"</a><IMG src='/images/BDOut.gif' align=center BORDER=0> &nbsp;/&nbsp;";
				over=ovtimenum+over;;
			}
        ...
        String wfText ="";
        if(over>0){
        	wfText+=over+"<IMG src='/images/BDOut.gif' align=center BORDER=0> &nbsp;/&nbsp;";
        }
        if(newremark1>0){
        	wfText+=newremark1+"<IMG src='/images/BDNew.gif' align=center BORDER=0> &nbsp;/&nbsp;";
        }
        if(newremark2>0){
        	wfText+=newremark2+"<IMG src='/images/BDNew2.gif' align=center BORDER=0> &nbsp;/&nbsp;";
        }
        //wfText+=")";
		...
		<%} %>
            <tr>
                ...
                <table class="viewform">
                    ...
                    <%
					int typerowcounts=(wftypeList.size()+1)/2;
					%>
                    <tr>
                        <!--左边table-->
                        <td valign=top>
                                ...
                                <tr>
                                    <td>
                                            ...
                                            for(int i=0;i<typerowcounts;i++){
                                            	...
                                            	if(fromAdvancedMenu==1){
                                            	%>
                                                <a href="/workflow/search/WFSearchTemp.jsp?method=reqeustbywftypeNode&fromadvancedmenu=<%=fromAdvancedMenu%>&infoId=<%=infoId%>&wftype=<%=typeid%>&complete=0&workFlowIDsRequest=<%=workFlowIDsRequest%>&workFlowNodeIDsRequest=<%=workFlowNodeIDsRequest%>&selectedContent=<%=selectedContent %>&menuType=<%=menuType %>" >
                                                <% } else { %>
                                                <a href="/workflow/search/WFSearchTemp.jsp?method=reqeustbywftype&wftype=<%=typeid%>&complete=0">
                                            	<% } /* edited end */ %>
                                                <%=Util.toScreen(typename,user.getLanguage())%></a>&nbsp;(<%=Util.toScreen(typecount,user.getLanguage())%>)
                                                </span></td>
													</tr>
												</table>
                                                <UL>
                                                    ...
                                                    for(int j=0;j<workflowList.size();j++){
                                                        ...
                                                        if(!curtypeid.equals(typeid))	continue;
                                                        workflowcount=(String)workflowcountList.get(j);
                                                        //newremarkwfcount0=(String)newremarkwfcount0List.get(j);
                                                        //newremarkwfcount1=(String)newremarkwfcount1List.get(j);
                                                        workflowname=WorkflowComInfo.getWorkflowname(workflowid);
                                                        int tempind=wftypeworkflowList.indexOf(typeid+","+workflowid);
                                                        int ovtimenum=0;
                                                        if(tempind>-1){
                                                        ...
                                                        %>
                                                        <LI>
														<div sytle="float:left;margin-right:120px;">
														<a href="/workflow/search/WFSearchTemp.jsp?method=reqeustbywfidNode&workflowid=<%=workflowid%>&nodeids=<%=t_nodeids%>&complete=0">
                                                        <%=Util.toScreen(workflowname,user.getLanguage())%></a>&nbsp;
                                                        (
                                                            <%if(ovtimenum>0){%>
                                                            <a href="/workflow/search/WFSearchTemp.jsp?method=reqeustbywfidNode&workflowid=<%=workflowid%>&nodeids=<%=t_nodeids%>&complete=8">
                                                            <%=ovtimenum%></a><IMG src="/images/BDOut.gif" align=absmiddle BORDER=0>
                                                            &nbsp;/
                                                            <%}if(!newremarkwfcount0.equals("0")){%>
                                                            <a href="/workflow/search/WFSearchTemp.jsp?method=reqeustbywfidNode&workflowid=<%=workflowid%>&nodeids=<%=t_nodeids%>&complete=3">
                                                            <%=Util.toScreen(newremarkwfcount0,user.getLanguage())%></a><IMG src="/images/BDNew.gif" align=absmiddle BORDER=0>
                                                            &nbsp;/
                                                         <%}%>
                                                         <%if(!newremarkwfcount1.equals("0")){%>
                                                            <a href="/workflow/search/WFSearchTemp.jsp?method=reqeustbywfidNode&workflowid=<%=workflowid%>&nodeids=<%=t_nodeids%>&complete=4">
                                                        ...
                                                        <%
                                                        workflowList.remove(j) ;
                                                        workflowcountList.remove(j) ;
                                                        //newremarkwfcount0List.remove(j) ;
                                                        //newremarkwfcount1List.remove(j) ;
                                                        j-- ;
                        ...
                        <table class="viewform">
                            <tr>
                                <td>
                                        ...
                                        <%
                                        for(int i=typerowcounts;i<wftypeList.size();i++){
                                            typeid=(String)wftypeList.get(i);
                                            typecount=(String)wftypecountList.get(i);
                                            typename=WorkTypeComInfo.getWorkTypename(typeid);
											String workFlowIDsRequest = "";
											...
											if(!"".equals(workFlowIDsRequest)){
												workFlowIDsRequest = workFlowIDsRequest.substring(0, workFlowIDsRequest.length()-1);
											}
											if(!"".equals(workFlowNodeIDsRequest)){
												workFlowNodeIDsRequest = workFlowNodeIDsRequest.substring(0, workFlowNodeIDsRequest.length()-1);
											}
                                            ...
                                            for(int j=0;j<workflowList.size();j++){
												...
												String t_nodeids = Util.null2String((String)wfNodeHahstable.get(workflowid));
                                                %>
                                                <LI><a href="/workflow/search/WFSearchTemp.jsp?method=reqeustbywfidNode&workflowid=<%=workflowid%>&nodeids=<%=t_nodeids%>&complete=0">
                                                <%=Util.toScreen(workflowname,user.getLanguage())%></a>&nbsp;
                                                (
                                                    <%if(ovtimenum>0){%>
                                                            <a href="/workflow/search/WFSearchTemp.jsp?method=reqeustbywfidNode&workflowid=<%=workflowid%>&nodeids=<%=t_nodeids%>&complete=8">
