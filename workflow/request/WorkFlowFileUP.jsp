              ...
              <td colspan="2">
                  <div>
                      ...
                      &nbsp;&nbsp;
                                ...
                                &nbsp;&nbsp;&nbsp;&nbsp;<span style="color:#262626;cursor:hand;TEXT-DECORATION:none" onclick="dosubmit();" id="btnSubmit<%=fieldid%>"><img src="/images/docs/submit.gif" border="0">
...
if(!fieldvalue.trim().equals("")){
    ...
    while(RecordSet.next()){
        ...
        if(fieldtype.equals("2")){
              ...
              %>
           	<% 
            if(!"1".equals(forbidAttDownload) && AttachmentCounts>1 && !nodownload && linknum==0&&urger==0 && ismonitor==0){
            %>
            <tr> 
              <td colSpan=3><nobr>
                  <button type=button class=btn accessKey=1  onclick="top.location='/weaver/weaver.file.FileDownload?fieldvalue=<%=fieldvalue%>&download=1&downloadBatch=1&requestid=<%=requestid%>'">
                     ...
                     <table>
                      ...
                      <tr>
                              ...
                              <td align="center"><nobr>
                                  <a href="#" style="text-decoration:underline" onmouseover="this.style.color='blue'" onclick='onChangeSharetype("span<%=fieldid%>_id_<%=linknum%>","<%=fieldid%>_del_<%=linknum%>");return false;'>[<span style="cursor:hand;color:black;"><%=SystemEnv.getHtmlLabelName(91,user.getLanguage())%></span>]</a>
                              ...
                              <%}
                                  if (urger==0 && ismonitor==0&&!nodownload) {
                              %>
                              <td align="center"><nobr>
        ...
        <tr>
            ...
            <td valign="top">
                ...
                <BUTTON class=btn accessKey=1  onclick='onChangeSharetype("span<%=fieldid%>_id_<%=linknum%>","<%=fieldid%>_del_<%=linknum%>")'><U><%=linknum%></U>-<%=SystemEnv.getHtmlLabelName(91,user.getLanguage())%>
            ...
            <td valign="top">
              <span id = "selectDownload">
                <%if(urger==0 && ismonitor==0&&((!Util.isExt(fileExtendName))||!nodownload)){%>
                <BUTTON class=btn accessKey=1  onclick="addDocReadTag('<%=showid%>');top.location='/weaver/weaver.file.FileDownload?fileid=<%=docImagefileid%>&download=1&requestid=<%=requestid%>&desrequestid=<%=desrequestid%>'">
...
function openDocPDF(showid,versionid,docImagefileid,isedit){
	jQuery($GetEle("flowbody")).attr("onbeforeunload", "");
	openFullWindowHaveBar("/docs/docs/DocDsp.jsp?id="+showid+"&imagefileId="+docImagefileid+"&isFromAccessory=true&isrequest=1&requestid=<%=requestid%>&desrequestid=<%=desrequestid%>");
}
