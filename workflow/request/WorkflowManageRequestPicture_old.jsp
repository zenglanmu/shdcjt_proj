...
String bkcol = "";          // ????¡¤??¨°????
int icount = 0;             // ????????
int curnodetype = 0;        // ????¡Á???  0: ???¡§??  1: ?¡À?¡ã 2:???¨¹

ArrayList operatednode = new ArrayList();               // ???¡è¡Á¡Â?¡Â???¨´?????¡§????????
ArrayList operaternode = new ArrayList();               // ?¨´????¡¤?¡À?????¡Á¡Â????¡Á¡Â??
ArrayList operaternode_ = new ArrayList();               // ?¨´????¡¤?¡À?????¡Á¡Â????¡Á¡Â?? added by xwj on 2005-05-15 for td1838
ArrayList viewernode = new ArrayList();                 // ?¡À?¡ã?????¨´?????¨¦????
ArrayList canoperaternode = new ArrayList();            // ?¡À?¡ã???????¨´??????¡Á¡Â??
...
while(rs.next()){
	icount ++;
    if(Util.null2String(rs.getString("nodeid")).equals(currentnodeid)){     // ?¡À?¡ã????
        curnodetype = 1;
        bkcol = "#005979";
    }
...
onclick="return oc_CurrentMenuOnClick(<%=icount%>)" onmouseout="return oc_CurrentMenuonmouseout(<%=icount%>)" >
    ...
    %>
        <%if(curnodetype==0){%>
	<img src="/images/iconemp.gif" title="<%=SystemEnv.getHtmlLabelName(16353,user.getLanguage())%>">
		<%
    ...
    %>
	<img src="/images/imgPersonHead.gif" title="<%=SystemEnv.getHtmlLabelName(16354,user.getLanguage())%>">
	<%
    ...
    <%
         if(curnodetype==0){
    %>
	 <TR id=D3><TD class=MenuPopup><img src="/images/iconemp.gif" title="<%=SystemEnv.getHtmlLabelName(16353,user.getLanguage())%>">
	<%
	        for(int i=0;i<operaternode.size();i++){
                String tmp = ""+operaternode.get(i);
                if((tmp).startsWith(""+rs.getString("nodeid")+"_"))
    ...
    %>
	<TR id=D1><TD class=MenuPopup><img src="/images/imgPersonHead.gif" title="<%=SystemEnv.getHtmlLabelName(16354,user.getLanguage())%>">
	<%
            for(int i=0;i<canoperaternode.size();i++){
                String tmp = ""+canoperaternode.get(i);
                String tmptype = tmp.substring(tmp.lastIndexOf("_")+1);
	...
	<TR id=D2><TD class=MenuPopup><img src="/images/icon_resource_flat.gif" title="<%=SystemEnv.getHtmlLabelName(16355,user.getLanguage())%>">
	<%
