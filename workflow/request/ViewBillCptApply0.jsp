...
while(RecordSet.next()){
		...
		<td <%if(dsptypes.indexOf("2_0")!=-1){%> style="display:none" <%}%>><a href='/cpt/capital/CptCapital.jsp?id=<%=RecordSet.getString("cptid")%>'><%=Util.toScreen(CapitalComInfo.getCapitalname(RecordSet.getString("cptid")),user.getLanguage())%> </a></td>
		...
		<td <%if(dsptypes.indexOf("7_0")!=-1){%> style="display:none" <%}%>><%=RecordSet.getString("needdate")%></td>
		<td <%if(dsptypes.indexOf("8_0")!=-1){%> style="display:none" <%}%>><%=Util.toScreen(RecordSet.getString("purpose"),user.getLanguage())%></td>
		<td <%if(dsptypes.indexOf("9_0")!=-1){%> style="display:none" <%}%>><%=Util.toScreen(RecordSet.getString("cptdesc"),user.getLanguage())%></td>            
	   </tr>
