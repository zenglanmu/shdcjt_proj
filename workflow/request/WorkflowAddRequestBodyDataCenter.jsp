...
%>
						...
						旬：<select name="day">
						<option value="05" <%if(currentdayTemp.compareTo("10") < 0 ) {%>selected<%}%>>上旬</option>
						<option value="15" <%if(currentdayTemp.compareTo("10")>=0 && currentdayTemp.compareTo("20")<0) {%>selected<%}%>>中旬</option>
						<option value="25" <%if(currentdayTemp.compareTo("20") >=0 ) {%>selected<%}%>>下旬</option>
						</select>
						<%}%>
						<% if(inprepfrequenceTemp.equals("6")) { %>
						半年：<select name="month">
						<option value="01" <%if(currentmonthTemp.compareTo("07") < 0 ) {%>selected<%}%>>上半年</option>
						<option value="07" <%if(currentmonthTemp.compareTo("07") >=0 ) {%>selected<%}%>>下半年</option>
						</select>
						<%}%>
						<% if(inprepfrequenceTemp.equals("7")) { %>
						季：<select name="month">
						<option value="01" <%if(currentmonthTemp.compareTo("04") < 0 ) {%>selected<%}%>>一季度</option>
						<option value="04" <%if(currentmonthTemp.compareTo("04")>=0 && currentmonthTemp.compareTo("07")<0) {%>selected<%}%>>二季度</option>
						<option value="07" <%if(currentmonthTemp.compareTo("07")>=0 && currentmonthTemp.compareTo("10")<0) {%>selected<%}%>>三季度</option>
						<option value="10" <%if(currentmonthTemp.compareTo("10") >=0 ) {%>selected<%}%>>四季度</option>
						</select>
						<%}%>
						<%}  else {%>
