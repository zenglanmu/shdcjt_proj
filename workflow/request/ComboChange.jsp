...
function insertSelect_All(){
	try{
		...
		window.parent.document.getElementById("field<%=childfieldtmpid%>").value="";
		if(ismand>=1 || <%=init%>==1){
				...
				if(comboStr!="" && valueStr!=""){
					...
					for(var i=0; i<selValues.length; i++){
						if(selValues[i]==childFieldValue && childFieldValue!=""){
							...
							window.parent.document.getElementById("field<%=childfieldtmpid%>").value=childFieldValue;
							hasSet = true;
							break;
						}
					}
			...
			if(hasSet == false){
				...
				//alert(usershowtype);
				if(usershowtype==2){
					wcell.ReadHttpImageFile("/images/BacoError.gif",nrowChild,ncolChild,true,true);
				}else{
