...
sql+=" group by Workflow_CustomDspField.fieldid,fieldname,workflow_fieldlable.fieldlable,workflow_formdict.fielddbtype, workflow_formdict.fieldhtmltype,workflow_formdict.type" ;
    ...
    sql+=" group by workflow_billfield.id,workflow_billfield.fieldname,workflow_billfield.fieldlabel,workflow_billfield.fielddbtype, workflow_billfield.fieldhtmltype,workflow_billfield.type" ;
}
}else{
if(isbill.equals("0")){
sql = "select * from (select Workflow_CustomDspField.queryorder ,Workflow_CustomDspField.showorder ,Workflow_CustomDspField.fieldid as id,fieldname as name,workflow_fieldlable.fieldlable as label,workflow_formdict.fielddbtype as dbtype, workflow_formdict.fieldhtmltype as httype,workflow_formdict.type as type from Workflow_CustomDspField,Workflow_Custom, workflow_formdict,workflow_fieldlable,workflow_formfield where workflow_formfield.formid=Workflow_Custom.formid and workflow_formfield.fieldid=workflow_fieldlable.fieldid and  Workflow_CustomDspField.customid=Workflow_Custom.id and Workflow_Custom.id="+customid+" and Workflow_Custom.isbill='0' and Workflow_CustomDspField.ifquery='1' and workflow_fieldlable.formid = Workflow_Custom.formid and workflow_fieldlable.isdefault = 1 and workflow_fieldlable.fieldid =Workflow_CustomDspField.fieldid and workflow_formdict.id = Workflow_CustomDspField.fieldid ";
}else if(isbill.equals("1")){
    if(RecordSet.getDBType().equals("oracle")){
...
if(issimple){
    sql+=" union select queryorder,showorder,fieldid as id,'' as name,'' as label,'' as dbtype,'' as httype,0 as type from Workflow_CustomDspField where ifquery='1' and fieldid in(-1,-2,-3,-4,-5,-6,-7,-8,-9) and customid="+tempcustomid;
}else{
    sql+=" union select queryorder,showorder,fieldid as id,'' as name,'' as label,'' as dbtype,'' as httype,0 as type from Workflow_CustomDspField where ifquery='1' and fieldid in(-7,-9) and customid="+tempcustomid;
}
...
function onShowWorkFlowSerach(inputname, spanname) {
	...
	retValue = window
			.showModalDialog("/systeminfo/BrowserMain.jsp?url=/workflow/workflow/WorkflowBrowser.jsp?sqlwhere=where isvalid='1' ");
