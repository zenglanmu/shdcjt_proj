		...
		<td valign="top">
			...
			<tr> 
				...
				<td class=Field>
					...
					年	
					<INPUT class="InputStyle" type="text" name="name" onchange='checkinput("name","NameSpan")' value="<%=name%>">
					<span id="NameSpan" name="NameSpan"></span>
				</td>
			</tr>
			<TR style="height: 1px!important;">
				<TD class=Line colSpan=2></TD>
			...
			<TR>
				...
				<TD class=Field>
					...
					<input id="HeadDoc" name="HeadDoc" type="hidden" value="<%=HeadDoc%>">
					<SPAN ID="HeadDocSpan" name="HeadDocSpan">
					<%
			...
			<table Class=ListStyle cols=4 id="oTable" cellspacing="1"><COLGROUP>
			  ...
			  <tr class=header> 
			   ...
			   <TR class=Line style="height: 1px!important;"><TD style="padding: 0!important;" colspan="4" ></TD></TR> 
			   <%
				boolean colorFlag = false ;
				int recordCount = 0 ;
				RecordSet.executeSql("select * from WebMagazineDetail where mainID = " + id +" order by id asc");
				while(RecordSet.next()){
				needcheck += ",node_"+recordCount+"_group";
				if (colorFlag){
				%>
				<tr class="DataDark"> 
				<%}else{%>
				<tr class="DataLight"> 
				<%}%>
					<td><input type='checkbox' name='check_node' value="<%=recordCount%>"></td>
					<td>					
					<input class='InputStyle' type='text' name='node_<%=recordCount%>_group' onBlur='checkinput("node_<%=recordCount%>_group","node_<%=recordCount%>_groupspan")' value="<%=RecordSet.getString("name")%>"><span id='node_<%=recordCount%>_groupspan'></span>
					</td>
					<td>
					<input type='checkbox' name='node_<%=recordCount%>_isview' value='1' <%if(RecordSet.getString("isView").equals("1")){%>checked<%}%>>
					</td>
					<td>
					<BUTTON class=Browser type="button" onclick='onShowMDocs("node_<%=recordCount%>_docs","node_<%=recordCount%>_docsspan")'></BUTTON>
					<input id="node_<%=recordCount%>_docs" name="node_<%=recordCount%>_docs" type="hidden" value='<%=Util.null2String(RecordSet.getString("docID"))%>'>
					<SPAN ID="node_<%=recordCount%>_docsspan" name="node_<%=recordCount%>_docsspan">
					<%
			  ...
			  <input type="hidden" name="totaldetail" value="<%=recordCount%>"> 
			  </FORM>
		</td>
		</tr>
		</TABLE>
