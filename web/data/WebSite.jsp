...
<TABLE class=ViewForm>
  ...
  <TBODY>
		...
		<TR>
          <TD>网站栏目名称</TD>
          <TD class=Field>			  
              <input name=name class=inputstyle  onchange='checkinput("name","nameimage");onChangeName();checkinput("linkKey","linkKeySpan")' value="<%if(!name.equals("")){%><%=name%><%}%>"><SPAN id=nameimage><%if(name.equals("")){%><IMG src="/images/BacoError.gif" align=absMiddle><%}%></SPAN>
		  </TD>
        </TR>
        ...
        <TR>
          <TD>访问关键字</TD>
          <TD class=Field>
              <input name=linkKey class=inputstyle  value="<%if(!linkKey.equals("")){%><%=linkKey%><%}%>" onchange='checkinput("linkKey","linkKeySpan")'><SPAN id="linkKeySpan"><%if(linkKey.equals("")){%><IMG src="/images/BacoError.gif" align=absMiddle><%}%>
		  </TD>
        </TR>
		<tr><td class=Line colspan=2></td></tr>
		<TR>
...
sub onNewsID()
    ...
    newsSpan.innerHtml = "<IMG src=/images/BacoError.gif align=absMiddle>"
