...
function TProc(AFlow, id, procType) {
	...
	this.InnerObject = null;
	this.MoveType = "";
	this.Img = this.Flow.Config._ImgPath+this.ProcType+'.gif';
	// 新增
	this.actFlag = "";
	this.waittime = "";
	this.isSltTrans = "1";
...
TProc.prototype.showError = function(errMsg){
	document.all(this.ID).imagedata.src = this.Flow.Config._ImgPath+"wrong.gif";
...
TProc.prototype.lostFocus = function() {
	...
	if(document.all(this.ID)!=null){
		document.all(this.ID).imagedata.src = this.Flow.Config._ImgPath+"background.gif";
	}
}

TProc.prototype.setSelected=function(){
	this.isSelect = true;
	document.all(this.ID).imagedata.src = this.Flow.Config._ImgPath+"hight.gif";
}
...
TTopFlow.prototype.parseXMLToTTopFlow=function(xmlDoc){
	...
	//start_td19600
		...
		oNode.Img = this.Config._ImgPath+oNode.ProcType+'.gif';
		// 新增
...
TTopFlow.prototype.getMaxProcID = function(obj) {
    ...
    */
	...
	//maxProcID = "2010";
	for(var i=0;i<arrProc.length;i++){
		maxProcID+=Math.round(Math.random()*10000)+","
	}
...
TTopFlow.prototype.getMaxStepID = function(arrStep,auto) {
	...
	*/
	for(var i=0;i<arrStep.length;i++){
		maxStepID+=Math.round(Math.random()*10000)+",";
	}
