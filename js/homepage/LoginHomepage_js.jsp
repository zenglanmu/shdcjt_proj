...
<script language=vbs>
	...
	sub onShowMultiCatalog(input,span,eid)	
		id = window.showModalDialog("/systeminfo/BrowserMain.jsp?url=/docs/category/MutilCategoryBrowser.jsp?para="+input.value)
		if NOT isempty(id) then
			if id(0)<> "" then	
				dummyidArray=Split(id(0),",")
				dummynames=Split(id(1),",")
...
{	
    ...
    if(msg==0)
    {
       	if(validatecode.value=='请输入以下验证码') 
           	validatecode.value='';
    }
    else if(msg==1)
    {
        if(validatecode.value=='') 
            validatecode.value='请输入以下验证码';
    }
}
var isClear = false;
var frmLogin;
function checkall(eid,passwordname,passworduser,needvalidate,needusb)
{
	...
	if(needvalidate=="1"){
		var validatecode = frmLogin.validatecode;
		if (validatecode&&(validatecode.value==""||validatecode.value=='请输入以下验证码')) 
		{
			errMessage="请输入以下验证码";
			alert(errMessage);
			validatecode.focus();
