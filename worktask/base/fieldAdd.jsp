...
if(message.equals("1")){
  ...
  <tr>
	...
	<span id=fieldnamespan><%if(fieldname.equals("")){%><IMG src="/images/BacoError.gif" align=absMiddle><%}%></span><span class=fontred>&nbsp&nbsp<br>
		<%=SystemEnv.getHtmlLabelName(15441,user.getLanguage())%>,<%=SystemEnv.getHtmlLabelName(19881,user.getLanguage())%>,<%=SystemEnv.getHtmlLabelName(21998,user.getLanguage())%>
	</span>
	<%}%>
	</td>
	<%}%>
...
<!--xwj for td2977 20051107 end-->
  ...
  <tr>
	...
	<option value="6" <%if(fieldhtmltype.equals("6")){%> selected<%}%>>
		...
		<%if(fieldhtmltype.equals("")){%>
		  <td class=field>
			<BUTTON class=btn type="button" id=btnaddRow style="display:none" name=btnaddRow onclick="addRow()"><%=SystemEnv.getHtmlLabelName(15443,user.getLanguage())%></BUTTON>
  	   		<BUTTON class=btn type="button" id=btnsubmitClear style="display:none" name=btnsubmitClear onclick="submitClear()"><%=SystemEnv.getHtmlLabelName(15444,user.getLanguage())%></BUTTON>
	  ...
	  if(fieldhtmltype.equals("4")){//xwj for @td2977 begin%>
		...
		if(fieldhtmltype.equals("5")){%>
		   <td class=field>
		   <BUTTON class=btn type="button" id=btnaddRow style="display:''" name=btnaddRow onclick="addRow()"><%=SystemEnv.getHtmlLabelName(15443,user.getLanguage())%></BUTTON>
  	   		<BUTTON class=btn type="button" id=btnsubmitClear style="display:''" name=btnsubmitClear onclick="submitClear()"><%=SystemEnv.getHtmlLabelName(15444,user.getLanguage())%></BUTTON>
  ...
  <tr>
  	...
  	<div id=selectdiv <%if(fieldhtmltype.equals("5")){%>style="display:''" <%} else {%>style="display:none"<%}%>>
  	  <table class="ViewForm" id="oTable" cols=5 border=0>
...
<script language=javascript>
	...
	function checksubmit(){
		fieldhtmltypelist = window.document.forms[0].fieldhtmltype;
		htmltypelist = document.getElementById("htmltype");
