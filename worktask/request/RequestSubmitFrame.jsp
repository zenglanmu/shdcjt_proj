...
function resetbanner(objid, typeid){
	...
	//alert("RequestSubmitList.jsp?wtid="+wtid+"&worktaskStatus="+worktaskStatus+"&type="+typeid+"&type_d=<%=type_d%>&objId=<%=objId%>&years="+years);
	o.location="RequestSubmitList.jsp?wtid="+wtid+"&worktaskStatus="+worktaskStatus+"&type="+typeid+"&type_d=<%=type_d%>&objId=<%=objId%>&years="+years;
}
...
function init(objid,typeid){
	...
	var o = parent.contentframe.iframes.document;
	o.location="RequestSubmitList.jsp?wtid=<%=wtid%>&worktaskStatus=<%=worktaskStatus%>&type=<%=type%>&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=years%>&months=<%=months%>&planDate=<%=planDate%>&quarters=<%=quarters%>&weeks=<%=weeks%>";
}
 </script>
...
<table style="width:98%;height:95%" border=0 cellspacing=0 cellpadding=0  scrolling=no id="tabPane">
	...
	<TBODY>
		...
		<%}else{%>
			<iframe src="RequestSubmitList.jsp?type=0&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=currentYear%>&wtid=<%=wtid%>&worktaskStatus=<%=worktaskStatus%>" ID="iframes" name="iframes" frameborder="0" style="width:100%;height:100%;border-right:1px solid #879293;border-bottom:1px solid #879293;border-left:1px solid #879293;padding:10px;padding-right:0" scrolling="auto"/>
		<%}%>
			</td>
		</tr>
		</TBODY>
