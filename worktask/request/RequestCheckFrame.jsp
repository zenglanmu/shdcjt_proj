...
function resetbanner(objid, typeid){
	...
	//alert("RequestCheckList.jsp?wtid="+wtid+"&worktaskStatus="+worktaskStatus+"&type="+typeid+"&type_d=<%=type_d%>&objId=<%=objId%>&years="+years);
	o.location="RequestCheckList.jsp?wtid="+wtid+"&worktaskStatus="+worktaskStatus+"&type="+typeid+"&type_d=<%=type_d%>&objId=<%=objId%>&years="+years+"&isfromleft=<%=isfromleft%>";
}
	...
	<TBODY>
		...
		<td name="oTDtype_6"  id="oTDtype_6">
			<select name="worktaskStatus_tmp" onchange="resetbanner(6,6)" style="width:100%" disabled>
				<option value="9" selected><%=SystemEnv.getHtmlLabelName(21982, user.getLanguage())%></option>
			</select>
			<input type="hidden" name="worktaskStatus" value="9" >
		</td>
		...
		<%}else{%>
			<iframe src="RequestCheckList.jsp?wtid=<%=wtid%>&worktaskStatus=<%=worktaskStatus%>&type=<%=type%>&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=currentYear%>&months=<%=months%>&planDate=<%=planDate%>&quarters=<%=quarters%>&weeks=<%=weeks%>&isfromleft=<%=isfromleft%>" ID="iframes" name="iframes" frameborder="0" style="width:100%;height:100%;border-right:1px solid #879293;border-bottom:1px solid #879293;border-left:1px solid #879293;padding:10px;padding-right:0" scrolling="auto"/>
		<%}%>
			</td>
		</tr>
		</TBODY>
