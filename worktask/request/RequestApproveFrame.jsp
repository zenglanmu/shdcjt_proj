...
function resetbanner(objid, typeid){
	...
	//alert("RequestApproveList.jsp?wtid="+wtid+"&worktaskStatus="+worktaskStatus+"&type="+typeid+"&type_d=<%=type_d%>&objId=<%=objId%>&years="+years);
	o.location="RequestApproveList.jsp?wtid="+wtid+"&worktaskStatus="+worktaskStatus+"&type="+typeid+"&type_d=<%=type_d%>&objId=<%=objId%>&years="+years+"&isfromleft=<%=isfromleft%>";
}
...
function init(objid,typeid){
	...
	var o = parent.contentframe.iframes.document;
	o.location="RequestApproveList.jsp?wtid=<%=wtid%>&worktaskStatus=<%=worktaskStatus%>&type=<%=type%>&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=years%>&months=<%=months%>&planDate=<%=planDate%>&quarters=<%=quarters%>&weeks=<%=weeks%>&isfromleft=<%=isfromleft%>";
}
 </script>
...
<table style="width:98%;height:95%" border=0 cellspacing=0 cellpadding=0  scrolling=no id="tabPane">
	...
	<TBODY>
		...
		<td name="oTDtype_6"  id="oTDtype_6">
			<select name="worktaskStatus_tmp" onchange="resetbanner(6,6)" style="width:100%" disabled>
			<option value="2" selected><%=SystemEnv.getHtmlLabelName(2242, user.getLanguage())%></option>
			</select>
			<input type="hidden" name="worktaskStatus" value="2" >
		</td>
		...
		<%}else{%>
			<iframe src="RequestApproveList.jsp?type=0&type_d=<%=type_d%>&objId=<%=objId%>&years=<%=currentYear%>&wtid=<%=wtid%>&worktaskStatus=<%=worktaskStatus%>&isfromleft=<%=isfromleft%>" ID="iframes" name="iframes" frameborder="0" style="width:100%;height:100%;border-right:1px solid #879293;border-bottom:1px solid #879293;border-left:1px solid #879293;padding:10px;padding-right:0" scrolling="auto"/>
		<%}%>
			</td>
		</tr>
		</TBODY>
