			...
			<TABLE class=Shadow>
				<TR>
					...
					<%
					    ...
					    "<sql backfields=\"t1.id,t1.name,t1.address,t1.customizeAddress,t1.caller,t1.contacter,t1.begindate,t1.begintime,t1.enddate,t1.endtime,t1.meetingstatus,t1.isdecision,(SELECT status FROM Meeting_View_Status WHERE meetingId = t1.id and userId = " + userid + ") AS status\" sqlisdistinct=\"true\" sqlform=\"" + sqlFrom + "\" sqlprimarykey=\"t1.id\" sqlorderby=\"t1.id\" sqlsortway=\"DESC\" sqlwhere=\""+Util.toHtmlForSplitPage(sqlWhere)+"\"/>"+
					    "<head>"+
					    "<col width=\"15%\" text=\""+SystemEnv.getHtmlLabelName(2151,user.getLanguage())+"\" column=\"name\" orderkey=\"name\" transmethod=\"weaver.splitepage.transform.SptmForMeeting.getMeetingName\" otherpara=\"column:id+column:status\" />"+
