...
if(method.equals("add") || method.equals("addSubmit"))
{
    ...
    if(method.equals("addSubmit")){
        ...
        if(!approvewfid.equals("0")&&!approvewfid.equals("")){
            response.sendRedirect("/workflow//BillMeetingOperation.jsp?src=submit&iscreate=1&MeetingID="+MaxID+"&approvewfid="+approvewfid+"&viewmeeting=1");
            return;
        }else{
...
if(method.equals("cancelMeeting"))
{
	...
	while(RecordSet.next()){
	   wfaccepter+=","+RecordSet.getString(1);
	}
	...
	if(RecordSet.next())
	{
		...
		stringBuffer.append("UPDATE WorkPlan SET deleted = '1'");		
		stringBuffer.append(" WHERE meetingId = ");
		stringBuffer.append("'");
		stringBuffer.append(meetingId);
		stringBuffer.append("'");
	...
	response.sendRedirect("/meeting/data/NewMeetings.jsp");
	return;
}
%>
