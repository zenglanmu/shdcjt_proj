...
<IFRAME name=frame2 id=frame2 src="MultiSelect.jsp?tabid=<%=tabid%>&resourceids=<%=resourceids%>" width=100%  height=100% frameborder=no scrolling=no>
浏览器不支持嵌入式框架，或被配置为不显示嵌入式框架。
</IFRAME>
...
<script language=javascript>
	function resetbanner(objid){
		...
		if(objid == 0 ){		        
		        window.frame1.location="SearchByOrgan.jsp";
		        try{
		        window.frame2.btnsub.style.display="none"
		        }catch(err){}
		        }			
		else if(objid == 1){
			window.frame1.location="SearchByGroup.jsp";
			try{
			window.frame2.btnsub.style.display="none"
			}catch(err){}
			}
		else if(objid == 2){
			window.frame1.location="Search.jsp";
			try{
