...
String sqlstr = "";
if(user.getLogintype().equals("1")){
	sqlstr = "select t1.department AS resultid,COUNT(distinct t1.id) AS resultcount from CRM_CustomerInfo  t1,"+leftjointable+"  t2 where t1.id = t2.relateditemid and t1.deleted = 0 group by t1.department order by resultcount";
}else{
	sqlstr = "select t1.department AS resultid,COUNT(t1.id) AS resultcount from CRM_CustomerInfo  t1 where t1.agent="+user.getUID() + "  and t1.deleted = 0  group by t1.department order by resultcount";
}
...
<%  
		PieTeeChart pie=TeeChartFactory.createPieChart(SystemEnv.getHtmlLabelName(15579,user.getLanguage())+SystemEnv.getHtmlLabelName(352,user.getLanguage()),650,300,PieTeeChart.SMS_LabelPercent);
//	pie.isDebug();
	...
	if(totalcustomer!=0){
		...
		pie.addSeries(DepartmentComInfo.getDepartmentname(resultid)+" ",resultcount);

	%>
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    <TD>
    <%if(resultid.equals("0")){%><%=SystemEnv.getHtmlLabelName(557,user.getLanguage())%><%} else {%>
    <a href="/hrm/company/HrmDepartmentDsp.jsp?id=<%=resultid%>">
    ...
    <TD>
    <%if(resultcount!=0){%><a href="/CRM/search/SearchOperation.jsp?msg=report&settype=department&id=<%=resultid%>"><%=resultcount%></a>
    <%} else {%><%=resultcount%><%}%></TD>
    <TD><%=resultpercent%>%</TD>
    </TR>
