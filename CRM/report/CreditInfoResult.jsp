...
int perPage = 10;
String sql = "";
String tempTable = "CITempTable"+ Util.getRandom();
if (credit.equals(""))
	return;
...
if (userType.equals("1")) {
	if (rs.getDBType().equals("oracle"))
		sql = "CREATE TABLE " + tempTable + " AS SELECT * FROM (SELECT DISTINCT"
			...
			+ " FROM CRM_customerinfo t1, CRM_CreditInfo t2, "+leftjointable+" t3"
			+ " WHERE t1.id = t3.relateditemid"
			+ " AND t2.id = " + credit
			+ " AND t1.CreditAmount >= t2.creditamount AND t1.CreditAmount <= t2.highamount"
			+ sqlTerm
			+ " ORDER BY t1.id DESC"
		...
		sql = "CREATE TABLE " + tempTable + " AS  (SELECT DISTINCT"
			+ " t1.id, t1.name, t1.manager, t1.website"
			+ " FROM CRM_customerinfo t1, CRM_CreditInfo t2, "+leftjointable+" t3 ) definition only";
        rs.executeSql(sql);
		...
		sql = "SELECT DISTINCT TOP " + ( pageNum * perPage + 1)
			...
			+ tempTable + " FROM CRM_customerinfo t1, CRM_CreditInfo t2, leftjointable t3"
			+ " WHERE t1.id = t3.relateditemid"
			+ " AND t2.id = " + credit
			+ " AND t1.CreditAmount >= t2.creditamount AND t1.CreditAmount <= t2.highamount"
			+ sqlTerm
	...
	if (rs.getDBType().equals("oracle"))
		sql = "CREATE TABLE " + tempTable + " AS SELECT * FROM (SELECT DISTINCT"
			+ " t1.id, t1.name, t1.manager, t1.website"
			+ " FROM CRM_customerinfo t1, CRM_CreditInfo t2"
			+ " WHERE t1.agent = " + userId
			+ " AND t2.id = " + credit
			+ " AND t1.CreditAmount >= t2.creditamount AND t1.CreditAmount <= t2.highamount"			
			+ sqlTerm
			+ " ORDER BY t1.id DESC"
		...
		sql = "CREATE TABLE " + tempTable + " AS (SELECT DISTINCT"
			+ " t1.id, t1.name, t1.manager, t1.website"
			+ " FROM CRM_customerinfo t1, CRM_CreditInfo t2 ) definition only";
        rs.executeSql(sql);
		...
		sql = "SELECT DISTINCT TOP " + ( pageNum * perPage + 1)
			+ " t1.id, t1.name, t1.manager, t1.website INTO "
			+ tempTable + " FROM CRM_customerinfo t1, CRM_CreditInfo t2"
			+ " WHERE t1.agent = " + userId
			+ " AND t2.id = " + credit
			+ " AND t1.CreditAmount >= t2.creditamount AND t1.CreditAmount <= t2.highamount"
			+ sqlTerm
...
<TR>
	...
	<TD valign="top">
		...
		<TD valign="top">
            ...
            <FORM id="frmmain" name="frmmain" method="post" action="CreditInfoResult.jsp">
...
function goBack() {
	document.location.href="/CRM/report/CreditInfoRpSum.jsp"
}
