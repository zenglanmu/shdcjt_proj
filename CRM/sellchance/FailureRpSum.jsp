...
<tr>
    ...
    <td class=field width=20%> <input  class=wuiBrowser type=hidden name=viewer value="<%=resource%>"
    ...
    <td width=10%><%=SystemEnv.getHtmlLabelName(2247,user.getLanguage())%></td>
...
<TABLE class=ListStyle width="100%" cellspacing=1>
  ...
  <TBODY>
  <TR class=Header>
    <TH><%=SystemEnv.getHtmlLabelName(15104,user.getLanguage())%></TH>
...
String leftjointable = CrmShareBase.getTempTable(""+user.getUID());
    String sqlstr="";
    if(sqlwhere.equals("")){
    	sqlstr = "select t1.defactor AS resultid,COUNT(distinct t1.id) AS resultcount from CRM_SellChance  t1,CRM_Failfactor  t2  ,"+leftjointable+" t4,CRM_CustomerInfo t3  where  t1.defactor=t2.id  and t3.deleted=0 and t3.id= t1.customerid and t1.customerid = t4.relateditemid group by t1.defactor order by resultcount";
    }
    else 
        sqlstr ="select t1.defactor AS resultid,COUNT(distinct t1.id) AS resultcount from CRM_SellChance  t1,CRM_Failfactor  t2  ,"+leftjointable+" t4,CRM_CustomerInfo t3  where  t1.defactor=t2.id  and t3.deleted=0 and t3.id= t1.customerid and t1.customerid = t4.relateditemid "+sqlwhere+"  group by t1.defactor order by resultcount";
  ...
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    ...
    <%=Util.toScreen(SellfailureComInfo.getFaStatusname(resultid),user.getLanguage())%><%}%></TD>
    <TD height="100%">
    ...
    <TD>
    <a href="/CRM/sellchance/SellChanceReport.jsp?defactor=<%=resultid%>&fromdate=<%=fromdate%>&enddate=<%=enddate%>&viewer=<%=resource%>&customer=<%=customer%>&sellstatusid=<%=sellstatusid%>&endtatusid=<%=endtatusid%>&preyield=<%=preyield%>&preyield_1=<%=preyield_1%>"><%=resultcount%></TD>
    <TD><%=resultpercent%>%</TD>
    </TR>
