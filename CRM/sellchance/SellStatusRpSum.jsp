...
<tbody>
<TR  style="height:2px"><TD class=Line1 colSpan=9></TD></TR>
<tr>
...
String leftjointable = CrmShareBase.getTempTable(""+user.getUID());
    String sqlstr = "";
    if(sqlwhere.equals("")){
	    sqlstr ="select t1.sellstatusid AS resultid,COUNT(distinct t1.id) AS resultcount from CRM_SellChance  t1,CRM_SellStatus  t2,"+leftjointable+" t4,CRM_CustomerInfo t3 where t1.sellstatusid=t2.id  and t3.deleted=0 and t3.id= t1.customerid and t1.customerid = t4.relateditemid group by t1.sellstatusid order by resultid" ;
    }else{
        sqlstr = "select t1.sellstatusid AS resultid,COUNT(distinct t1.id) AS resultcount from CRM_SellChance  t1,CRM_SellStatus  t2,"+leftjointable+" t4,CRM_CustomerInfo t3 where t1.sellstatusid=t2.id  and t3.deleted=0 and t3.id= t1.customerid and t1.customerid = t4.relateditemid "+sqlwhere+" group by t1.sellstatusid order by resultid" ;
    }
  ...
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    <TD>  
    <%=Util.toScreen(SellstatusComInfo.getSellStatusname(resultid),user.getLanguage())%></TD>
    <TD height="100%">        
    ...
    <TD><a href="/CRM/sellchance/SellChanceReport.jsp?sellstatusid=<%=resultid%>&fromdate=<%=fromdate%>&enddate=<%=enddate%>&viewer=<%=resource%>&customer=<%=customer%>&endtatusid=<%=endtatusid%>&preyield=<%=preyield%>&preyield_1=<%=preyield_1%>">
    <%=resultcount%></TD>
    <TD><%=resultpercent%>%</TD>
    </TR>
