...
RecordSet.next();
String time=RecordSet.getString("timespan");
String spannum=RecordSet.getString("spannum");
...
<TABLE class=ViewForm>
  ...
  <TR>
	...
	<TH><%=SystemEnv.getHtmlLabelName(61,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(87,user.getLanguage())%></TH>
		...
		<TD class=Field><INPUT text class=InputStyle maxLength=50 size=20 name="time" value="<%=time%>" onBlur='checkcount1(this);checkinput("time","timeimage")' onKeyPress="ItemCount_KeyPress()" ><SPAN id=timeimage>
		<%if(time.equals("")){%>
		...
		<TD class=Field><INPUT text class=InputStyle maxLength=50 size=20 name="spannum" value="<%=spannum%>" onBlur='checkcount1(this);checkinput("spannum","spannumimage")' onKeyPress="ItemCount_KeyPress()" ><SPAN id=spannumimage>  <%if(spannum.equals("")){%>
		<IMG src="/images/BacoError.gif" align=absMiddle>
		<%}%></SPAN></TD>
		</TR><tr  style="height: 1px"><td class=Line colspan=2></td></tr>      
