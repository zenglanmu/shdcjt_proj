...
String menuId = Util.null2String(request.getParameter("id"));
...
if(descLike.equalsIgnoreCase("")){
	...
	LeftMenuInfo editMenuInfo = leftMenuHandler.getMenuInfo(Util.getIntValue(menuId,0));
	

	menuLevel = Util.null2String(String.valueOf(editMenuInfo.getMenuLevel()));
	parentId = Util.null2String(String.valueOf(editMenuInfo.getParentId()));
	systemModuleId = Util.null2String(String.valueOf(editMenuInfo.getRelatedModuleId()));
	iconUrl = Util.null2String(editMenuInfo.getIconUrl());
	labelId = Util.null2String(String.valueOf(editMenuInfo.getLabelId()));
...
<%        if(Util.getIntValue(menuLevel,0)==1){                              %>
    <TD align=left><SPAN id=BacoTitle class=titlename>编辑: 左侧菜单 - 顶级菜单</SPAN></TD>
<%        }else{                                                             %>
    <TD align=left><SPAN id=BacoTitle class=titlename>编辑: 左侧菜单 - 子菜单</SPAN></TD>
<%        }                                                                 %>
    <TD align=right>&nbsp;</TD>
    <TD width=5></TD>
  <tr>
  ...
  <FORM style="MARGIN-TOP: 0px" name=frmView method=post action="ManageLeftMenuOperation.jsp">
   ...
   <input type="hidden" name="operation" value="addLeftMenuInfo">
    ...
    <TABLE class=Form>
          ...
          <TR> 
            <TD>名称</TD>
            <TD><SPAN id=menuNameSpan><%=SystemEnv.getHtmlLabelName(Util.getIntValue(labelId),user.getLanguage())%>
			</SPAN>	
			</TD>
          </TR>
          <TR class=Separator> 
...
<%        if(Util.getIntValue(menuLevel,0)==2){                              %>
          ...
          <TR> 
            <TD>链接页面的地址</TD>
            <TD Class=Field><INPUT  class=FieldxLong name="linkAddress" value="<%=linkAddress%>"></TD>
          </TR>
          <TR>
            <TD>上级菜单</TD>
...
<%			  
			if(menuLevel.equals("1")){				%>
			  <BUTTON class=Browser onClick="onShowPreLeftMenu()"></BUTTON>
<%			  }else{									%>
...
function addLeftMenuInfo(){
    frmView.operation.value = "editLeftMenuInfo";
    document.frmView.submit();
}
...
function onShowPreLeftMenu(){
    var id = window.showModalDialog("/systeminfo/BrowserMain.jsp?url=/systeminfo/menuconfig/Maint/PreTopLevelLeftMenuBrowser.jsp?infoId=<%=menuId%>");
	if(id==null){
		return;
	}
    if(id[0]!=null){
...
function onShowPreSubLeftMenu(){
	...
	var mValue = frmView.menuId.value;
    var id = window.showModalDialog("/systeminfo/BrowserMain.jsp?url=/systeminfo/menuconfig/Maint/PreSubLevelLeftMenuBrowser.jsp?parentId="+pValue+"&menuId="+mValue);
	if(id==null){
		return;
	}
    if(id[0]!=null){
