...
<%        String titleLabel = "";
 ...
 <DIV class=HdrProps></DIV>
  ...
  <FORM style="MARGIN-TOP: 0px" name=frmView method=post action="ManageMainMenuOperation.jsp">
   ...
   <input type="hidden" name="operation" value="addMainMenuInfo">

    <BUTTON class=btn id=btnSave accessKey=S name=btnSave onClick="addMainMenuInfo()"><U>S</U>-保存</BUTTON>
...
<%            if(Util.getIntValue(defaultParentId,0)!=0){
                  MainMenuInfo parentInfo = mainMenuHandler.getMenuInfo(Util.getIntValue(defaultParentId,0));
%>
                  <%=SystemEnv.getHtmlLabelName(parentInfo.getLabelId(),user.getLanguage())%>
<%            }       %>
              </SPAN>
...
%>
                  <%=SystemEnv.getHtmlLabelName(mainMenuInfo.getLabelId(),user.getLanguage())%>
<%            }       %>
              </SPAN>
...
<%        if(!defaultLevel.equals("0")){      %>
          ...
          <TR> 
			...
			<INPUT type="checkbox" name=needRightToVisible value="0" <% if(needRightToVisible.equals("1")) {%>checked<%}%>></TD>
            <TD>权限详细</TD>
          ...
          <TR> 
            <TD>通过开关控制</TD>
            <TD Class=Field colspan=1> 
			<INPUT type="checkbox" name=needSwitchToVisible value="0" <% if(needSwitchToVisible.equals("1")) {%>checked<%}%>></TD>
            <TD>类名称</TD>
            <TD Class=Field colspan=1 >
          ...
          <TR> 
            <TD>通过权限控制</TD>
            <TD Class=Field colspan=1> 
			<INPUT type="checkbox" name=needRightToView value="0" <% if(needRightToView.equals("1")) {%>checked<%}%> ></TD>
            <TD>权限详细</TD>
            <TD Class=Field colspan=4 >
          ...
          <TR> 
            <TD>通过开关控制</TD>
            <TD Class=Field colspan=1> 
			<INPUT type="checkbox" name=needSwitchToView value="0" <% if(needSwitchToView.equals("1")) {%>checked<%}%>></TD>
            <TD>类名称</TD>
            <TD Class=Field colspan=1 >
...
function addMainMenuInfo(){
    frmView.operation.value = "addMainMenuInfo";
    document.frmView.submit();
}
