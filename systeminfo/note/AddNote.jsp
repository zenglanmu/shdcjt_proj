...
<TABLE cellSpacing=0 cellPadding=0 width="100%" border=0>
  ...
  <TR>
    ...
    <TD align=left><SPAN id=BacoTitle class=titlename>添加: 提示</SPAN></TD>
    <TD align=right>&nbsp;</TD>
    <TD width=5></TD>
  <tr>
 <TBODY>
 ...
 <DIV class=HdrProps></DIV>
  <FORM style="MARGIN-TOP: 0px" name=frmView method=post action="NoteOperation.jsp">
    ...
    <br>
        <TABLE class=Form>
          ...
          <%
            ...
            %>
          <TR class=Section> 
            <TH colSpan=5>信息ID已经被占用,请重新选择</TH>
          </TR>
          <%}%>
           <TR> 
            <TD>信息ID</TD>
	...
	if(rs.getDBType().equals("oracle")){
		maxIdSql="select id from (select id  from htmlnoteindex order by id desc) where rownum=1 ";
	}else{
		maxIdSql = "select top 1 id  from htmlnoteindex order by id desc";
	}
...
%>
          <TR> 
            <Td><%=langname%></Td>
            <Td><INPUT class=FieldLong name="notename<%=langid%>"></Td>
          </TR>
<%
}
%>
<input type="hidden" name="operation" value="addnote">
</TABLE>
      </FORM>
      </BODY>
      </HTML>
