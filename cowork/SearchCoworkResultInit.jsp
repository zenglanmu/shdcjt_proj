...
try {
	...
	while(statement.next()){
		...
		if(index!=-1){
			...
			src = "/cowork/images/importent.gif";
		}else{
			title = SystemEnv.getHtmlLabelName(19780,user.getLanguage());
			src = "/cowork/images/notimportent.gif";	    
		}
%>
	    <TR CLASS="<%if(isLight){%>DataDark<%}else{%>DataLight<%}%>" id="thetr" title="<%=Util.toHtml(title1.length()<300?title1:title1.substring(0,300)).replace("<br>","\n")+"..."%>">
            ...
            <td onclick="browseTable_onclick()" onmouseover="browseTable_onmouseover()" onmouseout="browseTable_onmouseout()" style="font-weight:bold"><%if(levelvalue==1){%><img src="/cowork/images/isimport.gif" width=3 height=11 border=0>&nbsp;<%}%><%=tmpname%></td>
			<%}else{%>
            <td onclick="browseTable_onclick()" onmouseover="browseTable_onmouseover()" onmouseout="browseTable_onmouseout()"><%if(levelvalue==1){%><img src="/cowork/images/isimport.gif" width=3 height=11 border=0>&nbsp;<%}%><%=tmpname%></td>
			<%}%>
