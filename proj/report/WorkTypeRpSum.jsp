...
<%
	String sqlstr = "select t1.worktype AS resultid,COUNT(t1.id) AS resultcount from Prj_ProjectInfo  t1,PrjShareDetail  t2 where t1.id=t2.prjid and t2.usertype="+user.getLogintype()+" and t2.userid="+user.getUID()+"  group by t1.worktype order by resultcount";
  ...
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    ...
    <%=Util.toScreen(WorkTypeComInfo.getWorkTypename(resultid),user.getLanguage())%><%}%></TD>
    <TD>
    ...
    <TD>
    <%if(resultcount!=0){%><a href="/proj/search/SearchOperation.jsp?msg=report&settype=worktype&id=<%=resultid%>"><%=resultcount%></a>
    <%} else {%><%=resultcount%><%}%></TD>
    <TD><%=resultpercent%>%</TD>
    </TR>
