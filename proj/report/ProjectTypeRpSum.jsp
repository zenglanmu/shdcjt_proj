...
String titlename = SystemEnv.getHtmlLabelName(352,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(586,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
<%
	String sqlstr = "select t1.prjtype AS resultid,COUNT(t1.id) AS resultcount from Prj_ProjectInfo  t1,PrjShareDetail  t2 where t1.id=t2.prjid and t2.usertype="+user.getLogintype()+" and t2.userid="+user.getUID()+"  group by t1.prjtype order by resultcount";
  ...
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    ...
    <%=Util.toScreen(ProjectTypeComInfo.getProjectTypename(resultid),user.getLanguage())%><%}%></TD>
    <TD>
    ...
    <TD>
    <%if(resultcount!=0){%><a href="/proj/search/SearchOperation.jsp?msg=report&settype=projecttype&id=<%=resultid%>"><%=resultcount%></a>
    <%} else {%><%=resultcount%><%}%></TD>
    <TD><%=resultpercent%>%</TD>
    </TR>
