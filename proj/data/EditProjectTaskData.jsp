...
String isManagerFromView = Util.null2String(request.getParameter("isManagerFromView"));
String inputDisabled = Util.null2String(request.getParameter("inputDisabled"));
...
<!--任务列表-->
                    <TABLE  CLASS="viewForm"  valign="top">
                     ...
                     <input <%=inputDisabled%> type="hidden" onclick="getXmlDocStr1()" value="GetXmlDocStr">
...
<TABLE CLASS="ListStyle" valign="top" cellspacing=1 id="tblTask" onmousedown="mousedown(event)"	onmouseup="mouseup(event)"	onmousemove="mousemove(event)">
                            ...
                            <col width="10%">
                            <col width="8%">                                                      
                            <TR class="Header" <%if("false".equals(isManagerFromView)) out.println("rowStatus='disabled'");%> >
                                <TD></TD>
                                <TD><input <%=inputDisabled%> type="checkbox" onclick="javaScript:onCheckAll(this)" id="chkAllObj"></TD>
                            ...
                            <%out.println(ProjTempletUtil.getEditTaskProjStr(request,Util.getIntValue(ProjID),user.getUID()));%>
                        </TABLE>
                        <TEXTAREA NAME="task_xml" id="task_xml" ROWS="6" COLS="100" style="display:none"><%=ProjTempletUtil.getXmlStr()%></TEXTAREA>
                        <input type="hidden" id="task_iRowIndex" name="task_iRowIndex" value="<%=ProjTempletUtil.getMaxProjectTaskId(Util.getIntValue(ProjID))%>">
                        <input type="hidden" id="task_RowindexNum" name="task_RowindexNum" value="<%=ProjTempletUtil.getProjectTaskNum(Util.getIntValue(ProjID))%>">
</div> <!--divTaskList End-->  
<%
