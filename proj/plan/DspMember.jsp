...
%>
	<TR CLASS=<%if(isLight){%>DataLight<%}else{%>DataDark<%}%>>
          ...
          <TD>
			...
			<img src="/images/project_rank.gif"><a href="/proj/plan/ViewTask.jsp?taskrecordid=<%=RecordSet2.getString("id")%>" ><%=RecordSet2.getString("subject")%></a></TD>
          <TD nowrap>
<%
		String temptaskid="";
...
%>
			<%if(!temprequestcount.equals("0")){%>
				<a href="/proj/plan/ViewTask.jsp?taskrecordid=<%=RecordSet2.getString("id")%>" ><img src="/images/prj_request.gif" border=0 alt="工作流"></a><%=temprequestcount%> 
			<%}%>
			<%if(!tempdoccount.equals("0")){%>
				<a href="/proj/plan/ViewTask.jsp?taskrecordid=<%=RecordSet2.getString("id")%>" ><img src="/images/prj_doc.gif" border=0 alt="文档"></a><%=tempdoccount%>
			<%}%>
		  </TD>
