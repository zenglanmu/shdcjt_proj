...
String statusname = "";
if(Util.getIntValue(ProjectStatusComInfo.getProjectStatusname(status))>0&&Util.getIntValue(ProjectStatusComInfo.getProjectStatusname(status))<=7)
statusname = SystemEnv.getHtmlLabelName(Util.getIntValue(ProjectStatusComInfo.getProjectStatusname(status)),user.getLanguage());
statusname = statusname.equals("")?SystemEnv.getHtmlLabelName(220,user.getLanguage()):statusname ;
...
<%if(!isCurrentActived.equals("2") && ProjectProcessList.getCounts()>0 && ismanager && (status.equals("0"))||status.equals("")){//提交批准%>
	<%
...
%>
			<%if(!temprequestcount.equals("0")){%>
				<a href="/proj/plan/ViewTask.jsp?taskrecordid=<%=ProjectProcessList.getId()%>#anchor_wf" ><img src="/images/prj_request.gif" border=0 alt="<%=SystemEnv.getHtmlLabelName(259,user.getLanguage())%>"></a><%=temprequestcount%> 
			<%}%>
			<%if(!tempdoccount.equals("0")){%>
				<a href="/proj/plan/ViewTask.jsp?taskrecordid=<%=ProjectProcessList.getId()%>#anchor_doc" ><img src="/images/prj_doc.gif" border=0 alt="<%=SystemEnv.getHtmlLabelName(58,user.getLanguage())%>"></a><%=tempdoccount%>
			<%}%>
			<%if(coworkCount>0){%>
				<a href="/cowork/coworkview.jsp?taskIds=<%=taskIds%>&type=all"><img src="/images_face/ecologyFace_2/LeftMenuIcon/MyAssistance.gif" style="border:0" alt="<%=SystemEnv.getHtmlLabelName(17855,user.getLanguage())%>" /></a><%=coworkCount%>
			<%}%>
		  </TD>
         ...
         <td align=center>
            ...
            <a href='TaskOperation.jsp?ProjID=<%=ProjID%>&taskrecordid=<%=ProjectProcessList.getId()%>&method=uporder'><img src="/images/ArrowUpGreen.gif" border=0></a>
            <% } %>
            </td>
            <td width=50%>
            <% if(ismaxindex == -1 && recorderindex != 0 ) { %>
            <a href='TaskOperation.jsp?ProjID=<%=ProjID%>&taskrecordid=<%=ProjectProcessList.getId()%>&method=downorder'><img src="/images/ArrowDownRed.gif" border=0></a>
            <% } %>
            </td>
            </tr>
            </table>-->
