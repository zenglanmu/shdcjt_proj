...
String recursion(int pid,weaver.hrm.User u){
	...
	while(rs.next()){
		...
		if(rs.getInt("menuLevel")==3){
			temp += rs.getString("useCustomName").equals("1") ? "['<img src=\"/images_face/ecologyFace_2/LeftMenuIcon/level3.gif\">','"+rs.getString("customName")+"','"+rs.getString("linkAddress")+"','mainFrame','"+rs.getString("customName")+"'" : "['<img src=\"/images_face/ecologyFace_2/LeftMenuIcon/level3.gif\">','"+SystemEnv.getHtmlLabelName(rs.getInt("labelid"),u.getLanguage())+"','"+rs.getString("linkAddress")+"','mainFrame','"+SystemEnv.getHtmlLabelName(rs.getInt("labelid"),u.getLanguage())+"'" ;
		}else{
...
String recursionRemindInfo(){
	...
	while(rs.next()){
		if(rs.getString("statistic").equals("y"))
			temp += "['<img src=\"/images_face/ecologyFace_2/LeftMenuIcon/level3.gif\">','"+SystemEnv.getHtmlLabelName(rs.getInt("typedescription"),user.getLanguage())+"("+rs.getInt("count")+")','"+rs.getString("link")+"','mainFrame','"+rs.getString("typedescription")+"'],";
		else
			temp += "['<img src=\"/images_face/ecologyFace_2/LeftMenuIcon/level3.gif\">','"+SystemEnv.getHtmlLabelName(rs.getInt("typedescription"),user.getLanguage())+"','"+rs.getString("link")+"','mainFrame','"+rs.getString("typedescription")+"'],";
	}
	if(temp.endsWith(",")){
		return temp.substring(0,temp.length()-1);
	}else{
...
%>
<html>
<link rel="stylesheet" href="/LeftMenu/ThemeXP/theme.css" type="text/css">
<script language="JavaScript" src="/js/JSCookTree.js"></script>
<script language="JavaScript" src="/LeftMenu/ThemeXP/theme.js"></script>
