	...
	<div id="conditions">
		<div class="m-t-15">
			...
			<div class="left" style="margin-left: 3px;">
				...
				<span _img=''><img src="/images/BacoError.gif" align="absMiddle"></span>
			</div>
...
<div class="m-t-15">
	...
	<div class="btnGrayDrop relative left font13 color333" onclick="matchManner(this)" style="padding-right: 15px;">
		<div id="rule"><%=SystemEnv.getHtmlLabelName(30932,user.getLanguage()) %></div>
...
.checkedRadio{
	width: 14px;
	height: 14px;
	background: url("/email/images/checkedRadio.png") no-repeat;
	display: inline-block;
}
.unCheckedRadio{
	width: 14px;
	height: 14px;
	background: url("/email/images/unCheckedRadio.png") no-repeat;
	display: inline-block;
}
</style>
...
function getAndSetValue(himself, name) {
	...
	if($.trim($(himself).val())!=""){
		...
		$(himself).next().html("<img src='/images/BacoError.gif' align='absMiddle'>");
	}
...
function setRuleName(himself) {
	var _value_ = himself.value;
	$("div#addMailRule form").find("[name=ruleName]").val(_value_);
}
...
function submitAddMailRule(himself) {
	var _containForm_ = $("form#addMailRule");
...
function getValueSetToFrom(containForm) {
	...
	$("div#addMailRule div#conditions").find("input[type=hidden]").each(function(){
	...
	$("div#addMailRule div#actions").find("input[type=hidden]").each(function(){
...
function deleteHTML(himself) {
	...
	if(_length_ > 1) {
		...
		alert("<%=SystemEnv.getHtmlLabelName(30934,user.getLanguage()) %>");
	}
}
