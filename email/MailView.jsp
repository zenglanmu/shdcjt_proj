...
function toggleSendFrom(){
	...
	if($("arrow").src.indexOf("Left")==-1){
		styleDisplay = "none";
		$("arrow").src = "/images/mail_arrowLeft.gif";
	}else{
		styleDisplay = "";
		$("arrow").src = "/images/mail_arrowDown.gif";
	}
...
function addToContacter(mailAddress){
	...
	o.src = "/images/loading2.gif";
	new Ajax.Request("MailContacterOperation.jsp", {
		onSuccess : function(resp){
			o.src = "/images/replyDoc/usersuss.gif";
...
function parseEML(){
	if(confirm("<%=SystemEnv.getHtmlLabelName(20496,user.getLanguage())%>")){
		hideRightClickMenu();
		showMsgBox($("actionMsgBox"), "<img src='/images/loading2.gif'> <%=SystemEnv.getHtmlLabelName(20495,user.getLanguage())%>");
...
<tr class="header">
	<td width="50" nowrap><%=SystemEnv.getHtmlLabelName(344, user.getLanguage())//主题%>:</td>
	<td width="100%" style="font-weight:bold"><%=subject%></td>
	<td onclick="toggleSendFrom()" style="cursor:hand;text-align:right"><img id="arrow" src="/images/mail_arrowDown.gif">&nbsp;</td>
</tr>
<tr class="header">
	<td><%=SystemEnv.getHtmlLabelName(2034, user.getLanguage())//发件人%>:</td>
	<td colspan="2">
		<%if(sendfrom.equals("")){%>
			<span style="color:blue;text-decoration:underline;cursor:hand" onclick="redirect()"></span>
			<img src="/images/replyDoc/usersuss.gif" 
			style="vertical-align:middle;cursor:hand;<%if(sendfrom.equals(""))out.print("display:none;");%>" 
			alt="<%=SystemEnv.getHtmlLabelName(19956, user.getLanguage())//添加联系人%>" 
...
<tr class="header">
	<td><%=SystemEnv.getHtmlLabelName(97, user.getLanguage())//发件日期%>:</td>
	<td colspan="2"><%=senddate%></td>
</tr>
<tr class="header">
	<td><%=SystemEnv.getHtmlLabelName(2046, user.getLanguage())//收件人%>:</td>
	<td colspan="2">
	<%
	...
	for(int i=0;i<arraySendTo.length;i++){
		...
		out.println("<img src='/images/replyDoc/usersuss.gif' style='vertical-align:middle;cursor:hand'	alt='"+SystemEnv.getHtmlLabelName(19956, user.getLanguage())+"' onclick='addToContacter(\""+arraySendTo[i]+"\")'/> ");
	}
	%>
	</td>
</tr>
<%if(!sendcc.equals("")){%>
<tr class="header">
	<td><%=SystemEnv.getHtmlLabelName(2084, user.getLanguage())//抄送%>:</td>
	<td colspan="2">
	<%
	...
	for(int i=0;i<arraySendCc.length;i++){
		...
		out.print("<img src='/images/replyDoc/usersuss.gif' style='vertical-align:middle;cursor:hand' alt='"+SystemEnv.getHtmlLabelName(19956, user.getLanguage())+"' onclick='addToContacter(\""+arraySendCc[i]+"\")'/> ");
	}
	%>
	</td>
</tr>
...
%>
<tr class="header">
	<td><%=SystemEnv.getHtmlLabelName(156, user.getLanguage())//附件%>:</td>
	<td colspan="2">
	<%
	while(rs.next()){ 
...
%>
<tr class="header">
	<td nowrap><%=SystemEnv.getHtmlLabelName(258, user.getLanguage())//下载%><%=SystemEnv.getHtmlLabelName(71, user.getLanguage())//邮件%>:</td>
	<td colspan="2">
