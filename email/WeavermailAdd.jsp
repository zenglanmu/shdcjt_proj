...
if(!operation.equals("")) {
    ...
    if(operation.equals("forward")) {
        subject = "Fw:" + Util.toScreen(wmc.getSubject() ,user.getLanguage() , wmc.getSubencode()) ;
        content = "\n\n\n\n -----Original Message-----\n From : "+
                Util.toScreen(wmc.getSendfrom() ,user.getLanguage() , wmc.getFromencode()) +
                "\n Send : " + wmc.getSendDate() +
                "\n To : " + Util.toScreen(wmc.getTO() ,user.getLanguage() , wmc.getToencode()) +
...
<form id=weaver name=weaver ACTION="WeavermailSendOperation.jsp" METHOD="POST" enctype="multipart/form-data">
			  ...
			  <table class=ViewForm>
				...
				<tr>
				  ...
				  <td class=Field>
					...
					<button class=Browser id=SelectLocation onClick="onShowResourceGroup(TO)"></button>(<%=SystemEnv.getHtmlLabelName(16637,user.getLanguage())%>)
						  <span id=TOimage><img src="/images/BacoError.gif" align=absMiddle></span> <%=SystemEnv.getHtmlLabelName(2089,user.getLanguage())%>
				...
				<% if(operation.equals("forward") || operation.equals("send")) {
						...
						while(wmc.next()) {
							...
							//System.out.println("isfileattrachment:"+isfileattrachment);
	            ...
	            if(!location.equals("1")){
	                encodeFileName = new String((wmc.getCurrentFilename()).getBytes("ISO8859_1"), "GBK") ;
