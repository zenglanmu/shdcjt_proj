...
{
RecordSetd.execute("select workPlan.* ,a.planName from workPlan left join HrmPerformancePlanKindDetail a on a.id=workPlan.planProperty where (upPrincipal like '%"+tobjId+",%' or ','||cowork||',' like '%,"+objId+",%') and cycle='"+type+"' and planDate='"+planDate+"' and type_n='6' ");
}
else
{
RecordSetd.execute("select workPlan.* ,a.planName from workPlan left join HrmPerformancePlanKindDetail a on a.id=workPlan.planProperty where (upPrincipal like '%"+tobjId+",%' or ','+cowork+',' like '%,"+objId+",%') and cycle='"+type+"' and planDate='"+planDate+"' and type_n='6' ");
...
<%		}%>
		...
		<button id="addc" name="addc" class=AddDoc onclick="confirms('<%=RecordSetu.getString("id")%>','<%=RecordSetu.getString("downid")%>')"><%=SystemEnv.getHtmlLabelName(16634,user.getLanguage())%></button>
		<%}
		}
