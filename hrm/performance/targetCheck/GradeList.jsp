...
<TABLE class=ListStyle cellspacing=1 >
  ...
  <th><%=SystemEnv.getHtmlLabelName(18191,user.getLanguage())//对应目标%></th>
...
<%
    ...
    <%		}%>
	   ...
	   <% if (Util.null2String(RecordSet.getString("statusr")).equals("3")) {%><%=SystemEnv.getHtmlLabelName(1961,user.getLanguage())%><%}%>
		...
		%><td>
		<BUTTON class="Browser" id="SelectSTD" onclick="onShowSTD('point_<%=i%>','<%=goal%>')"></BUTTON>
...
<%  int minm=0;
    ...
    <%		}%>
		...
		<%if (j==0) {%>
		<TD rowspan="1">
		<input class="inputstyle"  value="<%=point2%>" name="point_0" id="point_0" maxlength="3" size="3" onchange="checkint('point_0')">
        </TD> <%}%>
