...
<tr>
<td  id=oTd2 name=oTd2 width=100% height=60% colspan="3">

<IFRAME name=frame2 id=frame2 src="/hrm/resource/MultiSelectByDec.jsp?tabid=<%=tabid%>&resourceids=<%=resourceids%>&initgroupid=<%=initgroupid%>&coworkid=<%=coworkid%>&sqlwhere=<%=sqlwhere%>" width=100%  height=100% frameborder=no scrolling=no>
浏览器不支持嵌入式框架，或被配置为不显示嵌入式框架。
</IFRAME>
...
<script language=javascript>
	function resetbanner(objid){
		...
		if(objid == 0 ){		        
		        window.frame1.location="/hrm/resource/SearchByOrganByDec.jsp?onlyselfdept=<%=onlyselfdept%>&sqlwhere=<%=sqlwhere%>&departments=<%=departments%>&subcompanyids=<%=subcompanyids%>&isall=<%=isall%>";
			...
			window.frame1.location="/hrm/resource/SearchByGroupByDec.jsp?sqlwhere=<%=sqlwhere%>";
			try{
				$(curDoc).find("#btnsub").css("display","none");
			}catch(err){}
			}
		else if(objid == 2){
			window.frame1.location="/hrm/resource/SearchByDec.jsp?status=<%=status%>&sqlwhere=<%=sqlwhere%>";
			try{
