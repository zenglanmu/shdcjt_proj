...
<IFRAME name=frame2 id=frame2 src="/hrm/resource/browser/transfer/Select.jsp?tabid=<%=tabid%>&resourceids=<%=resourceids%>&sqlwhere=<%=sqlwhere%>" width=100%  height=100% frameborder=no scrolling=no>
浏览器不支持嵌入式框架，或被配置为不显示嵌入式框架。
</IFRAME>
</td>
</tr>
...
<script language=javascript>
	function resetbanner(objid){
		...
		if(objid == 0 ){		        
		        window.frame1.location="/hrm/resource/browser/transfer/SearchByOrganSingle.jsp?sqlwhere=<%=sqlwhere%>";
		        try{
		        window.frame2.btnsub.style.display="none"
		        }catch(err){}
		        }			
		else if(objid == 1){
			window.frame1.location="/hrm/resource/browser/transfer/SearchByGroupSingle.jsp?sqlwhere=<%=sqlwhere%>";
			try{
			window.frame2.btnsub.style.display="none"
			}catch(err){}
			}
		else if(objid == 2){
			window.frame1.location="/hrm/resource/browser/transfer/SearchSingle.jsp?status=<%=status%>&sqlwhere=<%=sqlwhere%>";
			try{
