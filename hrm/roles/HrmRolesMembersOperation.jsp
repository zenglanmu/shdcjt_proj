...
if(!HrmUserVarify.checkUserRight("HrmRolesAdd:Add", user)){
    ...
    if(sharetype.equals("3")){
        ...
        String tmpsql="select * from Hrmresource where departmentid in ("+tmpdepartmentid +") and seclevel >="+seclevel + " and (status = 0 or status = 1 or status = 2 or status = 3) " ;
    ...
    if(sharetype.equals("5")) {
        String tmpsql="select * from hrmresource where seclevel >="+seclevel + " and (status = 0 or status = 1 or status = 2 or status = 3) " ;
        //out.print(tmpsql);
        rs.executeSql(tmpsql);
        while(rs.next()){
            String tmpresourceid = rs.getString("id");
...
/*      int id= 1 ;*/
     ...
     response.sendRedirect("HrmRolesMembers.jsp?operationType=New&id="+roleID);
