...
function initTree(){
CXLoadTreeItem("", "/hrm/tree/DepartmentSingleByRightXML.jsp?deptlevel=<%=deptlevel%>&excludeid=<%=excludeid%>&rightStr=<%=rightStr%>&isedit=<%=isedit%>&subcompanyid=<%=subcompanyid%>&notCompany=<%=notCompany%><%if(nodeid!=null){%>&init=true&nodeid=<%=nodeid%><%}%>");
