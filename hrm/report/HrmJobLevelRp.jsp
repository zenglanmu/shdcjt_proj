...
String titlename =SystemEnv.getHtmlLabelName(179,user.getLanguage())+"："+SystemEnv.getHtmlLabelName(1909,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
<tr>
<td valign="top">
<form name=frmmain method=post action="HrmJobLevelRp.jsp">
...
<tr>
   ...
   <input class=wuiBrowser id=location type=hidden name=location value="<%=location%>"
    ...
    <TD><%=SystemEnv.getHtmlLabelName(15871,user.getLanguage())%>:</TD>
    <TD class=Field><INPUT class=inputStyle maxLength=20 size=6 id="levelarea" name="levelarea"    onKeyPress="ItemCount_KeyPress()" onBlur='checknumber("levelarea")' value="<%=levelarea%>"><%=SystemEnv.getHtmlLabelName(15871,user.getLanguage())%>    
    </TD>
</tr>
...
<%
	...
	er = es.newExcelRow();
	er.addStringValue(SystemEnv.getHtmlLabelName(1909,user.getLanguage()), "Header");
	er.addStringValue(SystemEnv.getHtmlLabelName(1859,user.getLanguage()), "Header");
	er.addStringValue("%", "Header");
	PieTeeChart pie=TeeChartFactory.createPieChart(SystemEnv.getHtmlLabelName(15873,user.getLanguage()),550,400,PieTeeChart.SMS_LabelPercent);
	//pie.isDebug();
   ...
   String labelname = "";
    ...
    /*先算出职级为0的人数*/
    if(sqlwhere.equals("")){
        sqlstr="select count(*) resultcount from HrmResource  where (joblevel = 0 or joblevel is null) and (accounttype is null or accounttype=0)";
    }else{
        sqlstr="select count(*) resultcount from HrmResource where (joblevel = 0 or joblevel is null) and (accounttype is null or accounttype=0) "+sqlwhere;
    }
    ...
    //add dongping for TD1030
    BigDecimal b1 = new BigDecimal(resultpercent);
...
<%if(!(resultpercent==0)){%>
   <tr class=datalight>
        <TD>0<%=SystemEnv.getHtmlLabelName(15872,user.getLanguage())%> </TD>
        <TD height="100%">     
...
<%
	er = es.newExcelRow();
	er.addStringValue("0" + SystemEnv.getHtmlLabelName(15872,user.getLanguage()), "Border");
	er.addStringValue("" + resultcount, "Border");
	er.addStringValue("" + resultpercent + "%", "Border");
    labelname = "0"+SystemEnv.getHtmlLabelName(15872,user.getLanguage()) ;
...
<%
    sqlstr="select max(joblevel) from HrmResource where joblevel>0";
    rs.executeSql(sqlstr);
    rs.next();  
    int levelmax=rs.getInt(1);//职级最大        

    int levelnum = Util.getIntValue(levelarea);
    /*Modified By Charoes Huang On May 20,2004 FOR Bug 460*/
    int cyc = (levelnum==0?0:levelmax/levelnum) +1;//循环次数
    ...
    for(int i=0;i<cyc;i++){//循环列出
        int from_L= i*levelnum+1;        
        int to_L = (i+1)* levelnum;
                
        if(sqlwhere.equals("")){
        sqlstr="select count(*) resultcount from HrmResource where (accounttype is null or accounttype=0) and joblevel >="+from_L+"  and joblevel <="+to_L;
        }else{
        sqlstr="select count(*) resultcount from HrmResource where (accounttype is null or accounttype=0) and joblevel >="+from_L+"  and joblevel <="+to_L+sqlwhere;
        }           
        rs.executeSql(sqlstr);
        rs.next();   
        resultcount = rs.getInt(1);
        resultpercent=(float)resultcount*100/(float)total;       
        //add dongping for TD1030
        b1 = new BigDecimal(resultpercent);        
        resultpercent = b1.divide(b2, 2, BigDecimal.ROUND_HALF_UP).floatValue();

        %>
    <%if(!(resultpercent==0)){%> 
        <tr <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%> >
            <TD><%if(levelnum==1){%><%=from_L%> <%=SystemEnv.getHtmlLabelName(15872,user.getLanguage())%> <% }else{%> <%=from_L%>-<%=to_L%> <%=SystemEnv.getHtmlLabelName(15872,user.getLanguage())%> <%}%></TD>
            <TD height="100%"> 
                <%if(!(resultpercent==0)){
				String className=(resultpercent==100)?"redgraph":"greengraph";
				%>
    ...
    <%
		er = es.newExcelRow();
		if(levelnum==1){
			er.addStringValue(from_L + " " + SystemEnv.getHtmlLabelName(15872,user.getLanguage()), "Border");
		} else {
			er.addStringValue(from_L + "-" + to_L + " " + SystemEnv.getHtmlLabelName(15872,user.getLanguage()), "Border");
		}
		er.addStringValue("" + resultcount, "Border");
		er.addStringValue("" + resultpercent + "%", "Border");
    		labelname = from_L+"-"+to_L+ SystemEnv.getHtmlLabelName(15872,user.getLanguage()) ;
//    		GraphFile.addConditionlable(labelname) ;		
//    		GraphFile.addPiclinevalues ( ""+resultcount , labelname , GraphFile.random , null  );
			pie.addSeries(labelname,resultcount);
    		}%>
   ...
   ExcelFile.setFilename(SystemEnv.getHtmlLabelName(15873,user.getLanguage()));
   ExcelFile.addSheet(SystemEnv.getHtmlLabelName(15873,user.getLanguage()), es);
%>
