...
String sql = "";
if(!olddepartment.equals("")){
  sql = "select count(t1.id) from HrmStatusHistory t1,HrmJobtitles t2 where type_n = 2 "+sqlwhere;
}else{
...
String sqlstr = "";
if(!olddepartment.equals("")){
  sqlstr = "select t1.* from HrmStatusHistory t1,HrmJobtitles t2 where type_n = 2 "+sqlwhere+" order by changedate desc";
}else{
  sqlstr = "select t1.* from HrmStatusHistory t1 where type_n = 2 "+sqlwhere+" order by changedate desc";
}
...
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(1290,user.getLanguage())+",/hrm/report/hire/HrmRpHire.jsp,_self} " ;
RCMenuHeight += RCMenuHeightStep ;
%>	
...
<tr>
<td valign="top">
<form name=frmmain method=post action="HrmRpHireDetail.jsp">
...
<tr>    
    <td ><%=SystemEnv.getHtmlLabelName(15985,user.getLanguage())%></td>
