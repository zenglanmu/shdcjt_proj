...
String sql = "select count(id) from HrmStatusHistory t1 where type_n = 2 "+sqlwhereyear;
rs.executeSql(sql);
rs.next();
total = rs.getInt(1);
%>
...
<tr>
<td valign="top">
<form name=frmmain method=post action="HrmRpHireTime.jsp">
<%
   GraphFile.init ();
   ...
   GraphFile.setPiclable ( SystemEnv.getHtmlLabelName(15984,user.getLanguage())+"("+SystemEnv.getHtmlLabelName(1329,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(6076,user.getLanguage())+")" );
   GraphFile.newLine ();
   ...
   for(int month = 1;month<13;month++){
	   ...
	   sql = "select count(t1.id) resultcount from HrmStatusHistory t1 where type_n = 2 "+sqlwhere;		     
	   rs.executeSql(sql);
	   rs.next();
