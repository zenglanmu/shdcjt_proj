...
//String sql = "select count(id) from HrmStatusHistory t1 where type_n = 5 "+sqlwhere;
String sql = "select count(t1.id) from HrmStatusHistory t1,HrmResource t2 where (t2.accounttype is null or t2.accounttype=0) and type_n = 5 and t1.resourceid = t2.id "+sqlwhere;
rs.executeSql(sql);
rs.next();
total = rs.getInt(1);

//td1487, modified by xiaofeng
//String sqlstr = "select t3.id resultid, count(t1.id) resultcount from HrmStatusHistory t1,HrmJobtitles t2,HrmDepartment t3 where type_n = 5 and t1.oldjobtitleid = t2.id and t2.jobdepartmentid = t3.id "+sqlwhere+" group by t3.id";
String sqlstr = "select  t3.jobdepartmentid resultid, count(t1.id) resultcount from HrmStatusHistory t1,HrmResource t2,HrmJobTitles t3 where (t2.accounttype is null or t2.accounttype=0) and type_n = 5 and t1.resourceid = t2.id and t2.jobtitle = t3.id  "+sqlwhere+" group by t3.jobdepartmentid";
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(15958,user.getLanguage())+",/hrm/report/dismiss/HrmRpDismissTime.jsp,_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(15955,user.getLanguage())+",/hrm/report/dismiss/HrmDismissReport.jsp,_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(15729,user.getLanguage())+",/hrm/report/dismiss/HrmRpDismissDetail.jsp,_self} " ;
RCMenuHeight += RCMenuHeightStep ;
%>	
...
<tr>
<td valign="top">
<form name=frmmain method=post action="HrmRpDismiss.jsp">
...
<tr>    
    <td width=10%><%=SystemEnv.getHtmlLabelName(898,user.getLanguage())%></td>
    <td class=field>
...
<tbody>
  ...
  <%
   rs.first();
   	PieTeeChart pie=TeeChartFactory.createPieChart(SystemEnv.getHtmlLabelName(15959,user.getLanguage()),650,350);
	//pie.isDebug();
...
/*   GraphFile.init ();
   ...
   GraphFile.setPiclable (  SystemEnv.getHtmlLabelName(15959,user.getLanguage()) );
   GraphFile.newLine ();
...
*/   
   ...
   if(total!=0){
	...
	resultpercent=MathUtil.round((double)resultcount*100/total,2);
	//resultpercent=(float)((int)(resultpercent*100))/(float)100;			
	
   %>
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    <td><a href="HrmRpDismissDetail.jsp?olddepartment=<%=resultid%>">>>></a></td>
