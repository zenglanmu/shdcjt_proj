...
String titlename =SystemEnv.getHtmlLabelName(179,user.getLanguage())+"："+SystemEnv.getHtmlLabelName(6086,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
String sqlstr="";
if(sqlwhere.equals("")){
sqlstr = "select t1.jobtitle   resultid,COUNT(t1.id)   resultcount from HrmResource  t1,HrmJobTitles  t2 where (t1.accounttype is null or t1.accounttype=0) and t1.jobtitle=t2.id  group by t1.jobtitle order by resultcount";
}
else 
sqlstr = "select t1.jobtitle   resultid,COUNT(t1.id)   resultcount from HrmResource  t1,HrmJobTitles  t2 where (t1.accounttype is null or t1.accounttype=0) and t1.jobtitle=t2.id "+sqlwhere+" group by t1.jobtitle order by resultcount";
...
<tr>
<td valign="top">
<form name=frmmain method=post action="HrmJobTitleRp.jsp">
...
<%
	...
	BarTeeChart bar=TeeChartFactory.createBarChart(SystemEnv.getHtmlLabelName(15874,user.getLanguage()),700,500);
	//bar.isDebug();
...
/*   GraphFile.init ();
   ...
   GraphFile.setPiclable ( SystemEnv.getHtmlLabelName(15874,user.getLanguage()));
   GraphFile.newLine ();
...
*/   
  ...
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    <TD><a href="/hrm/jobtitles/HrmJobTitlesEdit.jsp?id=<%=resultid%>"> <%=Util.toScreen(JobTitleComInfo.getJobTitlesmark(resultid),user.getLanguage())%>-   
    <%=Util.toScreen(JobTitleComInfo.getJobTitlesname(resultid),user.getLanguage())%></TD>
    <TD height="100%">
    ...
    <%
		er = es.newExcelRow();
		er.addStringValue(Util.toScreen(JobTitleComInfo.getJobTitlesmark(resultid),user.getLanguage()) + "- " + Util.toScreen(JobTitleComInfo.getJobTitlesname(resultid),user.getLanguage()), "Border");
		er.addStringValue("" + resultcount, "Border");
		er.addStringValue("" + resultpercent + "%", "Border");
    		if(linecolor==0) linecolor=1;
    		else	linecolor=0;
    		String labelname =Util.toScreen(JobTitleComInfo.getJobTitlesmark(resultid),user.getLanguage()); 	
...
//      		GraphFile.addPiclinevalues ( ""+resultcount , labelname , GraphFile.random , null  );
		...
		bar.addSeries("GWBB",list1,"");

	} 
   ExcelFile.setFilename(SystemEnv.getHtmlLabelName(15874,user.getLanguage()));
   ExcelFile.addSheet(SystemEnv.getHtmlLabelName(15874,user.getLanguage()), es);
	%>
  </TBODY></TABLE>
  <br>
<!--<TABLE class=Viewform>
