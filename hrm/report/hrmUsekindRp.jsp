...
String titlename =  SystemEnv.getHtmlLabelName(179,user.getLanguage())+"："+SystemEnv.getHtmlLabelName(804,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
if(!department.equals("")){
	sqlwhere+=" and t1.departmentid ="+department;
    if(sqlwhere_0.equals("")){
...
<tr>
<td valign="top">
<form name=frmmain id="frmmain" method=post action="hrmUsekindRp.jsp">
...
<TABLE class=ListStyle cellspacing=1  width="100%">
  ...
  <TBODY>
  <TR class=Header>
    <TH><%=SystemEnv.getHtmlLabelName(804,user.getLanguage())%></TH>
...
<%
	...
	er = es.newExcelRow();
	er.addStringValue(SystemEnv.getHtmlLabelName(804,user.getLanguage()), "Header");
	er.addStringValue(SystemEnv.getHtmlLabelName(1859,user.getLanguage()), "Header");
	er.addStringValue("%", "Header");
	PieTeeChart pie=TeeChartFactory.createPieChart(SystemEnv.getHtmlLabelName(15926,user.getLanguage()),650,500,PieTeeChart.SMS_LabelPercent);
	//pie.isDebug();
...
/*   GraphFile.init ();
   ...
   GraphFile.setPiclable ( SystemEnv.getHtmlLabelName(15926,user.getLanguage()));
   GraphFile.newLine ();
...
*/   String labelname = "";
    /*先算出记录为空的人数*/
    if(sqlwhere.equals("")){
        sqlstr="select count(*) resultcount from HrmResource t1 where (t1.accounttype is null or t1.accounttype=0) and t1.usekind is null or t1.usekind ='' or t1.usekind='0'";
    }else{
        sqlstr="select count(*) resultcount from HrmResource t1 where (t1.accounttype is null or t1.accounttype=0) and (t1.usekind is null or t1.usekind ='' or t1.usekind='0')"+sqlwhere;
    }
...
if(total!=0){
        ...
        if(sqlwhere.equals("")){            
            sqlstr = "select  COUNT(*)   resultcount,t2.id usekindid from HrmResource t1 ,HrmUseKind t2  where (t1.accounttype is null or t1.accounttype=0) and t1.usekind = t2.id group by t2.id order by t2.id " ;           
        }else{            
            sqlstr = "select  COUNT(*)   resultcount,t2.id usekindid from HrmResource t1 ,HrmUseKind t2  where (t1.accounttype is null or t1.accounttype=0) and t1.usekind = t2.id "+sqlwhere+" group by t2.id order by t2.id "  ;   
        }
...
<%if(resultpercent!=0){%>
       <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
        <TD>        
        <%=Util.toScreen(UseKindComInfo.getUseKindname(resultid),user.getLanguage())%>
        </TD>
        <TD height="100%">      
			<%String className=(resultpercent==100)?"redgraph":"greengraph";%>
  ...
  <%
	er = es.newExcelRow();
	er.addStringValue(Util.toScreen(UseKindComInfo.getUseKindname(resultid),user.getLanguage()), "Border");
	er.addStringValue("" + resultcount, "Border");
	er.addStringValue("" + resultpercent + "%", "Border");
 	   	labelname =Util.toScreen(UseKindComInfo.getUseKindname(resultid),user.getLanguage()); 	
//      		GraphFile.addConditionlable(labelname) ;		
//      		GraphFile.addPiclinevalues ( ""+resultcount , labelname , GraphFile.random , null  );
  ...
  <%
   ...
   ExcelFile.setFilename(SystemEnv.getHtmlLabelName(15926,user.getLanguage()));
   ExcelFile.addSheet(SystemEnv.getHtmlLabelName(15926,user.getLanguage()), es);
  %>
