...
String titlename =SystemEnv.getHtmlLabelName(179,user.getLanguage())+"："+SystemEnv.getHtmlLabelName(806,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
<tr>
<td valign="top">
<form name=frmmain id="frmmain"  method=post action="HrmJobCallRp.jsp">
...
<%
	...
	PieTeeChart pie=TeeChartFactory.createPieChart(SystemEnv.getHtmlLabelName(15869,user.getLanguage()),600,400,PieTeeChart.SMS_LabelPercent);
	//pie.isDebug();
...
/*   GraphFile.init ();
   ...
   GraphFile.setPiclable (Util.toScreen(SystemEnv.getHtmlLabelName(15869,user.getLanguage()),user.getLanguage()));
   GraphFile.newLine ();
...
*/	RecordSet.first();
  ...
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
	  ...
	  <%
  	  ...
  	  {
		er = es.newExcelRow();
		er.addStringValue(Util.toScreen(JobCallComInfo.getJobCallname(resultid),user.getLanguage()), "Border");
  	  %>
    <TD><a href="/hrm/jobcall/HrmJobCallEdit.jsp?id=<%=resultid%>"> <%=Util.toScreen(JobCallComInfo.getJobCallname(resultid),user.getLanguage())%></TD>
    <%} %>
    <TD height="100%"> 
    ...
    <%
		...
		er.addStringValue("" + resultpercent + "%", "Border");
    		...
    		String labelname =Util.toScreen(JobCallComInfo.getJobCallname(resultid),user.getLanguage()); 	
    		if("0".equals(resultid))
    		{
    			labelname = "空";
    		}
...
//      		GraphFile.addPiclinevalues ( ""+resultcount , labelname , GraphFile.random , null  );    		
   ...
   ExcelFile.setFilename(SystemEnv.getHtmlLabelName(15869,user.getLanguage()));
   ExcelFile.addSheet(SystemEnv.getHtmlLabelName(15869,user.getLanguage()), es);
	%>   
  </TBODY></TABLE>
   <br>
<!--<TABLE class=ViewForm>
