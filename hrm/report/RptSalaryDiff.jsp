...
<DIV class=BtnBar>
    <BUTTON language=VBS class=BtnReset id=button1 accessKey=R name=button1 onclick="javascript:location.href='/hrm/report/HrmRp.jsp'"><U>R</U>-返回</BUTTON>
</DIV>
<table class=form>
  <colgroup>
  <col width="10%">
...
<TABLE class=listshort>
  ...
  <TBODY> 
  <TR class=section> 
    <TH>工资单 <%if( payid == 0 ){%><font color=red>未生成当前月工资单</font><%} else if( payido == 0 ){%><font color=red>未生成上月工资单</font><% }else{%></TH>
  </TR>
  <TR class=separator> 
    <TD class=sep1></TD>
  </TR>
...
<TABLE class=ListShort>
  <TBODY>
  <tr class=Header>
  <TH width="3%" rowspan=2 style="TEXT-ALIGN:center;TEXT-VALIGN:middle">姓名</TH>
  <TH width="7%" rowspan=2 style="TEXT-ALIGN:center;TEXT-VALIGN:middle">岗位</TH>
   <% 
  ...
  <tr class=Header>
   ...
   %>
   <TH width="5%" style="TEXT-ALIGN:center">个人</TH> 
   <TH width="5%" style="TEXT-ALIGN:center">公司</TH> 
   <%
    }
   %>
   </tr>
  ...
  <tr class='datadark'>
  <td colspan=2 align=right>上月</td>
  <%
            SalaryComInfo.setTofirstRow() ;
            itemsp.clear();
            while(SalaryComInfo.next()) {
  ...
  <tr class='datalight'>
  <td colspan=2 align=right>浮动比例</td>
