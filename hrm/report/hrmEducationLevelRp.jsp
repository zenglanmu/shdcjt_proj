...
String titlename =SystemEnv.getHtmlLabelName(179,user.getLanguage())+"："+SystemEnv.getHtmlLabelName(818,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
String sqlstr = "";
if(sqlwhere.equals("")){
    sqlstr = " select 0  resultid ,count(*) resultcount from HrmResource t1 where (educationlevel <2 or educationlevel is null) union	select educationlevel resultid  ,count(*) resultcount from HrmResource t1 where educationlevel > 1 group by  educationlevel ";
}else{
    sqlstr = " select 0  resultid ,count(*) resultcount from HrmResource t1 where (educationlevel <2 or educationlevel is null) "+sqlwhere+" union	select educationlevel resultid  ,count(*) resultcount from HrmResource t1 where educationlevel > 1 "+sqlwhere+" group by  educationlevel ";
}
...
<tr>
<td valign="top">
<form name=frmmain method=post action="hrmEducationLevelRp.jsp">
...
<%
	...
	PieTeeChart pie=TeeChartFactory.createPieChart(SystemEnv.getHtmlLabelName(15867,user.getLanguage()),550,400,PieTeeChart.SMS_LabelPercent);
	//pie.isDebug();
...
/*   GraphFile.init ();
   ...
   GraphFile.setPiclable ( SystemEnv.getHtmlLabelName(15867,user.getLanguage()));
   GraphFile.newLine ();
...
*/	RecordSet.first();
	if(total!=0){
		...
		if(resultid.equals("0")) resultid = "1";
		int resultcount = RecordSet.getInt(2);
  ...
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    <TD>  
    <%=Util.toScreen(EducationLevelComInfo.getEducationLevelname(resultid),user.getLanguage())%></TD>
    <TD height="100%">        
    ...
    <%
	er = es.newExcelRow();
	er.addStringValue(Util.toScreen(EducationLevelComInfo.getEducationLevelname(resultid),user.getLanguage()), "Border");
	er.addStringValue("" + resultcount, "Border");
	er.addStringValue("" + resultpercent + "%", "Border");
      String labelname =Util.toScreen(EducationLevelComInfo.getEducationLevelname(resultid),user.getLanguage()); 	 
...
//      GraphFile.addPiclinevalues ( ""+resultcount , labelname , GraphFile.random , null  );
   ...
   ExcelFile.setFilename(SystemEnv.getHtmlLabelName(15867,user.getLanguage()));
   ExcelFile.addSheet(SystemEnv.getHtmlLabelName(15867,user.getLanguage()), es);
	%>
  </TBODY></TABLE>
  <br>
<!--<TABLE class=ViewForm>
