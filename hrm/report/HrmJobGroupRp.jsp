...
String titlename =SystemEnv.getHtmlLabelName(179,user.getLanguage())+"："+SystemEnv.getHtmlLabelName(805,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
String sqlstr="";
if(sqlwhere.equals("")){
sqlstr = "select t4.id   resultid,COUNT(t1.id)   resultcount from HrmResource  t1,HrmJobTitles  t2,HrmJobActivities t3,HrmJobGroups t4 where (t1.accounttype is null or t1.accounttype=0) and t1.jobtitle=t2.id and t2.jobactivityid = t3.id and t3.jobgroupid = t4.id group by t4.id order by resultcount";
}
else 
sqlstr = "select t4.id   resultid,COUNT(t1.id)   resultcount from HrmResource  t1,HrmJobTitles  t2, HrmJobActivities t3,HrmJobGroups t4 where (t1.accounttype is null or t1.accounttype=0) and t1.jobtitle=t2.id and t2.jobactivityid = t3.id and t3.jobgroupid = t4.id "+sqlwhere+" group by t4.id order by resultcount";
...
<form name=frmmain id="frmmain" method=post action="HrmJobGroupRp.jsp">
...
<TABLE class=ListStyle cellspacing=1  width="100%">
  ...
  <TBODY>
  <TR class=Header>
    <TH><%=SystemEnv.getHtmlLabelName(806,user.getLanguage())%></TH>
...
<%
	...
	er = es.newExcelRow();
	er.addStringValue(SystemEnv.getHtmlLabelName(806,user.getLanguage()), "Header");
	er.addStringValue(SystemEnv.getHtmlLabelName(1859,user.getLanguage()), "Header");
	er.addStringValue("%", "Header");
	PieTeeChart pie=TeeChartFactory.createPieChart(SystemEnv.getHtmlLabelName(15870,user.getLanguage()),700,500,PieTeeChart.SMS_LabelPercent);
	//pie.isDebug();
...
/*   GraphFile.init ();
   ...
   GraphFile.setPiclable ( SystemEnv.getHtmlLabelName(15870,user.getLanguage()));
   GraphFile.newLine ();
...
*/	RecordSet.first();
  ...
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    <TD><a href="/hrm/jobactivities/HrmJobActivitiesEdit.jsp?id=<%=resultid%>"> <%=Util.toScreen(JobGroupsComInfo.getJobGroupsremarks(resultid),user.getLanguage())%>-   
    <%=Util.toScreen(JobGroupsComInfo.getJobGroupsname(resultid),user.getLanguage())%></TD>
    <TD height="100%">
    ...
    <%
		er = es.newExcelRow();
		er.addStringValue(Util.toScreen(JobGroupsComInfo.getJobGroupsremarks(resultid),user.getLanguage()) + "- " + Util.toScreen(JobGroupsComInfo.getJobGroupsname(resultid),user.getLanguage()), "Border");
		er.addStringValue("" + resultcount, "Border");
		er.addStringValue("" + resultpercent + "%", "Border");
    		if(linecolor==0) linecolor=1;
    		else	linecolor=0;
    		String labelname =Util.toScreen(JobGroupsComInfo.getJobGroupsname(resultid),user.getLanguage()); 	
...
//      		GraphFile.addPiclinevalues ( ""+resultcount , labelname , GraphFile.random , null  );
   ...
   ExcelFile.setFilename(SystemEnv.getHtmlLabelName(15870,user.getLanguage()));
   ExcelFile.addSheet(SystemEnv.getHtmlLabelName(15870,user.getLanguage()), es);
	%>
  </TBODY></TABLE>
<!--   <br>
<TABLE class=ViewForm>
