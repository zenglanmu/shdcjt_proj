...
String sql = "select count(id) from HrmStatusHistory t1 where type_n = 6 "+sqlwhere;
rs.executeSql(sql);
rs.next();
total = rs.getInt(1);

String sqlstr = "select t3.id resultid, count(t1.id) resultcount from HrmStatusHistory t1,HrmJobtitles t2,HrmDepartment t3 where type_n = 6 and t1.oldjobtitleid = t2.id and t2.jobdepartmentid = t3.id "+sqlwhere+" group by t3.id";
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(16028,user.getLanguage())+",/hrm/report/retire/HrmRpRetireTime.jsp,_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(16025,user.getLanguage())+",/hrm/report/retire/HrmRetireReport.jsp,_self} " ;
RCMenuHeight += RCMenuHeightStep ;
RCMenu += "{"+SystemEnv.getHtmlLabelName(15729,user.getLanguage())+",/hrm/report/retire/HrmRpRetireDetail.jsp,_self} " ;
RCMenuHeight += RCMenuHeightStep ;
%>	
...
<tr>
<td valign="top">
<form name=frmmain method=post action="HrmRpRetire.jsp">
...
<tr>    
    <td width=10%><%=SystemEnv.getHtmlLabelName(16029,user.getLanguage())%></td>
    <td class=field>
...
<tbody>
  ...
  <%
   ...
   GraphFile.setPiclable (  SystemEnv.getHtmlLabelName(16030,user.getLanguage()) );
   GraphFile.newLine ();
  ...
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    <td><a href="HrmRpRetireDetail.jsp?olddepartment=<%=resultid%>">>>></a></td>
