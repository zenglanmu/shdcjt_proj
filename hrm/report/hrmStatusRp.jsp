...
String titlename =SystemEnv.getHtmlLabelName(179,user.getLanguage())+":"+SystemEnv.getHtmlLabelName(602,user.getLanguage());
String needfav ="1";
String needhelp ="";
...
if(sqlwhere.equals("")){
    ...
    sql = "select count(*)  from HrmResource where (accounttype is null or accounttype=0) "+sqlwhere;
}    
rs.executeSql(sql);
rs.next();
total = rs.getInt(1);
...
<tr>
<td valign="top">
<form name=frmmain id="frmmain" method=post action="hrmStatusRp.jsp">
...
<TABLE class=ListStyle cellspacing=1  width="100%">
  ...
  <TBODY>
  <TR class=Header>
    <TH><%=SystemEnv.getHtmlLabelName(15890,user.getLanguage())%></TH>
...
<%
	...
	er = es.newExcelRow();
	er.addStringValue(SystemEnv.getHtmlLabelName(15890,user.getLanguage()), "Header");
	er.addStringValue(SystemEnv.getHtmlLabelName(1859,user.getLanguage()), "Header");
	er.addStringValue("%", "Header");
	PieTeeChart pie=TeeChartFactory.createPieChart(SystemEnv.getHtmlLabelName(15891,user.getLanguage()),650,500,PieTeeChart.SMS_LabelPercent);
	//pie.isDebug();	
...
/*   GraphFile.init ();
   ...
   GraphFile.setPiclable ( SystemEnv.getHtmlLabelName(15891,user.getLanguage()));
   GraphFile.newLine ();
   GraphFile.addPiclinecolor("#660033") ;
   GraphFile.addPiclinelable("Line") ;
*/   String labelname ="";
    if(sqlwhere.equals("")){
        sql = "select distinct status   from HrmResource where accounttype is null or accounttype=0";
    }else{
        sql = "select distinct status  from HrmResource where (accounttype is null or accounttype=0) "+sqlwhere;
    }
    rs.executeSql(sql);
    while(rs.next()){
    ...
    if(sqlwhere.equals("")){
	sqlstr = "select  COUNT(*)   resultcount from HrmResource   where (accounttype is null or accounttype=0) and status=" +tempStatus;
    }
    else 
    sqlstr = "select  COUNT(*)   resultcount from HrmResource   where (accounttype is null or accounttype=0) and status=" +tempStatus+sqlwhere ;
  ...
  <TR <%if(linecolor==0){%>class=datalight <%} else {%> class=datadark <%}%>>
    <TD>
        <% if(tempStatus.equals("9")) { labelname = SystemEnv.getHtmlLabelName(332,user.getLanguage()) ;
        }
        if(tempStatus.equals("0")) { labelname = SystemEnv.getHtmlLabelName(15710,user.getLanguage()); }
        if(tempStatus.equals("1")) { labelname = SystemEnv.getHtmlLabelName(15711,user.getLanguage()) ; }
        if(tempStatus.equals("2")) { labelname = SystemEnv.getHtmlLabelName(480,user.getLanguage()) ;
        }
        if(tempStatus.equals("3")) { labelname = SystemEnv.getHtmlLabelName(15844,user.getLanguage()) ; }
        if(tempStatus.equals("4")) { labelname = SystemEnv.getHtmlLabelName(6094,user.getLanguage()) ; }
        if(tempStatus.equals("5")) { labelname = SystemEnv.getHtmlLabelName(6091,user.getLanguage()) ; }
        if(tempStatus.equals("6")) { labelname = SystemEnv.getHtmlLabelName(6092,user.getLanguage()) ; }
        if(tempStatus.equals("7")) { labelname = SystemEnv.getHtmlLabelName(2245,user.getLanguage()) ; }
        if(tempStatus.equals("8")) { labelname = SystemEnv.getHtmlLabelName(1831,user.getLanguage()) ; }     
        %>
		<%=labelname%>
    </TD>
    <TD height="100%"> 
...
//      		GraphFile.addPiclinevalues ( ""+resultcount , labelname , GraphFile.random , null  );
   ...
   ExcelFile.setFilename(SystemEnv.getHtmlLabelName(15891,user.getLanguage()));
   ExcelFile.addSheet(SystemEnv.getHtmlLabelName(15891,user.getLanguage()), es);
    %>
  </TBODY></TABLE>
  <br>
<!--<TABLE class=ViewForm>
