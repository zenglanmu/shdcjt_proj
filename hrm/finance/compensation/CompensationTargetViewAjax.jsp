<%@ page language="java" contentType="text/html; charset=GBK" %><%@ page import="weaver.general.Util" %><%@ page import="java.util.ArrayList" %><jsp:useBean id="SubCompanyComInfo" class="weaver.hrm.company.SubCompanyComInfo" scope="page" /><jsp:useBean id="DepartmentComInfo" class="weaver.hrm.company.DepartmentComInfo" scope="page"/><jsp:useBean id="CompensationTargetMaint" class="weaver.hrm.finance.compensation.CompensationTargetMaint" scope="page"/><jsp:useBean id="RecordSet" class="weaver.conn.RecordSet" scope="page" />
...
<table  style="text-align:center;"  width="100%">
  ...
  <%}%>
      <TD width="50"><%=(i+1)%></TD>
      <%if(showdept){%>
      <TD width="200"><%=subcompanyname%></TD>
      <TD width="200" ><%=departmentname%></TD>
      <%}
      //System.out.println("i:"+i+"|"+cuserid);
      %>
      <TD width="100"><%if(isedit==1){%><input type='hidden'  id='Userid_<%=i%>' name='Userid_<%=i%>' value="<%=cuserid%>"><%}%><%=cusername%></TD>
      <TD width="100"><%=workcode%></TD>
      <%for(int j=0;j<targetlist.size();j++){
        String temp=(String)templist.get(j);
        if(isedit==1){
        %>
      <TD width="100"><input type='text' style="BORDER-LEFT-STYLE: none;BORDER-RIGHT-STYLE: none;BORDER-TOP-STYLE: none ;BORDER-BOTTOM: #000000 1px solid;BACKGROUND-COLOR: #FFFFFF;width:90%" name='target<%=j%>_<%=i%>' <%if(Util.getDoubleValue(temp)!=0){%>value="<%=temp%>"<%}%> onKeyPress="ItemNum_KeyPress()" onBlur="checknumber1(this)" maxlength="13"></TD>
      <%
        }else{
      %>
      <TD width="100"><%if(Util.getDoubleValue(temp)!=0){%><%=temp%><%}%></TD>
      <%}
      }
      if(isedit==1){
      %>
      <TD width="200"><input type='text' style="BORDER-LEFT-STYLE: none;BORDER-RIGHT-STYLE: none;BORDER-TOP-STYLE: none ;BORDER-BOTTOM: #000000 1px solid;BACKGROUND-COLOR: #FFFFFF;width:90%" name='memo_<%=i%>' value="<%=memo%>" maxlength=500></TD>
      <%
      }else{
      %>
