...
<table class=viewform>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class=field>
         <BUTTON class=Calendar id=selectstartdate onclick="getDate(plandatespan,plandate)"></BUTTON> 
...
<%
  ...
  %>
     <tr class='<%=( isLight ? "datalight" : "datadark" )%>'>
		...
		<TD><%=plandaters%></TD>
		<TD>
			<a href="HrmSalaryOperation.jsp?method=deleteplan&planid=<%=idrs%>&currentdate=<%=currentdate%>&salarybegindate=<%=salarybegindate%>&salaryenddate=<%=salaryenddate%>&payid=<%=payid%>&fromdepartment=<%=fromdepartment%>" ><img border=0 src="/images/icon_delete.gif"></a>
        </TD>
	</TR>
<%} %>
</TABLE>
...
<script language=javascript>
 function doCreate(obj){
     if(document.frmmain.createtype[1].checked && document.frmmain.plandate.value == ''){
     	 alert("<%=SystemEnv.getHtmlLabelName(23262,user.getLanguage())%>！") ;
         return ;
     }
     else if(document.frmmain.createtype[1].checked && document.frmmain.plandate.value <= '<%=theday%>'){
     	 alert("<%=SystemEnv.getHtmlLabelName(23263,user.getLanguage())%>！") ;
         return ;
     }
     else {
         obj.disabled = true ;
