<%@ page language="java" contentType="text/html; charset=GBK" %><%@ page import="weaver.general.Util" %><%@ page import="java.util.ArrayList" %><%@ page import="weaver.systeminfo.SystemEnv" %><jsp:useBean id="RecordSet" class="weaver.conn.RecordSet" scope="page" /><jsp:useBean id="ResourceComInfo" class="weaver.hrm.resource.ResourceComInfo" scope="page" /><jsp:useBean id="SalaryComInfo" class="weaver.hrm.finance.SalaryComInfo" scope="page" /><jsp:useBean id="DepartmentComInfo" class="weaver.hrm.company.DepartmentComInfo" scope="page"/><jsp:useBean id="JobTitlesComInfo" class="weaver.hrm.job.JobTitlesComInfo" scope="page"/><jsp:useBean id="JobActivitiesComInfo" class="weaver.hrm.job.JobActivitiesComInfo" scope="page" /><%@ page import="weaver.hrm.*" %><%
...
<%if(isLight){
  ...
  <%}%>
          <td width="200" rowspan="<%=resourcenum%>"><%if(showbox){%><input type="checkbox" name="chkdeptid" value="<%=departmentid%>" onClick="CheckAll(this.checked,<%=departmentid%>)"><%}%><%=Util.toScreen(DepartmentComInfo.getDepartmentname(""+departmentid),Language)%></td>
          <td width="100" <%if(tolrows%2==0){%>bgcolor="#e7e7e7"<%}else{%>bgcolor="#f5f5f5"<%}%>><%if(showbox){%><input type="checkbox" name="chkresourceid_<%=departmentid%>" value="<%=viewresourceid%>" onclick="CheckChanage(this,<%=departmentid%>)"><%}%><%=Util.toScreen(ResourceComInfo.getResourcename(viewresourceid),Language)%></td>
   ...
   %>
          <td width="100" <%if(tolrows%2==0){%>bgcolor="#e7e7e7"<%}else{%>bgcolor="#f5f5f5"<%}%>><%if(((String)statuslist.get(nowrows)).equals("1")){ %><%=SystemEnv.getHtmlLabelName(19558,Language)%><%}else{ %><font color="red"><%=SystemEnv.getHtmlLabelName(19557,Language)%></font><%}%></td>
      </tr>
   <%
  ...
  <TR style="BACKGROUND-COLOR: #f5f5f5;word-wrap:break-word; word-break:break-all;">
  <%}%>
          <td width="100" <%if(tolrows%2==0){%>bgcolor="#e7e7e7"<%}else{%>bgcolor="#f5f5f5"<%}%>><input type="checkbox"  name="chkresourceid_<%=departmentid%>" value="<%=viewresourceid%>" onclick="CheckChanage(this,<%=departmentid%>)"><%=Util.toScreen(ResourceComInfo.getResourcename(viewresourceid),Language)%></td>
          <td width="150" <%if(tolrows%2==0){%>bgcolor="#e7e7e7"<%}else{%>bgcolor="#f5f5f5"<%}%>><%=Util.toScreen(JobTitlesComInfo.getJobTitlesname(viewjobtitle),Language)%></td>
   <%
      itemeid="";
   ...
   %>
          <td width="100" <%if(tolrows%2==0){%>bgcolor="#e7e7e7"<%}else{%>bgcolor="#f5f5f5"<%}%>><%if(((String)statuslist.get(j)).equals("1")){%><%=SystemEnv.getHtmlLabelName(19558,Language)%><%}else{%><font color="red"><%=SystemEnv.getHtmlLabelName(19557,Language)%></font><%}%></td>
      </tr>
   <%
      }
      %>
