...
// 在修改工资单后做跳转
    ...
    if (method.equals("changesalary")) {
        ...
        response.sendRedirect("HrmSalaryChange.jsp");
        return;
                ...
                if (itemshow.equals("1")) {
                    ...
                    if (!itemtype.equals("2")) {
                        ...
                        if (!salary.equals(oldsalary)) {
                            ProcPara = payid + flag + itemid + flag + resourceidrs + flag + salary;
                    ...
                    if (itemtype.equals("2")) {
                        ...
                        if (!salary.equals(oldsalary)) {
                            ProcPara = payid + flag + itemid + "_1" + flag + resourceidrs + flag + salary;
                            RecordSet2.executeProc("HrmSalaryPaydetail_Update", ProcPara);
                            haschanged = true;
                        }
                        ...
                        if (!salary.equals(oldsalary)) {
                            ProcPara = payid + flag + itemid + "_2" + flag + resourceidrs + flag + salary;
                            RecordSet2.executeProc("HrmSalaryPaydetail_Update", ProcPara);
                            haschanged = true;
                        }
        ...
        try {
            ...
            if (departmentid > 0) {
                ...
                wherestr = " and departmentid in(" + departmentstr + ")";
            ...
            if (!payid.equals("")) {//计算工资单
                ...
                //删除该分部或部门下原有的工资单
                if (decdeptid.length() > 0) wherestr += " and departmentid not in(" + decdeptid + ")";
                sql = "delete from HrmSalarypaydetail where (status is null or status=0) and payid=" + payid + wherestr;
                //System.out.println(sql);
                RecordSet.executeSql(sql);
                //生成新的工资单
                //获得该分部及下级分部或部门下的所有人
                sql = "select distinct departmentid,subcompanyid1 from HrmResource where status in(0,1,2,3) " + wherestr + " order by subcompanyid1";
                ...
                while (RecordSet.next()) {
                    ...
                    String tempwhere=" where status in(0,1,2,3) and departmentid="+deptid+" " + wherestr+" order by departmentid,subcompanyid1";
