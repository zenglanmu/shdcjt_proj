...
RCMenu += "{"+SystemEnv.getHtmlLabelName(365 , user.getLanguage())+",/hrm/schedule/PSLPeriodAdd.jsp?subcompanyid="+subcompanyid+",_self} " ; 
...
<br>	
<TABLE class=ListStyle style="margin-bottom: 0px;">
 <TBODY>
  <TR class=Header>
...
<%//subcompanyid=0为总部
	RecordSet.executeSql("select * from HrmPSLPeriod where subcompanyid = " + subcompanyid + " order by PSLyear desc");
	int i = 0 ;
	while(RecordSet.next()){
		String id =Util.null2String(RecordSet.getString("id"));
		String annualyear=Util.null2String(RecordSet.getString("PSLyear"));
...
%>
    ...
    <TD><A href="PSLPeriodEdit.jsp?id=<%=id%>&subcompanyid=<%=subcompanyid%>"><%=annualyear%></A></TD>
    <TD><%=startdate%></TD>
    <TD><%=enddate%></TD>
    </TR>
<%}%>
...
function onSyn(){
   ...
   if(ids==""){
     ...
     if(confirm("<%=SystemEnv.getHtmlLabelName(21669,user.getLanguage())%>")){
       location.href="/hrm/schedule/PSLPeriodOperation.jsp?operation=syn&subcompanyid=<%=subcompanyid%>&ids="+ids;       
     }   
  }
}
   ...
   if(ids==""){
    ...
    if(confirm("<%=SystemEnv.getHtmlLabelName(15459,user.getLanguage())%>")){
       if(confirm("<%=SystemEnv.getHtmlLabelName(18260,user.getLanguage())%>")){       
          location.href="/hrm/schedule/PSLPeriodOperation.jsp?operation=syndelete&subcompanyid=<%=subcompanyid%>&ids="+ids;       
       }else{
          location.href="/hrm/schedule/PSLPeriodOperation.jsp?operation=delete&subcompanyid=<%=subcompanyid%>&ids="+ids;       
       }
    }
   }  
}
