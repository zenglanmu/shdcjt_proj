...
<span><font color="red"><%
 ...
 <br>
  <table class=Viewform>
    ...
    <tr class=Title> 
      <td><nobr><b><%=SystemEnv.getHtmlLabelName(21602,user.getLanguage())+SystemEnv.getHtmlLabelName(563,user.getLanguage())%></b></td>
      <td align=right></td>
    </tr>
    <tr class=spacing style="height:2px"> 
  ...
  <table class=viewform>
   ...
   <tr> 
    ...
    <tr> 
      <td>注意事项</td>
      <td>
       1、人员的年假数据必须填入导入文件Excel模板的第一个sheet中。<br><br>
       2、模板中的第一行为标题行，不能占用。数据必须从第二行开始，中间不能有空行<br><br>
       3、模板中的数据除了年假天数以外，其它数据不允许修改。<br><br>
       4、年假天数只能为数字。
      </td>
    </tr> 
    <TR style="height:2px"><TD class=Line1 colSpan=2></TD></TR> 
...
function goBack(){
    frmmain.action="AnnualManagementEdit.jsp?annualyear=<%=annualyear%>&subCompanyId=<%=subcompanyid%>&departmentid=<%=departmentid%>";
    frmmain.submit();
}
</script>
</BODY>
