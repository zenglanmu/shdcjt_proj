...
String titlename = SystemEnv.getHtmlLabelName(24038,user.getLanguage()); 
String needfav = "1" ; 
String needhelp = "" ; 
...
RCMenu += "{"+SystemEnv.getHtmlLabelName(365 , user.getLanguage())+",/hrm/schedule/PSLBatchAdd.jsp?subcompanyid="+subcompanyid+",_self} " ; 
...
<TABLE class=ListStyle cellspacing=1>
  ...
  <TR class=Header align=left>
    ...
    <TH><%=SystemEnv.getHtmlLabelName(24043,user.getLanguage())%></TH>
  </TR>

<%
	RecordSet.executeSql("select * from HrmPSLBatchProcess where subcompanyid = " + subcompanyid + " order by workingage asc");
	int i = 0 ;
	while(RecordSet.next()){
		String id =RecordSet.getString("id");
		String workingage=""+(int)RecordSet.getFloat("workingage");
		String PSLdays=Util.null2String(RecordSet.getString("PSLdays"));
...
%>
    ...
    <TD><A href="PSLBatchEdit.jsp?id=<%=id%>&subcompanyid=<%=subcompanyid%>"><%=workingage%></A></TD>
    <TD><%=PSLdays%></TD>
...
function onSyn(){
   ...
   if(ids==""){
     ...
     if(confirm("<%=SystemEnv.getHtmlLabelName(21669,user.getLanguage())%>")){
        location.href="/hrm/schedule/PSLBatchOperation.jsp?operation=syn&subcompanyid=<%=subcompanyid%>&ids="+ids;
     }   
  }
}
