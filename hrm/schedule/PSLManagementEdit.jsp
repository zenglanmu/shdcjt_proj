...
RCMenuHeight += RCMenuHeightStep;
RCMenu += "{"+SystemEnv.getHtmlLabelName(24047,user.getLanguage())+",javascript:onImport(),_self} " ;
RCMenuHeight += RCMenuHeightStep;
...
<TABLE class=ListStyle cellspacing=1 >
  ...
  %>
     ...
     <td><input type=text size=4 class=inputstyle name=PSLdays value="<%=Util.getFloatValue(tempPSLdays,0)%>"></td>
...
function onImport(){
    location="PaidSickLeaveImport.jsp?PSLyear=<%=PSLyear%>&subCompanyId=<%=subcompanyid%>&departmentid=<%=departmentid%>";
}
</script>
</BODY>
