...
String holidaydate=Util.fromScreen(request.getParameter("holidaydate"),user.getLanguage());
...
if(opera.equals("selectdate")){
    ...
    if(weekrestdays.indexOf(""+tempday.getTime().getDay()) != -1 ) {   // 休息日
        response.sendRedirect("HrmPubHolidayAdd.jsp?selectdate=1&countryid="+countryid+"&holidaydate="+holidaydate+"&selectdatetype=2"+"&showtype="+showtype+"&year="+year);
        return ;
    }
    else {
        response.sendRedirect("HrmPubHolidayAdd.jsp?selectdate=1&countryid="+countryid+"&holidaydate="+holidaydate+"&selectdatetype=1"+"&showtype="+showtype+"&year="+year);
        return ;
    } 
