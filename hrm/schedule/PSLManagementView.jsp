...
if(departmentid>0){
   ...
   sql = "select * from (select workcode,id,lastname,subcompanyid1,departmentid,jobtitle,dsporder from hrmresource where departmentid = " + departmentid + "  and (status = 0 or status = 1 or status = 2 or status = 3) and status != 10) a left join hrmPSLmanagement b on a.id = b.resourceid and PSLyear = '" + tempyear + "' order by dsporder,lastname,a.id";
}else{
   ...
   sql = "select * from (select workcode,id,lastname,subcompanyid1,departmentid,jobtitle,dsporder from hrmresource where subcompanyid1 = " + subcompanyid + " and (status = 0 or status = 1 or status = 2 or status = 3) and status != 10) a left join hrmPSLmanagement b on a.id = b.resourceid and PSLyear = '" + tempyear + "' order by dsporder,lastname,a.id";
}

String PSLperiod = "";
try{PSLperiod = HrmPaidSickManagement.getPaidSickPeriod(subcompanyid+"",tempyear);}catch(Exception e){}
...
<table class=Viewform>
  <COLGROUP><COL width="18%"><COL width="82%">
      ...
      <TR>
        ...
        <TD class=field>
          <select id="PSLyear" name="PSLyear" onchange="changeyear(this)">
...
<TABLE class=ListStyle cellspacing=1 >
  ...
  <TBODY>
  <TR class=Header>
    <TH><%=SystemEnv.getHtmlLabelName(24032,user.getLanguage())+SystemEnv.getHtmlLabelName(87,user.getLanguage())%></TH>
  </TR>
  </TABLE>
<TABLE class=ListStyle cellspacing=1 >
  <TBODY>
  <tr class=Header>
    ...
    <TH width=20%><%=SystemEnv.getHtmlLabelName(24043,user.getLanguage())%></TH>
  </tr>
  <%
    ExcelFile.init();
  ...
  %>
     ...
     <td><%=Util.getFloatValue(tempPSLdays,0)%></td>
  </tr>
  <%
    }
