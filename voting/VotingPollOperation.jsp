...
if(method.equals("pollsubmit")){
	...
	if("1".equals(isSeeResult) && !canresulet){
		out.println("<script language='javascript'>alert('"+SystemEnv.getHtmlLabelName(21724,user.getLanguage())+"');try{parent.parent.Dialog.close();}catch(e){e}window.close();</script>");
	}else{
	  response.sendRedirect("VotingPollResult.jsp?votingid="+votingid); 
	}       
}
