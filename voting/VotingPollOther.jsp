...
String titlename = Util.toScreen("网上调查:其他投票内容",user.getLanguage(),"0");
String needfav ="1";
String needhelp ="";
...
<DIV class=HdrProps><b>创建:</b><%=ResourceComInfo.getResourcename(createrid)%> <%=createdate%>  <%=createtime%>  <b>审批:</b><%if(!status.equals("0")){%><%=ResourceComInfo.getResourcename(approverid)%> <%=approvedate%>  <%=approvetime%><%}%></DIV>
...
<div>
<BUTTON class=BtnReset accessKey=B onclick="location.href='VotingPollResult.jsp?votingid=<%=votingid%>'"><U>B</U>-返回</BUTTON>
</div>
<table class=form>
...
<col width=25%><col width=55%><col width=10%><col width=10%>
  <TR class=Section>
    <TH colSpan=3>投票结果(共 <%=votingcount%> 人参与投票)</TH>
    <th><a href="votingpollremark.jsp?votingid=<%=votingid%>" target="_blank">用户意见</a></th>
  </TR>
  <TR class=separator>
    <TD class=Sep1 colSpan=4></TD></TR>
<%
...
%>
  <tr class=datalight>
    <td colspan=4><font color=red>其他投票内容</font></td>
  </tr>  
<%
...
%>
  <tr class=datadark>
    <td><%if(useranony.equals("1")){%>匿名天使<%}else{%><%=ResourceComInfo.getResourcename(resourceid)%><%}%>
