...
public class HRSynImplTask extends BaseCronJob{
    ...
    public String getSynDeptId(String hrpkcode) {
 	...
 	public void synHrmResource() {
        ...
        //同步人员
        sql = "select id,loginid,password,lastname,certificatenum,mobile,email,departmentid,jobtitle,modifydate,status,ismain,mhid,belongto,seclevel from HrmResource where appName='OA' and modifydate >= to_date('" + lastts + "','yyyy-mm-dd HH24:mi:ss') order by modifydate desc,ismain asc ";			 
        hrrs.executeSql(sql);
        while (hrrs.next()) {
