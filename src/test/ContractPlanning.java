 ...
 * 
 */
public class ContractPlanning  extends BaseCronJob
{
	...
	public void execute()
	{
		...
		{
			System.out.println("==========合约规划=============");
			System.out.println("======connection success======");
			JCO.Repository myRepository = new JCO.Repository("Repository", myConnection); // 活动的连接
			IFunctionTemplate ft = myRepository.getFunctionTemplate("ZDC_CO_CN_PLAN"); // 获取合约规划模板
			...
			Table basicContract = outputTable.getTable("ET_CO_CN_PLAN"); // 合约规划（主表）

			// 明细表数据以excel表的顺序来决定tb1, tb2, tb3
			Table tb1Detail = outputTable.getTable("ET_CO_AC_PLAN"); // 科目明细（明细表）

			// 合约规划（主表）字段与值
			List<Column> basicContractColLs = Arrays.asList(new Column[] { new Column(DB_KEY, ""),
					new Column("PSNAM", ""), new Column("POST1", ""), new Column("PROJ_PH", ""),
					new Column("POST2", ""), new Column("CONPANSTA", ""), new Column("APPSTATUS", ""),
					new Column("VERSION", ""), new Column("ZEMPLOYEE", "") });
			...
			List<Column> tb1DetailColLs = Arrays.asList(new Column[] { new Column(DB_KEY, ""),
					new Column("PLANCON", ""), new Column("COACCOUNT", ""), new Column("COACCOUNT_TEXT", ""),
					new Column("TYPE", ""), new Column("TGAMT", ""), new Column("NTTGAMT", ""),
					new Column("SPLTAMT", ""), new Column("NTSPLTAMT", ""), new Column("CON_DESC", ""),
					new Column("BIDTYPE", ""), new Column("DEPTMT", "") });

			// 此处都是为了构建科目明细细表结构HashMap，对应的明细表table结构hash，key-字段名，value-字段实体
			...
			{
				// 创建合约规划主表对应主流程中的，记录DB_KEY所生成的request_id记录下来
				for (int i = 0; i < basicContract.getNumRows(); i++)
				{
					...
					System.out.print("ZEMPLOYEE= " + ZEMPLOYEE);
					...
					tablename = str[1];
					logBean.writeLog("ET_CO_CN_PLAN new requestid =" + requestid + "||tablename=" + tablename);
					System.out.println("ET_CO_CN_PLAN new requestid =" + requestid + "||tablename=" + tablename);
			...
			if (tb1Detail.getNumRows() > 0)
			{
				...
				for (int j = 0; j < tb1Detail.getNumRows(); j++)
				{
					...
					logBean.writeLog("ET_CO_AC_PLAN科目明细明细表插入数据 = " + detialInfo);
			...
			{
				String detailInfo = "";
				logBean.writeLog("ET_CO_AC_PLAN = " + detailInfo);
				tb1SqlMap = new HashMap<String, List<SAPRecord>>();
			}
			...
			// 科目明细明细表
			String logtitleTb1 = "插入OA科目明细明细表SQL=";
	...
	private String createWorkflow(Map<String, Column> mainTableMap, String ZEMPLOYEE)
	{
		...
		String workflowid = "3813";// 合约规划流程workflowid
		...
		logBean.writeLog("合约规划流程的workflowid=" + workflowid + "||resourceid=" + resourceid);

		RequestService requestService = new RequestService();
		RequestInfo requestInfo = new RequestInfo();
		requestInfo.setWorkflowid(workflowid);
		requestInfo.setCreatorid(resourceid);
		requestInfo.setDescription("合约规划流程-" + lastname + "-" + TODAY.replace("-", ""));
