...
public class TargetCost extends BaseCronJob
{
	...
	public void execute()
	{
		...
		if (myConnection != null && myConnection.isAlive())
		{
			...
			IFunctionTemplate ft = myRepository.getFunctionTemplate("ZDC_CO_TGCS"); // 获取目标成本模板
			...
			Table basicContract = outputTable.getTable("ET_D_TGCS_ROOT"); // 目标成本（主表）

			// 明细表数据以excel表的顺序来决定tb1, tb2, tb3
			Table tb1Detail = outputTable.getTable("ET_PRJ_PAREA"); // 编制阶段面积（明细表）
			Table tb2Detail = outputTable.getTable("ET_TGCS_ACOS"); // 科目成本（明细表）
			Table tb3Detail = outputTable.getTable("ET_TGCS_HCOS"); // 业态成本（明细表）
			Table tb4Detail = outputTable.getTable("ET_CO_CON_FILE"); // 目标成本文件（明细表）
			...
			// 此处都是为了构建编制阶段面积明细表结构HashMap，对应的主表table结构hash，key-字段名，value-字段实体
			...
			// 此处都是为了构建科目成本明细表结构HashMap，对应的明细表table结构hash，key-字段名，value-字段实体
			...
			// 此处都是为了构建业态成本细表结构HashMap，对应的明细表table结构hash，key-字段名，value-字段实体
	...
	private String createWorkflow(Map<String, Column> mainTableMap, String ZEMPLOYEE)
	{
		...
		String workflowid = "3805";// 编制阶段面积流程workflowid
		...
		logBean.writeLog("TargetCost的workflowid=" + workflowid + "||resourceid=" + resourceid);

		RequestService requestService = new RequestService();
		RequestInfo requestInfo = new RequestInfo();
		requestInfo.setWorkflowid(workflowid);
		requestInfo.setCreatorid(resourceid);
		requestInfo.setDescription("目标成本流程-" + lastname + "-" + TODAY.replace("-", ""));
