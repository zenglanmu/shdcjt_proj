 ...
 * 
 */
public class SupplementaryAgreement extends BaseCronJob
{
	...
	{
		SupplementaryAgreement obj = new SupplementaryAgreement();
		obj.execute();
	}
	...
	public void execute()
	{
		...
		if (myConnection != null && myConnection.isAlive())
		{
			...
			List<Column> basicContractColLs = Arrays.asList(new Column[] { new Column(DB_KEY, ""),
					...
					new Column("CONTRACTNAME", ""), new Column("CONTTCURR", ""), new Column("CONTTCURR_NT", ""),
					new Column("UKURS", ""), new Column("FORECASTAMT", ""), new Column("AGGAMT", ""),
					new Column("TAX", ""), new Column("PSPID", ""), new Column("PSNAM", ""), new Column("BUKRS", ""),
					new Column("BUTXT", ""), new Column("LIFNR", ""), new Column("VENDNAME", ""),
					new Column("LIFNR_B", ""), new Column("VENDNAME_B", ""), new Column("BANK_ACC", ""),
					new Column("BANK_ACC_NAM", ""), new Column("LIFNR_3RD", ""), new Column("VENDNAME_3RD", ""),
					new Column("CONSTATUS", ""), new Column("APPSTATUS", ""), new Column("DELFLAG", ""),
					new Column("CUNAME", ""), new Column("CDEPT_DESC", ""), new Column("CDIVISION_DESC", ""),
					new Column("CREA_DATE", ""), new Column("CREA_TIME", ""), new Column("BCYY", ""),
			...
			if (tb1Detail.getNumRows() > 0)
			{
				...
				for (int j = 0; j < tb1Detail.getNumRows(); j++)
				{
					...
					logBean.writeLog("ET_CO_CON_CHG合同变更明细表插入数据 sapRecord = " + sapRecord.getColumnMap());
				...
				logBean.writeLog("ET_CO_CON_CHG合同变更明细表插入数据 = tb1SqlMap" + tb1SqlMap);
