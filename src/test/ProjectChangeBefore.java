 ...
 * 
 */
public class ProjectChangeBefore extends BaseCronJob
{
	...
	public void execute()
	{
		...
		{
			System.out.println("==========工程变更（事前）=============");
			System.out.println("======connection success======");
			JCO.Repository myRepository = new JCO.Repository("Repository", myConnection); // 活动的连接
			IFunctionTemplate ft = myRepository.getFunctionTemplate("ZDC_CO_SET_OSVR_S"); // 获取工程变更模板
			...
			Table tb2Detail = outputTable.getTable("ET_CO_CON_FILE"); // 合同附件（明细表）

			// 工程变更（主表）字段与值
			List<Column> basicContractColLs = Arrays.asList(new Column[] { new Column(DB_KEY, ""),
					new Column("OSVID", ""), new Column("OSVNO", ""), new Column("POST1", ""),
					new Column("CONTRACTNO", ""), new Column("CONTRACTNAME", ""), new Column("VENDNAME", ""),
					new Column("POSTION", ""), new Column("ESTCURR", ""), new Column("REVICURR", ""),
					new Column("BHESTCURR", ""), new Column("AUDITCURR", ""), new Column("BHAUDITCURR", ""),
					new Column("SGCURR", ""), new Column("SGDW_DATE", ""), new Column("DSTATUS", ""),
					new Column("APPSTATUS", ""), new Column("DTYPE", ""), new Column("RE", ""),
					new Column("RE_BG", ""), new Column("RE_CW", ""), new Column("RE_XC", ""), new Column("RE_YX", ""),
					new Column("RE_TF", ""), new Column("RE_CB", ""), new Column("RE_QTC", ""),
					new Column("RE_QT_TEXT", ""), new Column("VISATEXT", ""), new Column("CUNAME", ""),
					new Column("CDEPT_DESC", ""), new Column("CDIVISION_DESC", ""), new Column("CREA_DATE", ""),
			...
			// 合同附件明细表循环
			logBean.writeLog("tb3Detail.getNumRows() =" + tb2Detail.getNumRows());
			...
			// 合同附件明细表
			String logtitleTb2 = "插入OA合同附件明细表SQL=";
