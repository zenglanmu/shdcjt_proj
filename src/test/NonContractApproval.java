 ...
 * 
 */
public class NonContractApproval extends BaseCronJob
{
	...
	public void execute()
	{
		...
		{
			System.out.println("==========非合同审批=============");
			System.out.println("======connection success======");
			JCO.Repository myRepository = new JCO.Repository("Repository", myConnection); // 活动的连接
			IFunctionTemplate ft = myRepository.getFunctionTemplate("ZDC_CO_NCONT"); // 获取非合同审批模板
			...
			// 合同基本信息（主表）字段与值
			List<Column> basicContractColLs = Arrays.asList(new Column[] { new Column(CONTRACT_NO, ""),
					new Column("CONTCATE", ""), new Column("ACONTNO", ""), new Column("CONTRACTNAME", ""),
					new Column("CONTTCURR", ""), new Column("CONTACURR", ""), new Column("UKURS", ""),
					new Column("TAX", ""), new Column("FORECASTAMT", ""), new Column("AGGAMT", ""),
					new Column("PCONTNO", ""), new Column("PLANCON_DES", ""), new Column("PSPID", ""),
					new Column("PSNAM", ""), new Column("BUKRS", ""), new Column("BUTXT", ""), new Column("LIFNR", ""),
					new Column("VENDNAME", ""), new Column("CONSTATUS", ""), new Column("APPSTATUS", ""),
					new Column("CUNAME", ""), new Column("CDEPT_DESC", ""), new Column("CDIVISION_DESC", ""),
					new Column("CREA_DATE", ""), new Column("CREA_TIME", ""), new Column("ZEMPLOYEE", "") });
	...
	private String createWorkflow(Map<String, Column> mainTableMap, String ZEMPLOYEE)
	{
		...
		String workflowid = "3802";// 非合同审批流程workflowid
		...
		logBean.writeLog("非合同审批流程的workflowid=" + workflowid + "||resourceid=" + resourceid);

		RequestService requestService = new RequestService();
		RequestInfo requestInfo = new RequestInfo();
		requestInfo.setWorkflowid(workflowid);
		requestInfo.setCreatorid(resourceid);
		requestInfo.setDescription("非合同审批流程-" + lastname + "-" + TODAY.replace("-", ""));
