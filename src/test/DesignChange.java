 ...
 * 
 */
public class DesignChange extends BaseCronJob
{
	...
	public void execute()
	{
		...
		{
			System.out.println("==========设计变更（事后）=============");
			System.out.println("======connection success======");
			JCO.Repository myRepository = new JCO.Repository("Repository", myConnection); // 活动的连接
			IFunctionTemplate ft = myRepository.getFunctionTemplate("ZDC_CO_REV"); // 获取设计变更模板
			...
			Table basicContract = outputTable.getTable("ET_CO_REV"); // 变更信息（主表）

			// 明细表数据以excel表的顺序来决定tb1, tb2, tb3
			Table tb1Detail = outputTable.getTable("ET_CO_REV_IMP"); // 实际影响单位信息（明细表）
			Table tb2Detail = outputTable.getTable("ET_CO_REV_RPS"); // 责任方信息（明细表）
			...
			List<Column> basicContractColLs = Arrays.asList(new Column[] { new Column(DB_KEY, ""),
					new Column("LINKNO", ""), new Column("REVISIONNO", ""), new Column("EXREVINNO", ""),
					new Column("POST1", ""), new Column("SUBPROJ", ""), new Column("ESTCURR", ""),
					new Column("AUDITCURR", ""), new Column("AUDITCURR_NT", ""), new Column("REVICURR", ""),
					new Column("DSTATUS", ""), new Column("APPSTATUS", ""), new Column("REVIDATE", ""),
					new Column("ESTCURR_NT", ""), new Column("REASON", ""), new Column("RE_XG", ""),
					new Column("RE_YH", ""), new Column("RE_CW", ""), new Column("RE_YX", ""), new Column("RE_ZB", ""),
					new Column("RE_TZ", ""), new Column("RE_GY", ""), new Column("RE_ZR", ""),
					new Column("RE_QT_TEXT", ""), new Column("REVISIONTEXT", ""), new Column("CREA_UNAME", ""),
					new Column("LDEPT", ""), new Column("LDIVISION", ""), new Column("CREA_DATE", ""),
					new Column("CREA_TIME", ""), new Column("ZEMPLOYEE", "") });
			...
			List<Column> tb1DetailColLs = Arrays.asList(new Column[] { new Column(DB_KEY, ""),
					new Column("ZGLD_D_SET_OSVI", ""), new Column("ACONTNO", ""), new Column("CONTRACTNAME", ""),
					new Column("NAME1", ""), new Column("PSNAM", ""), new Column("ESTCURR", ""),
					new Column("ESTCURR_NT", ""), new Column("AUDITCURR", ""), new Column("AUDITCURR_NT", ""),
					new Column("WAERS", ""), new Column("SPLSTATUS", "") });

			// 此处都是为了构建实际影响单位信息明细表结构HashMap，对应的主表table结构hash，key-字段名，value-字段实体
			...
			List<Column> tb2DetailColLs = Arrays.asList(new Column[] { new Column(DB_KEY, ""),
					...
					new Column("NPDAMT", ""), new Column("WAERS", "") });

			// 此处都是为了构建责任方信息明细表结构HashMap，对应的明细表table结构hash，key-字段名，value-字段实体
			...
			if (basicContract.getNumRows() > 0)
			{
				...
				for (int i = 0; i < basicContract.getNumRows(); i++)
				{
					...
					tablename = str[1];
					logBean.writeLog("ET_CO_REV new requestid =" + requestid + "||tablename=" + tablename);
					System.out.println("ET_CO_REV new requestid =" + requestid + "||tablename=" + tablename);
			...
			if (tb1Detail.getNumRows() > 0)
			{
				...
				for (int j = 0; j < tb1Detail.getNumRows(); j++)
				{
					...
					logBean.writeLog("ET_CO_REV_IMP实际影响单位信息明细表插入数据 sapRecord = " + sapRecord.getColumnMap());
					String detialInfo = sb.toString();
					logBean.writeLog("ET_CO_REV_IMP实际影响单位信息明细表插入数据 = " + detialInfo);
				...
				logBean.writeLog("ET_CO_REV_IMP实际影响单位信息明细表插入数据 = tb1SqlMap" + tb1SqlMap);
			...
			if (tb2Detail.getNumRows() > 0)
			{
				...
				for (int j = 0; j < tb2Detail.getNumRows(); j++)
				{
					...
					sapRecord.putKeyValue(REQUEST_ID, new Column(REQUEST_ID, reqid));
					String detialInfo = sb.toString();
					logBean.writeLog("ET_CO_CON_QZ责任方信息明细表插入数据 = " + detialInfo);
			...
			// 实际影响单位信息明细表
			String logtitleTb1 = "插入OA实际影响单位信息明细表SQL=";
			RecordSet rsTb1 = new RecordSet();
			RecordSet rsTb1Add = new RecordSet();

			// 责任方信息明细表
			String logtitleTb2 = "插入OA责任方信息明细表SQL=";
	...
	private String createWorkflow(Map<String, Column> mainTableMap, String ZEMPLOYEE)
	{
		...
		String workflowid = "3811";// 设计变更流程workflowid
		...
		logBean.writeLog("设计变更流程的workflowid=" + workflowid + "||resourceid=" + resourceid);

		RequestService requestService = new RequestService();
		RequestInfo requestInfo = new RequestInfo();
		requestInfo.setWorkflowid(workflowid);
		requestInfo.setCreatorid(resourceid);
		requestInfo.setDescription("设计变更流程-" + lastname + "-" + TODAY.replace("-", ""));
