 ...
 <HEAD>
   <TITLE> New Document </TITLE>
	...
	<link href="/css/Weaver.css" type=text/css rel=stylesheet>
  	<script type="text/javascript" src="/js/menu/xtree.js"></script>
	<script type="text/javascript" src="/js/weaver.js"></script>
    <script type="text/javascript" src="/js/menu/xmlextras.js"></script>
    <script type="text/javascript" src="/js/menu/xloadtree.js"></script>   
	
	<link rel="stylesheet" href="/js/modalbox/dhtmlwindow.css" type="text/css" />
	<link rel="stylesheet" href="/js/modalbox/modal.css" type="text/css" />
	<script type="text/javascript" src="/js/modalbox/dhtmlwindow.js"></script>
	<script type="text/javascript" src="/js/modalbox/modal.js"></script>
 ...
 <BODY>
	...
	if (window.DOMParser)
	  ...
	  xmlDoc.loadXML("<%=xmlStr%>");
		...
		var tree = new WebFXLoadTree(treeName, "", "", "", "/images/xp/folder.png", "/images/xp/openfolder.png",xmlDoc);
		document.write(tree);
...
<script type="text/javascript">
	...
	function onAdd(parentMenuId,treeMenuId,parentMenuText){			
		...
		addwin=dhtmlmodal.open("agreebox", "iframe", "/formmode/menu/MenuMaintenanceAdd.jsp?menuflag=<%=menuflag%>&type=<%=type%>&id="+parentMenuId+"&resourceId=<%=resourceId%>&resourceType=<%=resourceType%>&sync=1", "<%=SystemEnv.getHtmlLabelName(20604,user.getLanguage())%>:"+parentMenuText, "width=500px,height=310px,center=1,resize=1,scrolling=0", "recal")
			...
			if(thisNode.childNodes.length>0){
				...
				if(isShowAllSubDel){
					if(confirm("<%=SystemEnv.getHtmlLabelName(20830,user.getLanguage())%>")){
						GetContent("/formmode/menu/MenuMaintenanceOperation.jsp?menuflag=<%=menuflag%>&type=<%=type%>&infoId="+menuId+"&resourceId=<%=resourceId%>&resourceType=<%=resourceType%>&method=delthisall");
						thisNode.remove();
					}
				} else {
					GetContent("/formmode/menu/MenuMaintenanceOperation.jsp?menuflag=<%=menuflag%>&type=<%=type%>&infoId="+menuId+"&resourceId=<%=resourceId%>&resourceType=<%=resourceType%>&method=del");
					thisNode.remove();
				}
			}
		}
 ...
 function hiddenNoVisbleMenu(obj){
	 obj.disabled=true;
	 window.location="/formmode/menu/MenuMaintenanceList.jsp?menuflag=<%=menuflag%>&type=<%=type%>&resourceId=<%=resourceId%>&resourceType=<%=resourceType%>&isCustom=<%=isCustom%>&mode=hidden"
 }

 function showNoVisbleMenu(obj){
	 obj.disabled=true;
	 window.location="/formmode/menu/MenuMaintenanceList.jsp?menuflag=<%=menuflag%>&type=<%=type%>&resourceId=<%=resourceId%>&resourceType=<%=resourceType%>&isCustom=<%=isCustom%>&mode=visible"
 }
 ...
 function synchAll(obj){
	 if(confirm("<%=SystemEnv.getHtmlLabelName(20764,user.getLanguage())%>")) {
		 obj.disabled=true;
		 window.location="/formmode/menu/MenuMaintenanceOperation.jsp?menuflag=<%=menuflag%>&type=<%=type%>&resourceId=<%=resourceId%>&resourceType=<%=resourceType%>&isCustom=<%=isCustom%>&mode=synchall"	
	 }
 }
