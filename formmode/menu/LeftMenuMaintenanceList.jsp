...
<%
    ...
    String titlename = SystemEnv.getHtmlLabelName(18986, user.getLanguage());
    titlename += " - ";
...
<%
RCMenu += "{"+SystemEnv.getHtmlLabelName(18378,user.getLanguage())+",javascript:onNew(this),_self} " ;
...
<col width="*">
<% if(isShowSyncInfo) { %>
<tr class="Title" ><th colspan="2"><%=SystemEnv.getHtmlLabelName(19014,user.getLanguage())%></th></tr>
<tr class="Spacing" ><td class="Line1" colspan="2"></td></tr>
<tr>
    <td><%=SystemEnv.getHtmlLabelName(19016, user.getLanguage())%></td>
...
<TABLE class=ListStyle cellspacing=1>
	...
	<TR class=Header>		
		<TH width="30"><%=SystemEnv.getHtmlLabelName(89,user.getLanguage())%></TH>
		<th width="*"><%=SystemEnv.getHtmlLabelName(18390,user.getLanguage())%></th>
		<th width="120"></th>
	</TR>
	<TR class=Line>
		<TD colSpan=4></TD>
            ...
            <table border="1" id="tbl<%=i%>" style="border-collapse:collapse;border-style:solid;width:100%" topid="<%=topId%>">
                <tbody>
                    <tr class=field>
						...
						topid="<%=topId%>"						
						>
                            <%if (subLevelLeftMenuInfos != null) {%><img src="/images/collapsed.gif"
                                                                         onclick="collapseTR()"
...
<table id="report" class="ReportStyle" style="margin-top:15px;">
    <TBODY>
        <TR><TD>            
            <B>1、<%=SystemEnv.getHtmlLabelName(89, user.getLanguage())%>:</B><%=SystemEnv.getHtmlLabelName(19051, user.getLanguage())%>
            <BR>
            <B>2、<%=SystemEnv.getHtmlLabelName(17607, user.getLanguage())%>:</B><%=SystemEnv.getHtmlLabelName(19052, user.getLanguage())%>
            <BR>
            <BR>
            <B><%=SystemEnv.getHtmlLabelName(15736, user.getLanguage())%>:</B><%=SystemEnv.getHtmlLabelName(19053, user.getLanguage())%>
        </TD>
        </TR>
    </TBODY>
</table>
