...
if(modeId > 0){
	...
	RCMenu += "{"+SystemEnv.getHtmlLabelName(91,user.getLanguage())+",javascript:deleteValue(),_self} " ;
	RCMenuHeight += RCMenuHeightStep;
}
%>
...
<TR class="Title">
	<TH colSpan=3>
		<%=SystemEnv.getHtmlLabelName(68,user.getLanguage())%>
	</TH>
</TR>
		...
		<select class=inputstyle  style="width:100%" name=fieldid id=fieldid title="<%=SystemEnv.getHtmlLabelName(15620,user.getLanguage())%>" 
			...
			<%
			 for(int i = 0; i < mainfields.size(); i++){
				 ...
				 String optionValue = viewtype+"_"+fieldid+"_"+fieldhtmltype+"_"+type+"_-1";
			 %>
...
<COLGROUP>
   ...
   <COL width="90%">
	<TR class="Header">
		<TH colSpan=2><%=SystemEnv.getHtmlLabelName(15635,user.getLanguage())%></TH></TR>
 	<TR class=header>
 		<th></th>
