...
String titlename = SystemEnv.getHtmlLabelName(28477,user.getLanguage());
String needfav ="";
String needhelp ="";
%>
...
<%
String types = "1,2";
String typenames = SystemEnv.getHtmlLabelName(16388,user.getLanguage())+","+
	...
	<COLGROUP>
	  ...
	  <TBODY>
	    <TR>
	      ...
	      <TD colSpan=4 align="right">
		    <a style="color:#262626;cursor:pointer; TEXT-DECORATION:none" onclick='linkagevaaddrow("<%=types%>","<%=typenames%>","<%=selectfields%>","<%=selectfieldnames%>","<%=selectfieldvalues%>","<%=selectfieldvaluenames%>")'>
		     <img src="/js/swfupload/add.gif" align="absmiddle" border="0">&nbsp;<%=SystemEnv.getHtmlLabelName(21690,user.getLanguage())%>
		    </a>
			&nbsp;
			<a style="color:#262626;cursor:pointer;TEXT-DECORATION:none"  onclick="linkagevadelrow()">
			 <img src="/js/swfupload/delete.gif" align="absmiddle" border="0">&nbsp;<%=SystemEnv.getHtmlLabelName(16182,user.getLanguage())%>
			</a>
	    </TR>
	    ...
	    <TR class=Header >
		  ...
		  <TH><%=SystemEnv.getHtmlLabelName(21686,user.getLanguage())%></TH>
		  <TH><%=SystemEnv.getHtmlLabelName(21687,user.getLanguage())%></TH>
		  <TH><%=SystemEnv.getHtmlLabelName(21688,user.getLanguage())%></TH>
		  <TH><%=SystemEnv.getHtmlLabelName(21689,user.getLanguage())%></TH>
		</TR>
		<%
		String checkfield="";
...
<!--	          </option>-->
	      ...
	      <TD><select class=inputstyle name='selectfieldid_<%=i%>' onchange="lavachangefield(this.value,<%=i%>)">
		      ...
		      if(!selectfieldid.substring(0,selectfieldid.indexOf("_")).equals("") && notselectfield==-1){%>
		          <option value="" style="background-color:red" selected><%=SystemEnv.getHtmlLabelName(21697,user.getLanguage())%></option>
		      <%}%>
		      </select>
		      <span id="selectfieldid_<%=i%>span"><%if(selectLists.size()<1){%><IMG src='/images/BacoError.gif' align=absMiddle><%}%></span>
	      </TD>
	      </TD>
	      <TD><select class=inputstyle name='selectfieldvalue_<%=i%>'>
		      ...
		      <%if(!selectfieldvalue.equals("") && notselectfieldvalue==-1){%>
		          <option value="" style="background-color:red" selected><%=SystemEnv.getHtmlLabelName(21698,user.getLanguage())%></option>
		      <%}%>
		      </select>
		      <span id="selectfieldvalue_<%=i%>span"><%if(svalueLists.size()<1){%><IMG src='/images/BacoError.gif' align=absMiddle><%}%></span>
	      </TD>
	      <TD>
	          <button type='button' class=Browser onclick="lavaShowMultiField('changefieldidsspan_<%=i%>','changefieldids_<%=i%>',modeid_<%=i%>.value,selectfieldid_<%=i%>.value)"></button><SPAN id="changefieldidsspan_<%=i%>"><%if(changefieldids.equals("")){%><IMG src='/images/BacoError.gif' align=absMiddle><%}else{%><%=fieldnames%><%}%></SPAN>
	      ...
	      <select class=inputstyle name='viewattr_<%=i%>' onchange="">
	        ...
	        <option value="3" <%if(viewattr.equals("3")){%>selected<%}%>><%=SystemEnv.getHtmlLabelName(17873,user.getLanguage())%></option>
	      </select>
	      </TD>
	    </TR>
	    <%	i++;
		...
		switch(j) {
            ...
            case 4:
            {
                ...
                var sHtml = "<button type='button' class=Browser onclick='lavaShowMultiField(\"changefieldidsspan_"+rowindex+"\",\"changefieldids_"+
                            ...
                            "'><IMG src='/images/BacoError.gif' align=absMiddle></SPAN><input type='hidden' class=inputstyle name='changefieldids_"+rowindex+"' id='changefieldids_"+rowindex+"'>";
                oDiv.innerHTML = sHtml;
                oCell.appendChild(oDiv);
                break;
            }
            case 5:
            {
                ...
                sHtml+="<option value=3><%=SystemEnv.getHtmlLabelName(17873,user.getLanguage())%></option>";
                sHtml+="</select>";
                oDiv.innerHTML = sHtml;
                oCell.appendChild(oDiv);
                break;
