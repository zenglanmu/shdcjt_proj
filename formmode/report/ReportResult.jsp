...
if(checkcons!=null){
	for(int i=0;i<checkcons.length;i++){
		...
		System.out.println("tmphtmltype:"+tmphtmltype+"		tmptype:"+tmptype+"		tmpcolname:"+tmpcolname+"		tmpvalue:"+tmpvalue);
	...
	if(isShowList.indexOf("-1")!=-1){
		...
		if(rs1.next()){
		    ...
		    fields.add("modedatacreatedate");System.out.println("modedatacreatedate:");
	...
	if(isShowList.indexOf("-2")!=-1){
		...
		System.out.println("select * from mode_ReportDspField where reportid = " + reportid + " and fieldid = -2");
		if(rs1.next()){
			rcListBean = new ReportCompositorListBean();
		    rcListBean.setCompositorList(rs1.getDouble("dsporder"));
		    rcListBean.setSqlFlag("a");
		    ...
		    compositorColList.add(rcListBean);
		    fields.add("modedatacreater");System.out.println("modedatacreater:");
		    if("1".equals(rs1.getString("dborder"))){
		        reportCompositorOrderBean = new ReportCompositorOrderBean();
		        reportCompositorOrderBean.setCompositorOrder(rs1.getInt("compositororder"));
				reportCompositorOrderBean.setOrderType(Util.null2String(rs1.getString("dbordertype")));
...
function outputexcel() {
    ...
    excelOutForm.action = "/formmode/report/ReportResultDataOutputExcel.jsp?reportid=<%=reportid%>&isShowList=<%=isShowList.toString()%>&useddetailtable=<%=useddetailtable%>";
    excelOutForm.method = "POST";
    var excelOutParam = "<INPUT type=\"hidden\" name=\"pmSql\" value=\"" + datasql + "\">";
    excelOutForm.innerHTML = excelOutParam
    excelOutForm.submit();
    //window.open("/formmode/report/ReportResultDataOutputExcel.jsp?reportid=<%=reportid%>&isShowList=<%=isShowList.toString()%>&useddetailtable=<%=useddetailtable%>&pmSql="+encodeURIComponent(datasql));
}
