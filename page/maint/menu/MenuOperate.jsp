...
<%
	...
	if("add".equals(method)){
		...
		sql="insert into menucustom(id,menuname,menuicon,menuhref,menutarget,menuparentid,menuindex,menutype,righttype,rightvalue,sharetype,sharevalue) values (1,'首页','','','mainiframe','0','1','"+now+"','','','5','1')";
		//System.out.println("add menucustom : "+sql);
		rs.executeSql(sql);

		response.sendRedirect("/page/maint/menu/MenuEdit.jsp?menutype="+menutype+"&id="+now+"&subCompanyId="+subCompanyId);
		...
		response.sendRedirect("/page/maint/menu/MenuCenter.jsp?menutype="+menutype+"&subCompanyId="+subCompanyId);
		...
		for(int i=0;i<jsonArray.length();i++){
			...
			if(index!=-1 && _index!=-1){
				...
				System.out.println("nodestr.length===="+menuhref);
		...
		//MenuCenterCominfo.updateCache(menuid);
		MenuCenterCominfo.clearCominfoCache();
		response.sendRedirect("/page/maint/menu/MenuCenter.jsp?menutype="+menutype+"&subCompanyId="+subCompanyId+"&msg=1");
		...
		//System.out.println("update menucenter set subCompanyId="+targetSubId+" where id = '"+tranMenuId+"'");
		//MenuCenterCominfo.updateCache(tranMenuId);
		MenuCenterCominfo.clearCominfoCache();
		response.sendRedirect("/page/maint/menu/MenuCenter.jsp?menutype=2&subCompanyId="+srcSubId);
