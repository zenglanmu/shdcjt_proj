...
<div id='divNode' style='display:none'>
	...
	<table class="viewForm" style="padding:10px;" > 							
		<tr>
			<td width="20%"><%=SystemEnv.getHtmlLabelName(229,user.getLanguage())%><!--标题--></td>
			<td width="80%" class="field">
		...
		<!--<tr>
			...
			<td width="80%" class="field" >
				...
				<span id=iconSpan name=iconSpan></span>
			</td>
		</tr>
		<tr><td class="line" colspan=2></td></tr>	-->
		<input type="hidden" id="icon"  name="icon" class='inputstyle' value="/js/extjs/resources/images/default/tree/leaf.gif" style="width:90%;" onChange="checkinput('icon','iconSpan')">
		<tr>
			<td width="20%"><%=SystemEnv.getHtmlLabelName(16208,user.getLanguage())%><!--链接--></td>
			<td width="80%"  class="field">
				<input type="text" id="href" name="href" class='inputstyle' value="" style="width:325;" onChange="checkinput('href','hrefSpan')">
		...
		<tr>
			...
			<td width="80%"  class="field">
			  	<SELECT class=InputStyle  name=sharetype id="sharetype" onchange="onChangeSharetype(this,sharevalue,sharetext)" >   
	                     ...
	                     <option value="5" selected><%=SystemEnv.getHtmlLabelName(235,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(127,user.getLanguage())%></option>
	                 </SELECT>
	                 &nbsp;&nbsp;
		...
		<tr>
			<td width="20%"></td>
			<td width="80%" class="field">
			<a href="javascript:addPurview()"><%=SystemEnv.getHtmlLabelName(611,user.getLanguage())%></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:removePurview()"><%=SystemEnv.getHtmlLabelName(91,user.getLanguage())%></a>
			</td>
		</tr>
		<tr style="height:1px;"><td class="line" colspan=2></td></tr>
		
		<tr id='purviewList'>
			<td width="20%"><%=SystemEnv.getHtmlLabelName(385,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(320,user.getLanguage())%></td>
			<td width="80%" class="field">
...
<!--
	...
	Ext.onReady(function(){
		...
		var tree = new Tree.TreePanel({
			...
			loader: new Tree.TreeLoader({
				dataUrl:'MenuTreeGet.jsp?typeid=<%=id%>&userid=<%=user.getUID()%>&subCompanyId=<%=subCompanyId%>&hasRight=<%=hasRight%>'
			})
		});
		...
		root = new Tree.AsyncTreeNode({			
			text: '<%=SystemEnv.getHtmlLabelName(345,user.getLanguage())%>',
			draggable:<%=hasRight%>,
			allowDrop:<%=hasRight%>,
			allowDrag:<%=hasRight%>,
		...
		Ext.get('loading').fadeOut();
		 ...
		 var win = new Ext.Window({
				...
				autoScroll:true,
				modal: true,
				title:'<%=SystemEnv.getHtmlLabelName(20604,user.getLanguage())%>',
				items: new Ext.Panel({ 
				   id:'divNode',         
				   autoHeight :true,       
				   contentEl:'divNode'
				...
				buttons: [{
					...
					handler: function(){  
						if(nodeOpType=="add"){
							...
							cNode.attributes.icon="/js/extjs/resources/images/default/tree/folder.gif";
							cNode.expand();
							...
							if($("#icon").val()!=""){
								if(tempObj.attr("src").substring(pos+1)=="s.gif"){
	...
	*/
	...
	function onAddNode() {  
			...
			$("#nameSpan").html("<IMG src='/images/BacoError.gif' align=absMiddle>");
			nodeAddIndex++;
			nodeOpType="add";
			$("#name").val("");
			//$("#icon").val("");
     ...
     function createContextMenu(nodeId) {  
   		 if(nodeId=='-1'){
             ...
             id : 'project-ctx',  
             items : [ {  
						 text : '<%=SystemEnv.getHtmlLabelName(20604,user.getLanguage())%>',  
						 scope : this,  
						 handler :onAddNode
					 }]  
         	});
             ...
             id : 'project-ctx',  
             items : [ {  
						 text : '<%=SystemEnv.getHtmlLabelName(103,user.getLanguage())%>',  
						 scope : this,  
						 ...
						 text : '<%=SystemEnv.getHtmlLabelName(91,user.getLanguage())%>',  
						 scope : this,  
...
function onShowLoginPages(input,span,eid){
	...
	if (datas){
		...
		if (mtype==1){
			if (datas.id!="") {
				$($G(span)).html("<a href='/homepage/LoginHomepage.jsp?hpid="+datas.id +"' target='_blank'>" + datas.name+ "</a>");
				$($G(input)).val("/homepage/LoginHomepage.jsp?hpid="+datas.id);
			}else {
				$($G(span)).html("");
				$($G(input)).val("");
			}
			...
			if (datas.id!="") {
				if(datas.name.length>4){
					datas.name=datas.name.substring(0,4)+"...";
			    }
				$($G(span)).html("<a href='/homepage/LoginHomepage.jsp?hpid="+datas.id +"' target='_blank'>" + datas.name+ "</a>");
				$($G(input)).val("/homepage/LoginHomepage.jsp?hpid="+datas.id);
			}
