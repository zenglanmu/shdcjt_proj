     ...
     <tr>
      ...
      %>
       <%if(mainType.equals("remind")){%>
         <td onclick="showRemind(this,<%=eid%>,'<%=mainType%>')" <%=i==0?"class='selected'":"class='unselected'"%> id="<%=eid%>_remind"><%=SystemEnv.getHtmlLabelName(26928,user.getLanguage())%><%=size>0?"("+size+")":""%></td><!-- 提醒 -->
       <%}else if(mainType.equals("comment")){ %>  
         <td onclick="showRemind(this,<%=eid%>,'<%=mainType%>')" <%=i==0?"class='selected'":"class='unselected'"%> id="<%=eid%>_comment"><%=SystemEnv.getHtmlLabelName(675,user.getLanguage())%><%=size>0?"("+size+")":""%></td><!-- 评论 -->
       <%}else if(mainType.equals("update")){ %>  
         <td onclick="showRemind(this,<%=eid%>,'<%=mainType%>')" <%=i==0?"class='selected'":"class='unselected'"%> id="<%=eid%>_update"><%=SystemEnv.getHtmlLabelName(17744,user.getLanguage())%><%=size>0?"("+size+")":""%></td><!-- 更新 -->
       <%} %>
      <% } %>   
     </tr>
   </table>
	 ...
	 <!-- 关注申请 -->
	  <%if("1".equals(remindType)){%>
		...
		<%if("9".equals(remindType)){
			...
			String disucssid=Util.TokenizerString2(remindValue, "|")[0];  //微博id
			String beReplayid=Util.TokenizerString2(remindValue, "|")[1];   //被评论的评论id
			String replayid=Util.TokenizerString2(remindValue, "|")[2];//评论id
