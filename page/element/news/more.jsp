...
<%
	...
	String tabSql = "";
	if(!tabId.equals("")){
		tabSql="select tabId,tabTitle,sqlWhere from hpNewsTabInfo where eid="+eid+" and tabid="+tabId+" order by tabId";
		rs.execute(tabSql);
		if(rs.next()){
			whereKey = rs.getString("sqlWhere");
		}
	}else{
		tabSql="select tabId,tabTitle,sqlWhere from hpNewsTabInfo where eid="+eid+" order by tabId";
		rs.execute(tabSql);
		if(rs.next()){
			whereKey = rs.getString("sqlWhere");
		}else{
...
function gotoBefore(){
	...
	current.value = document.getElementById("pageBefore").value;
	if(current.value>0 && current.value<=parseInt(maxPage))
		document.weaver.submit();
	else{
		document.getElementById("pageAfter").value=index;
		document.getElementById("pageBefore").value=index;
	}
}

function gotoAfter(){
	var current = document.getElementById("index");
	current.value = document.getElementById("pageAfter").value;
	if(current.value>0 && current.value<=parseInt(maxPage))
		document.weaver.submit();
	else{
		document.getElementById("pageAfter").value=index;
...
//尾页
function lastPage(){
	document.getElementById("index").value=<%=totalPage%>;
	document.weaver.submit();
}
