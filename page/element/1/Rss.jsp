...
<div id="tabContainer_<%=eid%>" class='tab2' style="width:100%;height:32px;overflow:hidden;display:none;">
		...
		<tr>
			<%for(int i=0;i<tabIdList.size();i++){ 
				...
				<%if(currenttab.equals(tabIdList.get(i))){ %>
					<td style="word-wrap:break-word;padding-top:5px;vertical-align:top;" id="tab_<%=eid%>" tabId=<%=tabIdList.get(i) %> class='tab2selected' onclick="loadContent('<%=eid%>','<%=url%>','<%=queryString+"&tabWhere="+tabWhereList.get(i)%>',event)">
