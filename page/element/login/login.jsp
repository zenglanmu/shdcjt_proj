...
<%
	...
	int needvalidate=settings.getNeedvalidate();//0: ·ñ,1: ÊÇ
	
%>
<style>
	#loginelement_<%=eid%> {
		MARGIN: 0px; BACKGROUND-COLOR: #d9d9d9
	}
	#frmLogin_<%=eid%> {
		DISPLAY: inline
	}
							...
							<TD width='100%' align=middle style="FONT: 9pt Verdana; COLOR: #333333">
								<FORM name=frmLogin_<%=eid%> id="frmLogin_<%=eid%>" action=<%=actionpageValue %> method=post target=<%if("2".equals(linkmode)){ %>_blank<%}else{ %>_parent<%} %>>
									...
									<TABLE cellSpacing=1 cellPadding=4 width=<%=widthValue %> height=<%=heightValue %> align=center>
										<TBODY>
											<TR>
												<TD style="color:#FF0000;font-size:9pt">
													<span class="left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
													<span  id="messageSpan_<%=eid%>"></span>
												</TD>
											</TR>
											<TR>
												<TD style="FONT: 9pt Verdana; COLOR: #333333">
													<span class="left"><%=SystemEnv.getHtmlLabelName(2072,7)%>:</span>
													<INPUT class=text size=12 name=<%=userparamname %> id="<%=userparamname %>">
												</TD>
											</TR>
											<TR>
												<TD style="FONT: 9pt Verdana; COLOR: #333333">
													<span class="left">ÃÜ&nbsp;&nbsp;&nbsp;Âë:</span>
											...
											<TR>
												<TD style="FONT: 9pt Verdana; COLOR: #333333">
													<span class="left">&nbsp;</span><img border=0 align='absmiddle'
											...
											<%} %>
											<TR>
												<TD align=left style="FONT: 9pt Verdana; COLOR: #333333">
													<button id="submitbutton_<%=eid %>" onclick="checkall('<%=eid %>','<%=userparamname %>','<%=userparampass %>','<%=needvalidate %>','<%=needusb %>');" style="BORDER-RIGHT: #bdbdbd 1px solid; BORDER-TOP: #bdbdbd 1px solid; FILTER: progid:DXImageTransform.Microsoft.Gradient(GradientType=0, StartColorStr=#ffffff, EndColorStr=#E7E7E7); BORDER-LEFT: #bdbdbd 1px solid; CURSOR: hand; BORDER-BOTTOM: #bdbdbd 1px solid;width:70px;height:20px;">
													 µÇ Â¼ 
													</button>
													&nbsp;
