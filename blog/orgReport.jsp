...
<div style="overflow:auto;width: 100%;height: 100%;">
      <div style="height:30px;line-height:30px;margin-top:0px" class="TopTitle">
	    <div class="topNav" style="float: left;margin-left:10px;color: #1D76A4;">
			<%=SystemEnv.getHtmlLabelName(26469,user.getLanguage())%>
		</div>
...
<div style="width:100%;">
 ...
 %>
	<tr class="item">
	    <td align="left"><span style="cursor: pointer;" onclick="extendRecord(this,'<%=content%>',<%=type%>,<%=year%>,<%=month %>,'blog','<%=reportType %>')"  extend="false" isLoad='false' ><img src="images/extend.png"/><%=SystemEnv.getHtmlLabelName(26470,user.getLanguage())%></span></td><!-- 微博报表 -->
	    <%for(int i=0;i<totaldateList.size();i++){
	    ...
	    <td>
	    	...
	    	%>
	    	<span title="<%=SystemEnv.getHtmlLabelName(15178,user.getLanguage())%><%=totalUnsubmit%><%=SystemEnv.getHtmlLabelName(1925,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(26932,user.getLanguage())%><%=totalWorkday*totalAttention%><%=SystemEnv.getHtmlLabelName(1925,user.getLanguage())%>"><%=reportManager.getReportIndexStar(index)%></span><%=index%><a href="javascript:openChart('<%=reportType %>','<%=content%>',<%=2011%>,<%=type%>,'<%=orgName%><%=SystemEnv.getHtmlLabelName(26467,user.getLanguage())+SystemEnv.getHtmlLabelName(15101,user.getLanguage())%>')" style="text-decoration: none"><img src="images/chart.png" style="margin-left: 3px;border: 0px" align="absmiddle" /></a></td>
	</tr>
  ...
  <%
	if(appDao.getAppVoByType("mood").isActive()){
		  ...
		  %>
		  <tr class="item">
	      <td align="left"><span style="cursor: pointer;" onclick="extendRecord(this,'<%=content%>',<%=type%>,<%=year%>,<%=month %>,'mood','mood')" extend="false" isLoad='false'><img src="images/extend.png"/><%=SystemEnv.getHtmlLabelName(26769,user.getLanguage())+SystemEnv.getHtmlLabelName(15101,user.getLanguage())%></span></td>  <!-- 心情报表 -->
	    <%for(int i=0;i<totaldateList.size();i++){
	        if(i<resultCountList.size()){
	        	boolean isWorkday=((Boolean)isWorkdayList.get(i)).booleanValue();
...
%>
  ...
  <table style="margin-top: 8px">
       <tr>
          <td><%=SystemEnv.getHtmlLabelName(85,user.getLanguage())%>：<span style="margin-right: 8px"><img src="images/submit-no.png" align="absmiddle" style="margin-right: 5px" /><%=SystemEnv.getHtmlLabelName(15178,user.getLanguage())%></span></td>
          <td><span  style="margin-right: 8px"><img src="images/submit-ok.png" align="absmiddle" style="margin-right: 5px" /><%=SystemEnv.getHtmlLabelName(15176,user.getLanguage())%></span></td>
       ...
       <%if(appDao.getAppVoByType("mood").isActive()){ %>
	       <tr>
	          <td style="padding-left: 38px"><span  style="margin-right: 8px"><img src="images/mood-unhappy.png" align="absmiddle" width="16px" style="margin-right: 5px" /><%=SystemEnv.getHtmlLabelName(26918,user.getLanguage())%></span></td>
	          <td><span  style="margin-right: 8px"><img src="images/mood-happy.png" align="absmiddle" width="16px" style="margin-right: 5px" /><%=SystemEnv.getHtmlLabelName(26917,user.getLanguage())%></span></td>
