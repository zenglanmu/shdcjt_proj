...
<%} %>	
	...
	<tr class="item<%=conditionid%>">
	    <td><input type="checkbox" class="condition" conType="<%=type%>" conValue="<%=value%>"/><a href="viewBlog.jsp?blogid=<%=value%>" class="index" target="_blank"><%=ResourceComInfo.getLastname(value)%></a><img src="images/delete.png" onclick="delCon(this,<%=conditionid%>)" style="margin-left: 3px;width:12px;cursor: pointer;" align="absmiddle"/></td>
	    <%
	      for(int i=0;i<totaldateList.size();i++){
	    	if(i<reportList.size()){
	    	   Map map=(Map)reportList.get(i);
 ...
 %>
  ...
  <%} %>	    
	    ...
	    <td><span title="<%=SystemEnv.getHtmlLabelName(15178,user.getLanguage())%><%=totalUnsubmit%><%=SystemEnv.getHtmlLabelName(1925,user.getLanguage())%><%=SystemEnv.getHtmlLabelName(26932,user.getLanguage())%><%=totalWorkday*totalAttention%><%=SystemEnv.getHtmlLabelName(1925,user.getLanguage())%>"><%=reportManager.getReportIndexStar(workIndex)%></span><%=workIndex%><a href="javascript:openChart('blog','<%=value%>',<%=year%>,<%=type%>,'<%=orgName%><%=SystemEnv.getHtmlLabelName(26467,user.getLanguage())+SystemEnv.getHtmlLabelName(15101,user.getLanguage())%>')" style="text-decoration: none"><img src="images/chart.png" style="margin-left: 3px;border: 0px" align="absmiddle" /></a></td>
	</tr>
  <%if(isAppend.equals("false")){ %>	
	</table>  
  <%}}else{ 
