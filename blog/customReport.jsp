...
<div style="width:100%;">
  <div align="center" id="reportTitle" style="margin-top: 8px;" >
    ...
    <a href="#" id="editTitle" onclick="editTitle()" style="margin-left: 8px;display: <%=isnew.equals("true")?"":"none"%>"><%=SystemEnv.getHtmlLabelName(93,user.getLanguage())%></a><!-- 编辑 -->
    <a href="#" id="savaTitle" onclick="saveTitle()" style="margin-left: 8px;display: none"><%=SystemEnv.getHtmlLabelName(86,user.getLanguage())%></a><!-- 保存 -->
  </div>
  <div style="margin-top: 3px;margin-bottom: 15px">
	...
	<div align="left" style="margin-top: 8px">
	  <div style="float: left;" class="reportTab">
		  ...
		  <a href="#" onclick="changeReport(this,'blog')" style="margin-right: 8px;<%="blog".endsWith(reportType)?"font-weight:bold;text-decoration:underline !important":"" %>"><%=SystemEnv.getHtmlLabelName(26467,user.getLanguage())+SystemEnv.getHtmlLabelName(15101,user.getLanguage())%></a><!-- 微博报表 -->
		  <%if(appDao.getAppVoByType("mood").isActive()){ %>
		  <a href="#" onclick="changeReport(this,'mood')" style="margin-right: 8px;<%="mood".endsWith(reportType)?"font-weight:bold;text-decoration:underline !important":"" %>"><%=SystemEnv.getHtmlLabelName(26769 ,user.getLanguage())+SystemEnv.getHtmlLabelName(15101,user.getLanguage())%></a><!-- 心情报表 -->
		  <%} %>
	  </div>
...
<div>
    ...
    <div id="condition"  style="margin-right: 8px;float: right;margin-top:8px;margin-bottom: 20px;display: <%=isnew.equals("true")?"":"none"%>">
       <a class="btnEcology" href="#" onclick="addCondition()">
			<div class="left" style="width:60px"><span ><%=SystemEnv.getHtmlLabelName(15582,user.getLanguage())%></span></div><!-- 添加条件 -->
			<div class="right"> &nbsp;</div>
	   </a>
    </div>
</div>
 ...
 function changeYear(){
  ...
  function compareChart(reportdiv,charType){
   ...
   if(conditions.length==0){
      alert("<%=SystemEnv.getHtmlLabelName(18131,user.getLanguage())%>");//请选择条件
      return ;
   }
   ...
   function addCondition(){
        index++;
        var str="<div class='conDiv' id='con_"+index+"'><div class='conType'><%=SystemEnv.getHtmlLabelName(15505,user.getLanguage())%>："+getShareTypeStr()+getShareContentStr()+ //报表条件
        "</div><div class='operationdiv'>"+
        "<div class='operationItem'><a class='btnEcology' href='#' onclick='deleteCon("+index+")'><div class='left' style='width:45px'><span ><%=SystemEnv.getHtmlLabelName(91,user.getLanguage())%></span></div><div class='right'> &nbsp;</div></a></div>"+	//删除
   ...
   function isCanView(attentionid){
    ...
    function createReport(index){
       ...
       jQuery.post("customReportRecord.jsp?reportType=<%=reportType%>&tempid=<%=tempid%>&isAppend="+isAppend+"&isExtent=false&year="+year+"&month="+month+"&value="+value+"&index="+index+"&type="+type+"&reportType="+reportType,function(data){
          data=jQuery.trim(data);
          if(data=="null")
             alert("<%=SystemEnv.getHtmlLabelName(26161,user.getLanguage())+SystemEnv.getHtmlLabelName(19040,user.getLanguage())%>"); //没有可查看人员 
          else{   
	          if(isAppend)
