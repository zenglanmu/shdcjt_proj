...
<table id="reportList" style="width:100%;;border-collapse:collapse;margin-top: 3px"  border="1" cellspacing="0" bordercolor="#9db1ba" cellpadding="2" >
	  ...
	  <tr style="background: url('images/report-head-bg.png');background-repeat: repeat-x;" height="20px">
	    ...
	    %>
	          <td class="tdWidth" nowrap="nowrap"><%=day%></td>
	    <%}}%>
	    <td style="width: 5%" align="center"><%=SystemEnv.getHtmlLabelName(26960,user.getLanguage())%></td><!-- 指数 -->
	</tr>
	<!-- 日期行  -->
	
	<!-- 微博报表  -->
	<tr class="item1">
	    <td><%=SystemEnv.getHtmlLabelName(26467,user.getLanguage())+SystemEnv.getHtmlLabelName(15101,user.getLanguage())%></td> <!-- 微博报表 -->
	    <%
	      for(int i=0;i<totaldateList.size();i++){
	    	if(i<reportList.size()){
	    	   Map map=(Map)reportList.get(i);
	...
	<tr class="item1">
	    <td><%=SystemEnv.getHtmlLabelName(26769 ,user.getLanguage())+SystemEnv.getHtmlLabelName(15101,user.getLanguage())%></td><!-- 心情报表 -->
	 ...
	 %>
	<tr class="item1">
	    <td><%=SystemEnv.getHtmlLabelName(15880 ,user.getLanguage())+SystemEnv.getHtmlLabelName(15101,user.getLanguage())%></td><!-- 考勤报表 -->
	    <%for(int i=0;i<totaldateList.size();i++){ 
	    	if(i<reportList.size()){
