...
<%
	...
	//排序
	Collections.sort(moduleorder,String.CASE_INSENSITIVE_ORDER);
%>
				...
				<TABLE class=ViewForm>
					...
					<TBODY>
						...
						<tr>
							...
							<tr class=DataHeader>
								<th></td>
								<th><%=SystemEnv.getHtmlLabelName(606, user.getLanguage())%></td>
								<th><%=SystemEnv.getHtmlLabelName(19049, user.getLanguage())%></td>
								<th><%=SystemEnv.getHtmlLabelName(15603, user.getLanguage())%></td>
								<th><%=SystemEnv.getHtmlLabelName(15513, user.getLanguage())%></td>
								<th><%=SystemEnv.getHtmlLabelName(19480, user.getLanguage())%></td>
								<%--暂时屏蔽图标设置
							...
							for(int i=0;i<moduleorder.size();i++){
								String index = (String)moduleorder.get(i);
							...
							<tr>
								...
								<td style="display:none">
									...
									<div id="iconUpload_<%=modulescope%>" style="display:<% if(icon!=null&&!"".equals(icon)){ %>none<%}else{%>block<%}%>">
										<input type="file" id="icon_<%=modulescope%>" name="config_iconfile_<%=modulescope%>" class="InputStyle" style="width:90%">
									</div>
								</td>
							</tr>
							<TR style="height:1px;">
...
<script type="text/javascript">
	...
	function onSelectMultiCatalog(input,span,scope){
		var vbid = window.showModalDialog("/systeminfo/BrowserMain.jsp?url=/docs/category/MutilCategoryBrowser.jsp?para="+$("#"+input).val());
		if(vbid) {
			clearDocumentForm();
			document.getElementById("cate_0").checked = true;
			if (vbid.id!="") {
