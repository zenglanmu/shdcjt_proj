...
function onSave(){
	...
	if(!checkresult) return checkCanSubmit("onSave",null);   
   ...
   <%}else{%>
    ...
    <%
					...
					alert("<%=SystemEnv.getHtmlLabelName(26090,user.getLanguage())%>");
					return;
				}
			  }
			}
...
function docCheckIn(docId) {
	...
	document.getElementById("DocCheckInOutUtilIframe").src="/docs/docs/DocCheckInOutUtilIframe.jsp?operation=docCheckIn&docId="+docId+"&userId=<%=user.getUID()%>";
}
...
function docVersionWord(vid){
	...
	if(id!=null&&id!='undefined')
    	if(id.id!="")
    		window.location="DocEditExt.jsp?id=<%=docid%>&versionId="+id.id+"&fromFlowDoc=<%=fromFlowDoc%>&requestid=<%=requestid%>&isFromAccessory=<%=isFromAccessory%>&isNoTurnWhenHasToPage=true&topage=<%=URLEncoder.encode(topageFromOther)%>";
}
