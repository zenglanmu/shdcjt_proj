...
if(method.equalsIgnoreCase("saveastmpl")){
    ...
    RecordSet.executeSql(
		...
		" editionPrefix,readerCanViewHistoryEdition,isOpenApproveWf,validityApproveWf,invalidityApproveWf,useCustomSearch,defaultdummycata" +
		" ," + fromdir +
		" ,appointedWorkflowId " + 
		" ,isPrintControl " +
		" ,printApplyWorkflowId " +
    ...
    if(tmplId>0){
        ...
        RecordSet.executeSql(
            ...
            " operationcode,userid,subcompanyid,DocSecCategoryTemplateId " +
            " ) " +
            " ( select " +
            " -1,-dirtype,seclevel,departmentid,roleid,rolelevel,usertype,permissiontype," +
            " operationcode,userid,subcompanyid," + tmplId +
			" from DirAccessControlList where dirid=" + secid + " and dirtype=2 " +
			" ) "
        );
        ...
        RecordSet.executeSql(
                ...
                " departmentid,roleid,foralluser,crmid,downloadlevel,DocSecCategoryTemplateId" +
                " ) " +
                " ( select " +
                " -1,sharetype,seclevel,rolelevel,sharelevel,userid,subcompanyid," +
                " departmentid,roleid,foralluser,crmid,downloadlevel," + tmplId +
    			" from DocSecCategoryShare where seccategoryid = " + secid +
    			" ) "
        );
        ...
        while(RecordSet.next()){
            ...
            if(tmpId > 0){
                ...
                RecordSet1.executeSql(
	                    ...
	                    " " + isCustom + ",'" + newScope + "'," + newScopeId + "," + newFieldId + "," + tmplId +
    ...
    String sql =
    		...
    		" ,isPrintControl,printApplyWorkflowId,relationable,maxOfficeDocFileSize,isOpenAttachment,isAutoExtendInfo" +
    ...
    RecordSet.executeSql(
    		...
    		" from DirAccessControlList where DocSecCategoryTemplateId=" + tmplId +
    		" ) "
    );
    /*RecordSet.executeSql(
    ...
    RecordSet.executeSql(
			...
			" from DocSecCategoryShare where DocSecCategoryTemplateId=" + tmplId +
			" ) "
    );
    ...
    while(RecordSet.next()){
        ...
        if(tmpId > 0){
            ...
            RecordSet1.executeSql(
                    ...
                    " ( select " + secid + "," +
                    " viewindex,type,labelid,visible,customName,columnWidth,mustInput," +
                    " " + isCustom + ",'" + newScope + "'," + newScopeId + "," + newFieldId + "," + 0 +
        			" from DocSecCategoryDocProperty where id = " + tmpId +
        			" ) "
            );
    ...
    if(tmplId>0){
        RecordSet.executeSql("delete from DocSecCategoryTemplate where id = " + tmplId);

        RecordSet.executeSql("delete from DirAccessControlList where DocSecCategoryTemplateId = " + tmplId);
        
        //RecordSet.executeSql("delete from DirAccessPermission where DocSecCategoryTemplateId = " + tmplId);

        RecordSet.executeSql("delete from secCreaterDocPope where DocSecCategoryTemplateId = " + tmplId);

        RecordSet.executeSql("delete from DocSecCategoryShare where DocSecCategoryTemplateId = " + tmplId);

        RecordSet.executeSql("delete from codedetail where codemainid in (select id from codemain where DocSecCategoryTemplateId = " + tmplId + ")");
        RecordSet.executeSql("delete from codemain where DocSecCategoryTemplateId = " + tmplId);
        
        RecordSet.executeSql("delete from cus_formfield where scope='DocCustomFieldBySecCategoryTemplate' and scopeid=" + tmplId);

        RecordSet.executeSql("delete from DocSecCategoryDocProperty where DocSecCategoryTemplateId = " + tmplId);

        RecordSet.executeSql("delete from DocSecCategoryMouldBookMark where DocSecCategoryMouldId in (select id from DocSecCategoryMould where DocSecCategoryTemplateId = " + tmplId + ")");
        RecordSet.executeSql("delete from DocSecCategoryMould where DocSecCategoryTemplateId = " + tmplId);
        
        RecordSet.executeSql("delete from DocSecCategoryApproveWfDetail where DocSecCategoryTemplateId = " + tmplId);
        
        RecordSet.executeSql("delete from DocSecCategoryCusSearch where DocSecCategoryTemplateId = " + tmplId);
    }
