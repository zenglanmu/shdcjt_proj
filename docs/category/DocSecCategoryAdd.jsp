...
int id = -1;
int subid = Util.getIntValue(request.getParameter("id"),0);
    ...
    <TD vAlign=top>
        ...
        <TABLE class=ViewForm>
          ...
          <TR class=Title>
            ...
            <td align=right><A href="DocMainCategory.jsp" target="mainFrame"><%=SystemEnv.getHtmlLabelName(65,user.getLanguage())%></A> > <A href="DocMainCategoryEdit.jsp?id=<%=mainid%>" target="contentframe"><%=MainCategoryComInfo.getMainCategoryname(""+mainid)%></A> > <A href="DocSubCategoryEdit.jsp?id=<%=subcategoryid%>" target="contentframe"><%=SubCategoryComInfo.getSubCategoryname(subcategoryid) %></A>
            </td>
          </TR>
...
<TR><TD class=Line colSpan=2></TD></TR>
          <tr>
            ...
            <td class=Field>
              <%if(canEdit){%>
              <button class=Browser onclick="onShowDirMould()"></button>
              <%}%>
...
<TR style="height: 1px!important;"><TD class=Line colSpan=2></TD></TR>
        <TR>
            ...
            <TD class=Field>
                 <INPUT class=InputStyle type=checkbox <%if(noDownload==1){%>checked<%}%> value=1 name="noDownload" onclick="setBacthDownload(this);" <%if(!canEdit){%>disabled<%}%>>
            </TR>
 ...
 <%-- Office文档最大 --%>
        ...
        <TR>
            ...
            <TD class=Field>
                <INPUT class=InputStyle type=input size=4 defValue="8" onchange="checkPositiveNumber('<%=SystemEnv.getHtmlLabelName(24024,user.getLanguage())%>',this)" name="maxOfficeDocFileSize" onKeyPress="ItemCount_KeyPress()" value="8" <%if(!canEdit){%>disabled<%}%>>M（<%=SystemEnv.getHtmlLabelName(24025,user.getLanguage())%>）
          </TR>
<%-- 附件上传最大 --%>            
        <TR style="height: 1px!important;"><TD class=Line colSpan=2></TD></TR>
        <TR>
            ...
            <TD class=Field>
                <INPUT class=InputStyle type=input size=4 defValue="<%=maxUploadFileSize%>" onchange="checkPositiveNumber('<%=SystemEnv.getHtmlLabelName(18580,user.getLanguage())%>',this)" name="maxUploadFileSize" value="<%=maxUploadFileSize%>" <%if(!canEdit){%>disabled<%}%>>M
          </TR>
...
<TR style="height: 1px;"><TD class=Line colSpan=2></TD></TR>
        <TR>
            ...
            <TD class=Field>
                <INPUT class=InputStyle type=checkbox <%if(bacthDownload==1){%>checked<%}%>  value=1 name="bacthDownload" <%if(!canEdit){%>disabled<%}%>>
            </TR>          
        ...
        <TR>
            ...
            <TD class=Field>							
				<%if(canEdit){%>
					...
					<INPUT  class="wuiBrowser" _url="/systeminfo/BrowserMain.jsp?url=/docs/category/DocTreeDocFieldBrowserMulti.jsp" type=hidden value="" name="defaultDummyCata" id="defaultDummyCata">
...
<%-- 新建工作流指定流程 --%>
        ...
        <TR>
            ...
            <TD class=Field>							     		
				<%if(canEdit){%>
							<INPUT class=wuiBrowser _url="/systeminfo/BrowserMain.jsp?url=/workflow/workflow/WorkflowBrowser.jsp?isValid=1" type=hidden name="appointedWorkflowId" id="appointedWorkflowId" value="0" >
							<!-- 
							<button class=Browser onClick="onShowAppointedWorkflow('appointedWorkflowId','appointedWorkflowNameSpan')"></button> 
							<INPUT  type=hidden name="appointedWorkflowId" id="appointedWorkflowId" value="0" >
							<span id="appointedWorkflowNameSpan"></span>	
							 -->		
				<%}%>
            </TR>          
...
<TR style="height: 1px!important;"><TD class=Line colSpan=2></TD></TR>
        ...
        <TR>
          ...
          <TD class=Field><INPUT maxLength=5 size=5 class=InputStyle name="secorder" onKeyPress="ItemNum_KeyPress()" onBlur='checknumber("secorder")' value=""></TD>
         </TR>
		<TR style="height: 1px!important;"><TD class=Line colSpan=2></TD></TR>  
          </TBODY>
        </TABLE>
...
<%
	...
	if("1".equals(isUseFTPOfSystem)){
		...
		String refreshSec="";
		RecordSet.executeSql("select * from DocSubCatFTPConfig where subCategoryId=" + subid);
		if(RecordSet.next()){
			refreshSec = Util.null2String(RecordSet.getString("refreshSec"));
			if(refreshSec.equals("1")){
      ...
      --%>
        ...
        <table class=ViewForm style="display:none">
          ...
          <tr>
            ...
            <td class=Field>
              <%if(canEdit){%>
              <input class='wuiBrowser' type=hidden name="approvewfid" value="<%=approvewfid%>" _url="/systeminfo/BrowserMain.jsp?url=/workflow/workflow/WorkflowBrowser.jsp?sqlwhere= where isbill=1 and formid=28" _displayText="">
